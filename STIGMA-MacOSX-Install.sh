...
if [ -f "/opt/local/bin/port" ]; then
  ...
  env PATH=$PATH:/opt/homebrew/bin brew install curl wget make cmake pkg-config pugixml wavpack flac opusfile mpg123 libmodplug libsndfile libogg libsndfile libvorbis vorbis-tools sdl2 openal-soft alure fluid-synth libxmp dumb boost
