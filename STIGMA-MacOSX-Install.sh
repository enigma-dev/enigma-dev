...
if [ -f "/opt/local/bin/port" ]; then
  ...
  brew install curl wget cmake pkg-config pugixml wavpack flac opusfile mpg123 libmodplug libsndfile libogg libsndfile libvorbis vorbis-tools openal-soft alure fluid-synth libxmp dumb
  sudo port -N install gcc12 gmake protobuf-c libsdl2 glfw glew glm libpng libvpx box2D freetype libffi rapidjson libyaml pugixml yaml-cpp grpc boost
  sudo port -N reclaim
