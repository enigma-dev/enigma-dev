...
if [ -f "/opt/local/bin/port" ]; then
  ...
  brew install curl wget gdb gcc make cmake pkg-config flac opusfile mpg123 libmodplug libsndfile libogg libsndfile libvorbis vorbis-tools sdl2 openal-soft alure dumb boost
  sudo port -N install protobuf-c glfw glew glm libpng libvpx box2D freetype libffi rapidjson libyaml pugixml yaml-cpp grpc
  sudo port -N reclaim
