...
static bool menu_add_item_ext(HMENU menu, int iid, int id, string str, unsigned int type, unsigned int state,
...
int show_menu(int x, int y, string text) {
  ...
  int rv =
      TrackPopupMenuEx(menu, TPM_LEFTBUTTON | TPM_RETURNCMD | TPM_LEFTALIGN | TPM_TOPALIGN, x, y, enigma::hWnd, NULL);
...
int show_menu_ext(int x, int y, string text) {
  ...
  int rv = TrackPopupMenuEx(ms->menu, TPM_LEFTBUTTON | TPM_RETURNCMD | TPM_LEFTALIGN | TPM_TOPALIGN, x, y, enigma::hWnd,
                            NULL);
  SendMessage(enigma::hWnd, WM_NULL, 0, 0);
  delete ms;
  return rv;
