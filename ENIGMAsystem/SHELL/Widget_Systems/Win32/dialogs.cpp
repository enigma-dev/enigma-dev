...
void show_error(string errortext, const bool fatal) {
  ...
  if (MessageBox(NULL, errortext.c_str(), "Error", MB_ABORTRETRYIGNORE | MB_ICONERROR) == IDABORT) exit(0);
...
static INT CALLBACK GetDirectoryAltProc(HWND hwnd, UINT uMsg, LPARAM lp, LPARAM pData) {
  if (uMsg == BFFM_INITIALIZED) SetWindowText(hwnd, gs_cap.c_str());
...
void message_caption(bool show, string str) { gs_cap = str; }
...
void show_info(string info, int bgcolor, int left, int top, int width, int height, bool embedGameWindow,
  ...
  if (embedGameWindow) {
    ...
    WNDCLASS wc = {CS_VREDRAW | CS_HREDRAW,
                   (WNDPROC)ShowInfoProc,
                   0,
                   0,
                   enigma::hInstance,
                   0,
    ...
    parent = CreateWindow("infodialog", TEXT(caption.c_str()), flags, left, top, width, height, enigma::hWnd, 0,
                          enigma::hInstance, 0);
  ...
  enigma::infore =
      CreateWindowEx(WS_EX_TOPMOST, "RICHEDIT", TEXT("information text"),
                     ES_LEFT | ES_MULTILINE | ES_AUTOVSCROLL | ES_AUTOHSCROLL | ES_WANTRETURN | ES_READONLY | WS_CHILD |
  ...
  SendMessage(enigma::infore, EM_SETBKGNDCOLOR, (WPARAM)0,
              (LPARAM)RGB(__GETR(bgcolor), __GETG(bgcolor), __GETB(bgcolor)));
...
bool get_string_canceled() { return gs_form_canceled; }
...
string get_open_filename(string filter, string filename, string caption) {
  ...
  ofn.lpstrTitle = caption.length() ? caption.c_str() : NULL;
