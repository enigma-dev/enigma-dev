...
struct gtkl_widget : gtkl_placer {
  ...
  gtkl_widget(int id, ui_handle me, int at, int w, int h)
      : gtkl_placer(id, w, h, o_widget), me(me), actions(0), attribs(at) {}
...
struct gtkl_table : gtkl_container {
  ...
  bool reserved(char cid) { return atts.find(cid) != atts.end(); }
  ...
  void resize(int xnew, int ynew, int dwid, int dhgt) {
    ...
    for (int i = 0; i < gsx; i++)
      for (int ii = 0; ii < gsy; ii++) givex[i][ii] = cw, givey[i][ii] = ch;
    for (atti mi = atts.begin(); mi != atts.end(); mi++) {
      // Make all allocated cells not give at all
      for (int xz = mi->second.x; xz < mi->second.r; xz++)
        for (int yz = mi->second.y; yz < mi->second.b; yz++) givex[xz][yz] = givey[xz][yz] = 0;
      // Make not-completely-occupied allocated cells have max give
      for (int xz = mi->second.x; xz < mi->second.r; xz++)
        for (int yz = mi->second.y + (mi->second.child->srh + 8) / ch; yz < mi->second.b; yz++) givey[xz][yz] = ch;
      for (int yz = mi->second.y; yz < mi->second.b; yz++)
        for (int xz = mi->second.x + (mi->second.child->srw + 8) / cw; xz < mi->second.r; xz++) givex[xz][yz] = cw;
      // Decrease vertical give (for each horizontal unit)
      for (int xz = min(mi->second.x + (mi->second.child->srw + 8) / cw, mi->second.r - 1), yz = mi->second.y;
           yz < mi->second.b; yz++)
        givex[xz][yz] = cw * (xz + 1 - mi->second.x) - (mi->second.child->srw + 8);
      // Decrease horizontal give (for each vertical unit)
      for (int yz = min(mi->second.y + (mi->second.child->srh + 8) / ch, mi->second.b - 1), xz = mi->second.x;
           xz < mi->second.r; xz++)
    ...
    for (atti mi = atts.begin(); mi != atts.end(); mi++)
      mi->second.d.x = mi->second.x * cw, mi->second.d.y = mi->second.y * ch,
      mi->second.d.w = (mi->second.child->srw + 8), mi->second.d.h = (mi->second.child->srh + 8);

    // Expand grid where absolutely necessary
    int maxgx[gsx], maxgy[gsy];  //Compute resize allotment
    for (int xx = 0; xx < gsx; xx++) maxgx[xx] = cw;
    for (int yy = 0; yy < gsy; yy++) maxgy[yy] = ch;
    for (int xx = 0; xx < gsx; xx++)
      for (int yy = 0; yy < gsy; yy++) {
        if (givex[xx][yy] < maxgx[xx]) maxgx[xx] = givex[xx][yy];
        if (givey[xx][yy] < maxgy[xx]) maxgy[xx] = givey[xx][yy];
      }

    int losx = 0, losy = 0;  // Total left-over space (always <= 0)
    for (int i = 0; i < gsx; i++) losx += maxgx[i];
    for (int i = 0; i < gsy; i++) losy += maxgy[i];
    for (atti mi = atts.begin(); mi != atts.end(); mi++)
      mi->second.child->resize(x + mi->second.d.x, y + mi->second.d.y, mi->second.d.w, mi->second.d.h);

    // Expand items to fill new bounds
    for (atti mi = atts.begin(); mi != atts.end(); mi++) {
      for (int yy = mi->second.y, xx = mi->second.x; xx < mi->second.r; xx++) mi->second.d.w += givex[xx][yy];
      for (int xx = mi->second.x, yy = mi->second.y; yy < mi->second.b; yy++) mi->second.d.h += givey[xx][yy];
    }
    for (atti mi = atts.begin(); mi != atts.end(); mi++)
      mi->second.child->resize(x + mi->second.d.x, y + mi->second.d.y, mi->second.d.w, mi->second.d.h);
  }
  void resolve() {
    ...
    for (atti mi = atts.begin(); mi != atts.end(); mi++) mi->second.child->resolve();

    // Calculate minimal X size
    for (int xx = 0; xx < gsx; xx++) {
      int tc = 0;
      for (atti mi = atts.begin(); mi != atts.end(); mi++)
        if (mi->second.x <= xx and mi->second.r > xx) tc += mi->second.child->srh;
      if (tc > srh) srh = tc;
    }
    ...
    for (int yy = 0; yy < gsy; yy++) {
      int tr = 0;
      for (atti mi = atts.begin(); mi != atts.end(); mi++)
        if (mi->second.y <= yy and mi->second.b > yy) tr += mi->second.child->srw;
      if (tr > srw) srw = tr;
    }
    srw += 8, srh += 8;
  }
