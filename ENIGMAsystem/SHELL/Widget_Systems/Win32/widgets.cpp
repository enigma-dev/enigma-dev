...
struct enigma_widget_alignment : gtkl_container {
  ...
  void resolve() {
    if (child) child->resolve(), srw = child->srw + 8, srh = child->srh + 8;
  }
  void resize(int x, int y, int w, int h) {
    if (child) child->resize(x + 4, y + 4, w - 8, h - 8);
  }
};

bool wgt_exists(int id) {
  return unsigned(id) < widgets.size() and widgets[id] and
         (widgets[id]->type != o_widget or IsWindow(getWidget(id)->me));
}
...
static LRESULT CALLBACK GenWindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
  switch (message) {
    case WM_COMMAND:
      if (HIWORD(wParam) == BN_CLICKED) getWidget((int)getID(lParam))->actions++;
      break;
    case WM_SIZE: {
...
int wgt_window_create(int w, int h) {
  HWND win = CreateWindowEx(0, enigma_window_generic_class, "caption", WS_POPUP | WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
                            CW_USEDEFAULT, w, h, enigma::hWnd, (HMENU)(ptrdiff_t)widget_idmax, enigma::hInstance,
                            (LPVOID)(ptrdiff_t)widget_idmax);
  if (!win) return -1;
  setID(win, widget_idmax);
  fixFont(win);
...
void wgt_window_show(int wgt) {
  ...
  AdjustWindowRect(&r, WS_CAPTION | WS_THICKFRAME, FALSE);
  SetWindowPos(getWidget(wgt)->me, NULL, 0, 0, r.right - r.left, r.bottom - r.top,
               SWP_SHOWWINDOW | SWP_NOZORDER | SWP_NOREPOSITION);
...
int wgt_layout_create(int win, string layout, int hpad, int vpad) {
  ...
  for (unsigned y = 0; y < rows.size(); y++)
    for (unsigned x = 0; x < cc; x++) {
      ...
      if (!table->attach(wa, x, x + cs, y, y + rs, cat)) delete wa;
    }
  log_enigma_widget(table);
  window->layout_id = widget_idmax;
  RECT wr;
  GetClientRect(window->me, &wr);
...
void enigma_button::resolve() {
  ...
  char t[tl + 1];
  Button_GetText(me, t, tl + 1);
  SIZE d;
  GetTextExtentPoint32(dc, t, tl, &d);
  ReleaseDC(me, dc);
  srw = d.cx + 8, srh = d.cy + 8;
}

int wgt_button_create(string text) {
  HWND butn = CreateWindow("button", text.c_str(), BS_PUSHBUTTON, 0, 0, 24, 24, NULL, (HMENU)NULL, NULL,
                           (LPVOID *)(ptrdiff_t)widget_idmax);
  if (!butn) return -1;
  setID(butn, widget_idmax);
  fixFont(butn);
...
struct enigma_combobox : enigma_widget {
  ...
  void resize(int xr, int yr, int wr, int hr) {
    x = xr, y = yr, w = wr, h = 24;
    MoveWindow(me, x, y, w, h + 128, 1);
  }
...
int wgt_combobox_create(string contents) {
  HWND cbox =
      CreateWindow("combobox", "", CBS_DROPDOWNLIST | CBS_HASSTRINGS, 0, 0, 24, 144, NULL, (HMENU)NULL, NULL, NULL);
  setID(cbox, widget_idmax);
  fixFont(cbox);
...
int wgt_combobox_get_selection(int cbbox) { return ComboBox_GetCurSel(getWidget(cbbox)->me); }
...
int wgt_checkbox_create(string text) {
  ...
  setID(chbox, widget_idmax);
  fixFont(chbox);
  log_enigma_widget(new enigma_checkbox(widget_idmax, chbox, BS_AUTOCHECKBOX, 24, 24));
  return widget_idmax++;
}
bool wgt_checkbox_get_checked(int cbox) { return Button_GetCheck(getWidget(cbox)->me); }
