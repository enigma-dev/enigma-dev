...
static void widget_destroy(GtkObject *who, gpointer wgtid) {
  ...
  if (widgets[(int)long(wgtid)]->occc.size())
    printf("  (and %lu children)\n", widgets[(int)long(wgtid)]->occc.size()), widgets[(int)long(wgtid)]->occc.clear();
}

inline void log_enigma_widget(enigma_widget *wgt) {
  const int ind = widgets.size();
  widgets.push_back(wgt);
  g_signal_connect(wgt->me, "destroy", G_CALLBACK(widget_destroy), (void *)ind);
}

bool wgt_exists(int id) { return unsigned(id) < widgets.size() and widgets[id]->exists; }
  ...
  for (unsigned y = 0; y < rows.size(); y++)
    for (unsigned x = 0; x < cc; x++) {
      ...
      if (insd.second) {
        ...
        gtk_table_attach(GTK_TABLE(tbl), insd.first->second, x, x + cs, y, y + rs, GTK_EXPAND_FILL, GTK_EXPAND_FILL,
                         hpad, vpad);
      }
    }
...
static void button_callback(GtkButton *button, void *user_data) { widgets[(int)long(user_data)]->actions++; }
