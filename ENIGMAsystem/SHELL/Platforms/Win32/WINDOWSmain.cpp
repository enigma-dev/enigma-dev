...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int iCmdShow) {
  ...
  if ((argv = CommandLineToArgvW(GetCommandLineW(), &enigma::main_argc))) {
    for (int i = 0; i < enigma::main_argc; i++) {
      char buffer[256];
      snprintf(buffer, 256, "%ls",
               argv[i]);  //Maybe we should check and show some warning if the argument was longer than 256
      string param = buffer;
  ...
  enigma::hWnd =
      CreateWindow("EnigmaDevGameMainWindow", "", (enigma::getwindowstyle() & ~(WS_VISIBLE)), (screen_width - wid) / 2,
...
string parameter_string(int x) {
  if (x < enigma::main_argc) return enigma::main_argv[x];

  return "";
}

int parameter_count() { return enigma::main_argc; }
...
unsigned long long disk_size(std::string drive) {
  ...
  if (!GetDiskFreeSpace((drive == "") ? NULL : drive.c_str(), &sectorsPerCluster, &bytesPerSector, &freeClusters,
                        &totalClusters))
    return 0;
...
void action_webpage(const std::string &url) { ShellExecute(NULL, "open", url.c_str(), NULL, NULL, SW_SHOWNORMAL); }
