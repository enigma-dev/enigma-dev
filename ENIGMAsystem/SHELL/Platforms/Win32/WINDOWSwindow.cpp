...
static int displayInitialResolutionWidth = 0, displayInitialResolutionHeight = 0, displayInitialBitdepth = 0,
           displayInitialFrequency = 0;
...
LONG_PTR getwindowstyle() {
  ...
  if (isVisible) newlong |= WS_VISIBLE;
...
void clampwindow() {
  RECT c;
  c.left = enigma::windowX;
  c.top = enigma::windowY;
  c.right = enigma::windowX + enigma::windowWidth;
  c.bottom = enigma::windowY + enigma::windowHeight;
...
void setwindowsize() {
  ...
  int parWidth = isFullScreen ? GetSystemMetrics(SM_CXSCREEN) : windowWidth,
      parHeight = isFullScreen ? GetSystemMetrics(SM_CYSCREEN) : windowHeight;
...
void window_set_alpha(double alpha) {
  ...
  SetWindowLong(enigma::hWnd, GWL_EXSTYLE, GetWindowLong(enigma::hWnd, GWL_EXSTYLE) | WS_EX_LAYERED);
...
bool window_get_showborder() { return enigma::showBorder; }
...
void window_set_visible(bool visible) {
  if (enigma::isVisible == visible) return;
...
int window_get_visible() { return enigma::isVisible; }

void window_set_minimized(bool minimized) { ShowWindow(enigma::hWnd, (minimized ? SW_MINIMIZE : SW_RESTORE)); }

void window_set_maximized(bool maximized) { ShowWindow(enigma::hWnd, (maximized ? SW_MAXIMIZE : SW_RESTORE)); }
...
bool window_get_stayontop() { return enigma::stayOnTop; }
...
void display_reset() {
  ...
  if (!EnumDisplaySettings(NULL, ENUM_CURRENT_SETTINGS, &devMode)) return;
...
bool display_set_size(int w, int h) {
  ...
  if (displayInitialResolutionWidth == 0) displayInitialResolutionWidth = devMode.dmPelsWidth;

  if (displayInitialResolutionHeight == 0) displayInitialResolutionHeight = devMode.dmPelsHeight;
...
bool display_set_frequency(int freq) {
  ...
  if (displayInitialFrequency == 0) displayInitialFrequency = devMode.dmBitsPerPel;
...
bool display_set_colordepth(int depth) {
  ...
  if (displayInitialBitdepth == 0) displayInitialBitdepth = devMode.dmBitsPerPel;
...
bool display_set_all(int w, int h, int freq, int bitdepth) {
  ...
  if (freq != -1) {
    if (displayInitialFrequency == 0) displayInitialFrequency = devMode.dmDisplayFrequency;
