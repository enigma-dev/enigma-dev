...
struct e_joystick {
  ...
  e_joystick(int d, string n, int ac, int bc)
      : device(d),
        name(n),
        axiscount(ac),
        buttoncount(bc),
        ...
        button(new bool[bc]),
        threashold(.25),
        wrapaxis_positive(new char[ac]),
        wrapaxis_negative(new char[ac]),
        wrapbutton(new char[bc]) {
    for (int i = 0; i < 8; i++) axis[i] = wrapaxis_positive[i] = wrapaxis_negative[i] = wrapbutton[i] = button[i] = 0;
    for (int i = 8; i < 16; i++) wrapbutton[i] = button[i] = 0;
  }

  ~e_joystick() { close(device); }
};
...
static void handle_joystick(e_joystick *my_joystick) {
  ...
  while (rd and rd != -1) {
    ...
    if (a.type != JS_EVENT_BUTTON)
      ...
      if (my_joystick->wrapbutton[a.number]) enigma::keybdstatus[(int)my_joystick->wrapbutton[a.number]] = a.value;
    }
...
void handle_joysticks() {
  for (unsigned i = 0; i < joysticks.size(); i++)
    if (joysticks[i]) handle_joystick(joysticks[i]);
}
}  // namespace enigma
...
bool joystick_load(int id) {
  ...
  int ac = 4, bc = 4;
  if (ioctl(device, JSIOCGAXES, &ac) or ioctl(device, JSIOCGBUTTONS, &bc)) return (close(device), false);

  char name[256];
  name[0] = 0;
  if (ioctl(device, JSIOCGNAME(256), name) > 0) devn = name;
...
void joystick_map_button(int id, int butnum, char key) {
  checkId(void());
  if ((unsigned)id >= enigma::joysticks.size()) return;
  enigma::e_joystick *js = enigma::joysticks[id];
  if (js and butnum < js->buttoncount) js->wrapbutton[butnum] = key;
}
...
void joystick_map_axis(int id, int axisnum, char keyneg, char keypos) {
  ...
  if ((unsigned)id >= enigma::joysticks.size()) return;
  enigma::e_joystick *js = enigma::joysticks[id];
  if (js and axisnum < js->axiscount) js->wrapaxis_negative[axisnum] = keyneg, js->wrapaxis_positive[axisnum] = keypos;
}
...
bool joystick_exists(int id) {
  if (size_t(id) >= enigma::joysticks.size()) return false;
  const enigma::e_joystick *const js = enigma::joysticks[id];
