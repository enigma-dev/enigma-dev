...
void setwindowsize(int forceX = -1, int forceY = -1) {
  ...
  int parWidth = isFullScreen ? XWidthOfScreen(screen) : windowWidth,
      parHeight = isFullScreen ? XHeightOfScreen(screen) : windowHeight;
...
void window_set_visible(bool visible) {
  if (visible) {
    ...
    if (visx != -1 && visy != -1) window_set_position(visx, visy);
  } else {
...
bool window_get_minimized() { return enigma::isMinimized; }

bool window_get_maximized() { return enigma::isMaximized; }
...
void window_set_rectangle(int x, int y, int w, int h) { XMoveResizeWindow(disp, win, x, y, w, h); }
...
void keyboard_wait() {
  ...
  for (;;) {
    io_handle();
    for (int i = 0; i < 255; i++)
      if (enigma::keybdstatus[i]) return;
