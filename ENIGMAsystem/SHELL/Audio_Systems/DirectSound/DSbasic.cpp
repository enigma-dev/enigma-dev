...
int sound_add(string fname, int kind, bool preload)  //At the moment, the latter two arguments do nothing! =D
{
  ...
  if (fread(fdata, 1, flen, afile) != flen) puts("WARNING: Resource stream cut short while loading sound data");
...
bool sound_replace(int sound, string fname, int kind, bool preload) { return false; }
