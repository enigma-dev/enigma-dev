...
double FMODBlockersFree() {
  if (!inited) {
    { FMODASSERT(FMOD_ERR_INITIALIZATION); }
  }
  if (geometry == NULL) {
    { FMODASSERT(FMOD_ERR_INITIALIZATION); }
  }
...
//static DWORD br = 0;
FMOD_RESULT F_CALLBACK myread(void *handle, void *buffer, unsigned int sizebytes, unsigned int *bytesread,
                              void *userdata) {
  if (!handle) {
    return FMOD_ERR_INVALID_PARAM;
  }
...
FMOD_RESULT F_CALLBACK myseek(void *handle, unsigned int pos, void *userdata) {
  ...
  if (pos != SetFilePointer((HANDLE)handle, pos, NULL, FILE_BEGIN)) return FMOD_ERR_FILE_COULDNOTSEEK;
...
double FMODinit(double maxsounds, double supportwebfiles = 0) {
  if (inited) {
    FMODASSERT(FMOD_ERR_INITIALIZED);
  }
  ...
  if (version < FMOD_VERSION) {
    ...
    MessageBoxA(GetActiveWindow(),
                "initFMOD-Incompatible fmodex.dll found. It's possible another game or product is running using and "
                "older version of this dll or the supplied fmod dll is incompatible.",
                "FMOD Error", MB_ICONSTOP);
  ...
  if (numdrivers == 0) {
    ...
    if (caps &
        FMOD_CAPS_HARDWARE_EMULATED) /* The user has the 'Acceleration' slider set to off!  This is really bad for latency!. */
    {                                /* You might want to warn the user about this. */
      MessageBeep(MB_ICONWARNING);
      MessageBoxA(GetActiveWindow(),
                  "You've turned windows 'Acceleration' too low and will experience major slow downs. You can fix this "
                  "in the control panel's hardware acceleration options. Location varies depending your Windows "
                  "version.",
                  "FMOD Warning", MB_ICONWARNING);
      //Dont care if this fails
      FMOD_System_SetDSPBufferSize(
          mainsystem, 1024,
          10); /* At 48khz, the latency between issuing an fmod command and hearing it will now be about 213ms. */
    }
    //Dont care if this fails
  ...
  result = FMOD_System_Init(mainsystem, (int)maxsounds, FMOD_INIT_NORMAL /*|FMOD_INIT_VOL0_BECOMES_VIRTUAL*/,
                            0);  // Initialize FMOD.
  ...
  if (supportwebfiles) FMODASSERT(FMOD_System_SetStreamBufferSize(mainsystem, 64 * 1024, FMOD_TIMEUNIT_RAWBYTES));
  ...
  if (!supportwebfiles) FMODASSERT(FMOD_System_SetFileSystem(mainsystem, myopen, myclose, myread, myseek, 2048));
...
double FMODSoundAddEffect(double sound, double effect, double pos) {
  ...
  if (!(sound > 0)) {
    { FMODASSERT(FMOD_ERR_INVALID_HANDLE); }
  }

  if ((pos < 0)) {
    { FMODASSERT(FMOD_ERR_INVALID_PARAM); }
  }
  if ((pos > 9)) {
    { FMODASSERT(FMOD_ERR_INVALID_PARAM); }
  }
  if ((effect < FMOD_DSP_TYPE_MIXER)) {
    { FMODASSERT(FMOD_ERR_INVALID_PARAM); }
  }
  if ((effect > FMOD_DSP_TYPE_LOWPASS_SIMPLE)) {
    { FMODASSERT(FMOD_ERR_INVALID_PARAM); }
  }
  void *a = NULL;
...
double FMODSoundInstanciate(double sound) {
  ...
  for (int i = 0; i < 10; i++) {
    if (mi->moreeffects[i] != NULL) {
      { FMODASSERT(FMOD_Channel_AddDSP(channel, mi->moreeffects[i], 0)) }
    };
  }
...
double FMODListenerSet3dPositionEx(double number, double x, double y, double z, double fx, double fy, double fz,
...
double FMODGetLastError(void) { return (double)lasterror; }
...
double FMODInstanceGetSpeakerMix(double instance, float *frontleft, float *frontright, float *center, float *lfe,
  ...
  FMODASSERT(FMOD_Channel_GetSpeakerMix((FMOD_CHANNEL *)(DWORD)instance, frontleft, frontright, center, lfe, backleft,
...
double FMODSoundGetLength(double sound) {
  ...
  if (mi->gotlen) return (double)mi->len;
...
double FMODInstanceSetPosition(double instance, double p) {
  ...
  FMODASSERT(FMOD_Channel_SetPosition((FMOD_CHANNEL *)(DWORD)instance,
                                      (UINT)(p * FMODSoundGetLength((double)(DWORD)sound)), FMOD_TIMEUNIT_MS));
  return (double)1;
}
...
double FMODInstanceSet3dCone(double instance, double insideconeangle, double outsideconeangle, double outsidevolume) {
  ...
  FMODASSERT(FMOD_Channel_Set3DConeSettings((FMOD_CHANNEL *)(DWORD)instance, (float)insideconeangle,
...
double FMODBlockerSetStrength(double blocker, double strength) {
  ...
  if (polies > 0) {
    for (int i = 0; i < polies; i++) {
      FMODASSERT(
          FMOD_Geometry_SetPolygonAttributes((FMOD_GEOMETRY *)(DWORD)blocker, i, (float)strength, (float)strength, 1));
    }
    return (double)1;
  }
  return (double)0;
...
double FMODInstanceGetSpectrumSnapshot(double instance, double channel, double size, LPSTR Buffer) {
  ...
  FMODASSERT(
      FMOD_Channel_GetSpectrum((FMOD_CHANNEL *)(DWORD)instance, samplebuffer, (int)size, (int)channel, spectrumwindow));
  int i;
  float maxv = 0;
  for (i = 0; i < size; i++) {
...
double FMODSoundSetLoopPoints(double sound, double start, double end) {
  ...
  FMODASSERT(FMOD_Sound_SetLoopPoints((FMOD_SOUND *)(DWORD)sound, (UINT)(start * l), FMOD_TIMEUNIT_MS, (UINT)(end * l),
                                      FMOD_TIMEUNIT_MS));
  return (double)1;
}
...
double FMODInstanceSetSpeakerMix(double instance, double frontleft, double frontright, double center, double lfe,
  ...
  FMODASSERT(FMOD_Channel_SetSpeakerMix(
      (FMOD_CHANNEL *)(DWORD)instance, max(0, min(5, (float)frontleft)), max(0, min(5, (float)frontright)),
      max(0, min(5, (float)center)), max(0, min(5, (float)lfe)), max(0, min(5, (float)backleft)),
      max(0, min(5, (float)backright)), max(0, min(5, (float)sideleft)), max(0, min(5, (float)sideright))));
  return (double)1;
}
...
double FMODInstanceSetLoopPoints(double instance, double start, double end) {
  ...
  //FMODASSERT(FMOD_Sound_GetLength(sound, &l, FMOD_TIMEUNIT_MS));
  UINT l = (UINT)FMODSoundGetLength((double)(DWORD)sound);
  FMODASSERT(FMOD_Channel_SetLoopPoints((FMOD_CHANNEL *)(DWORD)instance, (UINT)(start * l), FMOD_TIMEUNIT_MS,
                                        (UINT)(end * l), FMOD_TIMEUNIT_MS));
  return (double)1;
}
...
double FMODUpdateTakeOverWhileLocked() {
  ...
  DWORD dontcare = 0;
  threadstate = 1;
  updatethread = CreateThread(NULL,            // default security attributes
...
double FMODInstanceGetNextTag(double instance) {
  ...
  int l;
  l = min(2047, strlen(tag.name) + 1);
  lstrcpynA(tagname, tag.name, l);
  tagname[l + 1] = 0;
  if (tag.data == NULL) return (double)2;
  if (tag.datatype == FMOD_TAGDATATYPE_STRING) {
    l = min(2047, strlen((char *)tag.data) + 1);
    lstrcpynA(tagdata, (char *)tag.data, l);
    tagdata[l + 1] = 0;
    return (double)1;
...
double FMODSoundSetMusicChannelVolume(double sound, double channel, double volume) {
  ...
  FMODASSERT(
      FMOD_Sound_SetMusicChannelVolume((FMOD_SOUND *)(DWORD)sound, (int)(DWORD)channel, (float)max(0, min(volume, 1))));
  return (double)1;
}
...
double FMODEffectSetSpeakerActive(double effect, double speaker, double active) {
  ...
  //FMOD_SPEAKER  speaker,
  //FMOD_BOOL active = 0;
  FMODASSERT(FMOD_DSP_SetSpeakerActive((FMOD_DSP *)(DWORD)effect, (FMOD_SPEAKER)(int)(DWORD)speaker,
                                       (FMOD_BOOL)(int)(DWORD)active));
  return (double)1;
}
...
double FMODCreateSoundFromMicInput() {
  ...
  FMODASSERT(FMOD_System_CreateSound(mainsystem, 0, FMOD_2D | FMOD_SOFTWARE | FMOD_LOOP_NORMAL | FMOD_OPENUSER, &exinfo,
                                     &sound));
  if (sound == NULL) {
    { FMODASSERT(FMOD_ERR_INVALID_HANDLE); }
  }
