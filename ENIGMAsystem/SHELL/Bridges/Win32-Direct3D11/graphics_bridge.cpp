...
void initialize_render_targets(UINT samples=1) {
  ...
  if (FAILED(result)) {
    DEBUG_MESSAGE("Failed to obtain pointer to Direct3D11 back buffer.", MESSAGE_TYPE::FATAL_ERROR);
  }

  // Create the render target view with the back buffer pointer.
  result = m_device->CreateRenderTargetView(backBufferPtr, NULL, &m_renderTargetView);
  if (FAILED(result)) {
    DEBUG_MESSAGE("Failed to create Direct3D11 render target view.", MESSAGE_TYPE::FATAL_ERROR);
  }
  ...
  if (FAILED(result)) {
    DEBUG_MESSAGE("Failed to create Direct3D11 Texture2D for depth stencil buffer.", MESSAGE_TYPE::FATAL_ERROR);
  }
  ...
  if (FAILED(result)) {
    DEBUG_MESSAGE("Failed to create Direct3D11 depth stencil view.", MESSAGE_TYPE::FATAL_ERROR);
  }
...
void EnableDrawing(void* handle) {
  ...
  if (FAILED(result)) {
    DEBUG_MESSAGE("Failed to create Direct3D11 device and swap chain.", MESSAGE_TYPE::FATAL_ERROR);
  }
