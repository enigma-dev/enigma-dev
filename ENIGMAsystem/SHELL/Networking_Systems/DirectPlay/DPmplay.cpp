...
bool session_migrate_host =
    false;  // when the host leaves, should the server move to the next available host or just close
...
bool mplay_init_ipx() {
  ...
  if (CoCreateInstance(CLSID_DirectPlay, NULL, CLSCTX_INPROC_SERVER, IID_IDirectPlay3A, (LPVOID *)&enigma::lpdp) !=
      S_OK) {
...
void mplay_session_end() { DWORD hr = enigma::lpdp->Close(); }
...
void mplay_session_mode(bool move) { enigma::session_migrate_host = move; }
...
void mplay_end() {
  ...
  {
    if (enigma::lpdplobby) enigma::lpdplobby->Release();
