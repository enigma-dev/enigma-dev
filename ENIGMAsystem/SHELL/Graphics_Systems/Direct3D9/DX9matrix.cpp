...
void d3d_set_perspective(bool enable) {
  ...
  if (enable) {
    D3DXMatrixPerspectiveFovLH(&matProjection, 45, -view_wview[view_current] / (double)view_hview[view_current], 32000,
                               -32000);
  } else {
...
void d3d_set_projection(gs_scalar xfrom, gs_scalar yfrom, gs_scalar zfrom, gs_scalar xto, gs_scalar yto, gs_scalar zto,
  ...
  D3DXMatrixPerspectiveFovLH(&matProj, D3DXToRadian(45), view_wview[view_current] / (double)view_hview[view_current],
                             1.0f, 32000.0f);
...
void d3d_set_projection_ortho(gs_scalar x, gs_scalar y, gs_scalar width, gs_scalar height, gs_scalar angle) {
  ...
  D3DXMatrixOrthoOffCenterLH(&matProjection, 0, (FLOAT)width, 0, (FLOAT)height,
...
void d3d_set_projection_perspective(gs_scalar x, gs_scalar y, gs_scalar width, gs_scalar height, gs_scalar angle) {
  ...
  D3DXMatrixOrthoOffCenterLH(&matView, (FLOAT)x, (FLOAT)x + width, (FLOAT)y, (FLOAT)y + height,
                             -32000.0f,  // the near view-plane
                             32000.0f);  // the far view-plane
  ...
  D3DXMatrixPerspectiveFovLH(&matProj, D3DXToRadian(60), width / height, 0.1, 32000);  // the far view-plane
...
bool d3d_transform_stack_empty() { return (trans_stack_size == 0); }
