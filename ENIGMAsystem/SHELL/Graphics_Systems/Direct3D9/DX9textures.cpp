...
int graphics_create_texture(unsigned width, unsigned height, unsigned fullwidth, unsigned fullheight, void* pxdata,
  ...
  d3dmgr->CreateTexture(fullwidth, fullheight, 1, mipmap ? D3DUSAGE_AUTOGENMIPMAP : 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED,
                        &texture, 0);
  D3DLOCKED_RECT rect;
...
void graphics_copy_texture_part(int source, int destination, int xoff, int yoff, int w, int h, int x, int y) {
  ...
  for (unsigned int i = 0; i < hi; ++i) {
    memcpy(static_cast<unsigned char*>(rect.pBits) + (dw * (i + y) + x) * 4, bitmap + xoff * 4 + sfw * (i + yoff) * 4,
           wi * 4);
  }
  textureStructs[destination]->gTexture->UnlockRect(0);
...
void texture_reset() { d3dmgr->SetTexture(0, NULL); }
