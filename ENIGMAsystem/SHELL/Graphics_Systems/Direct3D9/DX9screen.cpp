...
void clear_view(float x, float y, float w, float h, float angle, bool showcolor) {
  ...
  if (enigma::d3dMode) clearflags |= D3DCLEAR_ZBUFFER;

  d3dmgr->Clear(0, NULL, clearflags, D3DCOLOR_XRGB(__GETR(clearcolor), __GETG(clearcolor), __GETB(clearcolor)), 1.0f,
                0);
}
...
void screen_redraw() {
  ...
  if (enigma::gui_used) {
    ...
    if (enigma::d3dMode) d3dmgr->Clear(0, NULL, D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(0, 0, 0), 1.0f, 0);
  ...
  if (!view_enabled) {
    ...
    for (view_current = 0; view_current < 7; view_current++) {
      if (view_visible[(int)view_current]) {
        ...
        screen_set_viewport(view_xport[vc], view_yport[vc], (window_get_region_width_scaled() - view_xport[vc]),
                            (window_get_region_height_scaled() - view_yport[vc]));
        d3d_set_projection_ortho(view_xview[vc], view_wview[vc] + view_xview[vc], view_yview[vc],
                                 view_hview[vc] + view_yview[vc], view_angle[vc]);
        break;
      }
    }
  }
