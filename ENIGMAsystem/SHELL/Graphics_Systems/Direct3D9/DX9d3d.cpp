...
D3DCULL cullingstates[3] = {D3DCULL_NONE, D3DCULL_CCW, D3DCULL_CW};

D3DFILLMODE fillmodes[3] = {D3DFILL_POINT, D3DFILL_WIREFRAME, D3DFILL_SOLID};

D3DCMPFUNC depthoperators[8] = {D3DCMP_NEVER,   D3DCMP_LESS,     D3DCMP_EQUAL,        D3DCMP_LESSEQUAL,
                                D3DCMP_GREATER, D3DCMP_NOTEQUAL, D3DCMP_GREATEREQUAL, D3DCMP_ALWAYS};

D3DFOGMODE fogmodes[3] = {D3DFOG_EXP, D3DFOG_EXP2, D3DFOG_LINEAR};

namespace enigma_user {

void d3d_depth_clear() { d3d_depth_clear_value(1.0f); }

void d3d_depth_clear_value(float value) { d3dmgr->Clear(0, NULL, D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(0, 0, 0), value, 0); }
...
void d3d_set_software_vertex_processing(bool software) { d3dmgr->SetSoftwareVertexProcessing(software); }
...
void d3d_set_fog_color(int color) {
  d3dmgr->SetRenderState(D3DRS_FOGCOLOR, D3DCOLOR_COLORVALUE(__GETR(color), __GETG(color), __GETB(color),
                                                             1.0f));  // Highest 8 bits are not used.
}
...
void d3d_set_fog_density(double density) { d3dmgr->SetRenderState(D3DRS_FOGDENSITY, *(DWORD *)(&density)); }
...
void d3d_set_fill_mode(int fill) { d3dmgr->SetRenderState(D3DRS_FILLMODE, fillmodes[fill]); }

void d3d_set_line_width(float value) {}

void d3d_set_point_size(float value) { d3dmgr->SetRenderState(D3DRS_POINTSIZE, value); }

void d3d_set_depth_operator(int mode) { d3dmgr->SetRenderState(D3DRS_ZFUNC, depthoperators[mode]); }
 ...
 public:
  ...
  bool light_define_direction(int id, gs_scalar dx, gs_scalar dy, gs_scalar dz, int col) {
    ...
    if (light_ind.find(id) != light_ind.end()) {
      ...
      if (ms >= caps.MaxActiveLights) return false;
...
void d3d_light_specularity(int facemode, int r, int g, int b, double a) { float specular[4] = {r, g, b, a}; }
...
void d3d_stencil_end_mask() { d3dmgr->SetRenderState(D3DRS_STENCILENABLE, false); }
