 ...
 public:
  ...
  void BufferGenerate() {
    ...
    if (idata.size() > 0) {
      ...
      if (indexbuffer == NULL) {
        if (vbotype == 1) {
          d3dmgr->CreateIndexBuffer(idata.size() * sizeof(unsigned), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY,
                                    D3DFMT_INDEX32, D3DPOOL_SYSTEMMEM, &indexbuffer, NULL);
        } else {
          d3dmgr->CreateIndexBuffer(idata.size() * sizeof(unsigned), D3DUSAGE_WRITEONLY, D3DFMT_INDEX32,
    ...
    if (vertexbuffer == NULL) {
      if (vbotype == 1) {
        d3dmgr->CreateVertexBuffer(vdata.size() * sizeof(gs_scalar), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, GetFVF(),
                                   D3DPOOL_SYSTEMMEM, &vertexbuffer, NULL);
      } else {
        d3dmgr->CreateVertexBuffer(vdata.size() * sizeof(gs_scalar), D3DUSAGE_WRITEONLY, GetFVF(), D3DPOOL_MANAGED,
                                   &vertexbuffer, NULL);
      }
    }
  ...
  void Draw() {
    ...
    if (triangleIndexedCount > 0) {
      d3dmgr->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, base, base, triangleVertCount, offset, triangleIndexedCount / 3);
      offset += triangleIndexedCount;
      base += triangleVertCount / stride;
    }
    if (lineVertCount > 0) {
      d3dmgr->DrawIndexedPrimitive(D3DPT_LINELIST, base, 0, lineVertCount / stride, offset, lineIndexedCount / 2);
      offset += lineIndexedCount;
      base += lineVertCount / stride;
    }
    if (pointIndexedCount > 0) {
      d3dmgr->DrawIndexedPrimitive(D3DPT_POINTLIST, base, 0, pointCount, offset, pointIndexedCount);
    }
