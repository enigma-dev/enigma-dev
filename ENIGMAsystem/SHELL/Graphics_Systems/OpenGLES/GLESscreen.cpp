...
int screen_redraw() {
  if (!view_enabled) {
    ...
    for (enigma::instance_event_iterator = event_draw->next; enigma::instance_event_iterator != NULL;
    ...
    for (view_current = 0; view_current < 7; view_current++)
      if (view_visible[(int)view_current]) {
        ...
        glScalef(1, -1, 1);
#if ENIGMA_WS_IPHONE != 0 || ENIGMA_WS_ANDROID != 0
        glOrthof((int)view_xview[vc], (int)view_wview[vc] + (int)view_xview[vc], (int)view_yview[vc],
                 (int)view_hview[vc] + (int)view_yview[vc], 0, 1);  //OPENGLES put f back in
#else
        glOrtho((int)view_xview[vc], (int)view_wview[vc] + (int)view_xview[vc], (int)view_yview[vc],
                (int)view_hview[vc] + (int)view_yview[vc], 0, 1);  //OPENGLES put f back in
#endif
        if (background_showcolor) {
          int clearcolor = ((int)background_color) & 0xFFFFFF;
