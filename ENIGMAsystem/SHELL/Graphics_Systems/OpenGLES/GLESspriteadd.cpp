...
int sprite_add(string filename, double imgnumb, double precise, double transparent, double smooth, double preload,
  ...
  glTexImage2D(GL_TEXTURE_2D, (int)smooth,
               GL_RGBA,  //4, OPENGLES
               fullwidth, fullheight, 0, GL_RGBA, GL_UNSIGNED_BYTE, pxdata);
...
void sprites_allocate_initial(int spr_highid) {
  ...
  for (int i = 0; i < spr_highid; i++) spritestructarray[i] = NULL;
}
...
void sprite_add_subimage(int sprid, int x, int y, unsigned int w, unsigned int h, unsigned char *chunk) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, fullwidth, fullheight, 0, GL_RGBA, GL_UNSIGNED_BYTE,
               imgpxdata);  //OPENGLES changed 4 to GL_RGBA
