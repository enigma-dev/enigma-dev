...
GLenum ptypes_by_id[16] = {
    GL_POINTS, GL_POINTS, GL_LINES, GL_LINE_STRIP, GL_TRIANGLES, GL_TRIANGLE_STRIP, GL_TRIANGLE_FAN, GL_LINE_LOOP,
    GL_TRIANGLE_STRIP, GL_TRIANGLE_STRIP, GL_TRIANGLE_STRIP,

    //These are padding.
    GL_POINTS, GL_POINTS, GL_POINTS, GL_POINTS,
    GL_POINTS};  //OPENGLES replaced GL_QUADS, GL_QUAD_STRIP, GL_POLYGON, with GL_TRIANGLE_STRIP's
...
int draw_primitive_begin(int kind) {
  ...
  if (++__currentpdepth > PRIMDEPTH2) {
    show_error("Max open primitive count exceeded. Disable the limit via the buffer option, or increase buffer depth",
               0);
    return -1;
  }
...
int draw_primitive_begin_texture(int kind, unsigned tex) {
  if (enigma::bound_texture != tex) glBindTexture(GL_TEXTURE_2D, enigma::bound_texture = tex);
  GLenum dink = ptypes_by_id[kind & 15];
#if !PRIMBUFFER
  // glBegin(kind);
#else
...
int draw_vertex_color(gs_scalar x, gs_scalar y, int color, float alpha) {
  ...
  untexture();
  glColor4f((col & 0xFF) / 255.0, ((col & 0xFF00) >> 8) / 255.0, ((col & 0xFF0000) >> 16) / 255.0, alpha);
//glVertex2f(x,y); OPENGLES
