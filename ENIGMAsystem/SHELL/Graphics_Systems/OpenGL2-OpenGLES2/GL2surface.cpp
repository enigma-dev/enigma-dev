...
namespace enigma
{
  ...
  //WHERE TO PUT THIS!!!!!????
  //This is GL2 only, because I need formats and types that might not be compatible between graphics systems
...
int surface_create(int width, int height, bool depthbuffer, bool stencilbuffer, bool writeonly)
{
  ...
  */
  ...
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, textureStructs[texture]->gltex, 0);
...
int surface_create_msaa(int width, int height, int samples)
{
  ...
  */
  ...
  glBindFramebuffer(GL_FRAMEBUFFER, fbo);
  //TODO: FIX
  //glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE, textureStructs[texture]->gltex, 0);
...
void surface_add_colorbuffer(int id, int index, int internalFormat, unsigned format, unsigned type){
  ...
  //glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0+index, GL_TEXTURE_2D, textureStructs[texture]->gltex, 0);
  //glBindFramebuffer(GL_FRAMEBUFFER, enigma::bound_framebuffer);
}
...
void surface_copy_part(int destination, float x, float y, int source, int xs, int ys, int ws, int hs)
{
  ...
  glBindFramebuffer(GL_FRAMEBUFFER, ssurf.fbo);
  glReadPixels(xs,ys,ws,hs,GL_RGBA,GL_UNSIGNED_BYTE,surfbuf);
  glBindFramebuffer(GL_FRAMEBUFFER, dsurf.fbo);
