...
void d3d_model_destroy(const unsigned id)
{
  meshes[id]->Clear();
  delete meshes[id];
}

void d3d_model_copy(const unsigned id, const unsigned int source)
{
  //TODO: Write copy meshes code
}

void d3d_model_merge(const unsigned id, const unsigned int source)
{
...
bool d3d_model_exists(const unsigned id)
{
  return (id >= 0 && id < meshes.size());
}

void d3d_model_clear(const unsigned id)
{
  meshes[id]->Clear();
}

void d3d_model_save(const unsigned id, string fname)
{
  //TODO: Write save code for meshes
}

bool d3d_model_load(const unsigned id, string fname)
{
...
void d3d_model_draw(const unsigned id) // overload for no additional texture or transformation call's
{
    meshes[id]->Draw();
}

void d3d_model_draw(const unsigned id, gs_scalar x, gs_scalar y, gs_scalar z) // overload for no additional texture call's
{
    glTranslatef(x, y, z);
    meshes[id]->Draw();
    glTranslatef(-x, -y, -z);
...
void d3d_model_draw(const unsigned id, int texId)
{
    texture_use(get_texture(texId));
    meshes[id]->Draw();
}

void d3d_model_draw(const unsigned id, gs_scalar x, gs_scalar y, gs_scalar z, int texId)
{
    texture_use(get_texture(texId));
    d3d_model_draw(id, x, y, z);
}

void d3d_model_primitive_begin(const unsigned id, int kind)
{
  meshes[id]->Begin(kind);
}

void d3d_model_primitive_end(const unsigned id)
{
  meshes[id]->End();
}

void d3d_model_open(const unsigned id, int offset)
{
  meshes[id]->Open(offset);
}

void d3d_model_close(const unsigned id)
{
  meshes[id]->Close();
}

void d3d_model_vertex(const unsigned id, gs_scalar x, gs_scalar y, gs_scalar z)
{
...
void d3d_model_index(const unsigned id, GLuint in)
{
  meshes[id]->VertexIndex(in);
}

void d3d_model_vertex_color(const unsigned id, gs_scalar x, gs_scalar y, gs_scalar z, int col, double alpha)
{
  meshes[id]->VertexVector(x, y, z);
  meshes[id]->ColorVector(col, alpha);
  meshes[id]->useColor();
...
void d3d_model_vertex_texture(const unsigned id, gs_scalar x, gs_scalar y, gs_scalar z, gs_scalar tx, gs_scalar ty)
{
  meshes[id]->VertexVector(x, y, z);
...
void d3d_model_vertex_texture_color(const unsigned id, gs_scalar x, gs_scalar y, gs_scalar z, gs_scalar tx, gs_scalar ty, int col, double alpha)
{
  meshes[id]->VertexVector(x, y, z);
  meshes[id]->TextureVector(tx, ty);
  meshes[id]->ColorVector(col, alpha);
...
void d3d_model_vertex_normal(const unsigned id, gs_scalar x, gs_scalar y, gs_scalar z, gs_scalar nx, gs_scalar ny, gs_scalar nz)
{
  meshes[id]->VertexVector(x, y, z);
...
void d3d_model_vertex_normal_color(const unsigned id, gs_scalar x, gs_scalar y, gs_scalar z, gs_scalar nx, gs_scalar ny, gs_scalar nz, int col, double alpha)
{
  meshes[id]->VertexVector(x, y, z);
  meshes[id]->NormalVector(nx, ny, nz);
  meshes[id]->ColorVector(col, alpha);
...
void d3d_model_vertex_normal_texture(const unsigned id, gs_scalar x, gs_scalar y, gs_scalar z, gs_scalar nx, gs_scalar ny, gs_scalar nz, gs_scalar tx, gs_scalar ty)
{
  meshes[id]->VertexVector(x, y, z);
  meshes[id]->NormalVector(nx, ny, nz);
  meshes[id]->TextureVector(tx, ty);
...
void d3d_model_vertex_normal_texture_color(const unsigned id, gs_scalar x, gs_scalar y, gs_scalar z, gs_scalar nx, gs_scalar ny, gs_scalar nz, gs_scalar tx, gs_scalar ty, int col, double alpha)
{
  meshes[id]->VertexVector(x, y, z);
  meshes[id]->NormalVector(nx, ny, nz);
  meshes[id]->TextureVector(tx, ty);
...
void d3d_model_block(const unsigned id, gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, gs_scalar hrep, gs_scalar vrep, bool closed)
{
...
void d3d_model_cylinder(const unsigned id, gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, gs_scalar hrep, gs_scalar vrep, bool closed, int steps)
{
  unsigned fp = meshes[id]->currentPrimitive;
  meshes[id]->SetPrimitive(meshes[id]->basicShapesPrimitive);
...
void d3d_model_cone(const unsigned id, gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, gs_scalar hrep, gs_scalar vrep, bool closed, int steps)
{
  unsigned fp = meshes[id]->currentPrimitive;
  meshes[id]->SetPrimitive(meshes[id]->basicShapesPrimitive);
...
void d3d_model_ellipsoid(const unsigned id, gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, gs_scalar hrep, gs_scalar vrep, int steps)
{
  unsigned fp = meshes[id]->currentPrimitive;
  meshes[id]->SetPrimitive(meshes[id]->basicShapesPrimitive);
...
void d3d_model_icosahedron(const unsigned id)
{
  unsigned fp = meshes[id]->currentPrimitive;
  meshes[id]->SetPrimitive(meshes[id]->basicShapesPrimitive);
  // fill icosahedron
...
void d3d_model_torus(const unsigned id, gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar hrep, gs_scalar vrep, int csteps, int tsteps, double radius, double tradius, double TWOPI)
{
  unsigned fp = meshes[id]->currentPrimitive;
  meshes[id]->SetPrimitive(meshes[id]->basicShapesPrimitive);
...
void d3d_model_wall(const unsigned id, gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, gs_scalar hrep, gs_scalar vrep)
{
...
void d3d_model_floor(const unsigned id, gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, gs_scalar hrep, gs_scalar vrep)
{
