...
GLenum renderstates[3] = {GL_NICEST, GL_FASTEST, GL_DONT_CARE};

GLenum fogmodes[3] = {GL_EXP, GL_EXP2, GL_LINEAR};

GLenum depthoperators[8] = {GL_NEVER, GL_LESS, GL_EQUAL, GL_LEQUAL, GL_GREATER, GL_NOTEQUAL, GL_GEQUAL, GL_ALWAYS};

GLenum fillmodes[3] = {GL_POINT, GL_LINE, GL_FILL};

GLenum windingstates[2] = {GL_CW, GL_CCW};

GLenum cullingstates[3] = {GL_BACK, GL_FRONT, GL_FRONT_AND_BACK};

GLenum stenciloperators[8] = {GL_KEEP, GL_ZERO, GL_REPLACE, GL_INCR, GL_INCR_WRAP, GL_DECR, GL_DECR_WRAP, GL_INVERT};
...
void d3d_end() {
  ...
  d3d_set_projection_ortho(0, 0, view_wview[view_current], view_hview[view_current],
                           0);  //This should probably be changed not to use views
}
...
void d3d_set_depth(double dep) {}  //TODO: Write function

void d3d_set_shading(bool smooth) {}

void d3d_set_clip_plane(bool enable) { (enable ? glEnable : glDisable)(GL_CLIP_DISTANCE0); }
...
class d3d_lights {
 ...
 public:
  ...
  void lights_enable(bool enable) { lights_enabled = enable; }

  void light_update() {
    enigma_user::glsl_uniformi(enigma::shaderprograms[enigma::bound_shader]->uni_lightEnable, lights_enabled);
    if (lights_enabled == true) {
      enigma_user::glsl_uniform4fv(enigma::shaderprograms[enigma::bound_shader]->uni_ambient_color, 1,
                                   global_ambient_color);
      enigma_user::glsl_uniform4fv(enigma::shaderprograms[enigma::bound_shader]->uni_material_ambient, 1,
                                   material.ambient);
      enigma_user::glsl_uniform4fv(enigma::shaderprograms[enigma::bound_shader]->uni_material_diffuse, 1,
                                   material.diffuse);
      enigma_user::glsl_uniform4fv(enigma::shaderprograms[enigma::bound_shader]->uni_material_specular, 1,
                                   material.specular);
      enigma_user::glsl_uniformf(enigma::shaderprograms[enigma::bound_shader]->uni_material_shininess,
                                 material.shininess);
    }
  }
      ...
      for (unsigned int i = 0; i < lights.size(); ++i) {
        if (lights[i].enabled == true) {
          if (lights[i].update == true) {
            enigma_user::glsl_uniform4fv(enigma::shaderprograms[enigma::bound_shader]->uni_light_ambient[al], 1,
                                         lights[i].ambient);
            enigma_user::glsl_uniform4fv(enigma::shaderprograms[enigma::bound_shader]->uni_light_diffuse[al], 1,
                                         lights[i].diffuse);
            enigma_user::glsl_uniform4fv(enigma::shaderprograms[enigma::bound_shader]->uni_light_specular[al], 1,
                                         lights[i].specular);
            if (lights[i].position[3] != 0.0) {  //Light is a point light
              enigma_user::glsl_uniformf(enigma::shaderprograms[enigma::bound_shader]->uni_light_cAttenuation[al],
                                         lights[i].constant_attenuation);
              enigma_user::glsl_uniformf(enigma::shaderprograms[enigma::bound_shader]->uni_light_lAttenuation[al],
                                         lights[i].linear_attenuation);
              enigma_user::glsl_uniformf(enigma::shaderprograms[enigma::bound_shader]->uni_light_qAttenuation[al],
                                         lights[i].quadratic_attenuation);
            }
            enigma_user::glsl_uniform4fv(enigma::shaderprograms[enigma::bound_shader]->uni_light_position[al], 1,
                                         lights[i].transformed_position);
  ...
  void light_update_positions() {
    ...
    for (unsigned int i = 0; i < lights.size(); ++i) {
      if (lights[i].type == 0) {  //Directional light
        ...
        lpos_eyespace = enigma::normal_matrix *
                        enigma::Vector3(lights[i].position[0], lights[i].position[1], lights[i].position[2]);
        lights[i].transformed_position[0] = lpos_eyespace.x, lights[i].transformed_position[1] = lpos_eyespace.y,
        lights[i].transformed_position[2] = lpos_eyespace.z, lights[i].transformed_position[3] = lights[i].position[3];
      } else {  //Point lights
        enigma::Vector4 lpos_eyespace;
        lpos_eyespace = enigma::mv_matrix *
                        enigma::Vector4(lights[i].position[0], lights[i].position[1], lights[i].position[2], 1.0);
        lights[i].transformed_position[0] = lpos_eyespace.x, lights[i].transformed_position[1] = lpos_eyespace.y,
        lights[i].transformed_position[2] = lpos_eyespace.z, lights[i].transformed_position[3] = lights[i].position[3];
      }
      if (lights[i].enabled == true) {
...
bool d3d_light_set_specularity(int id, int r, int g, int b, double a) {
  return enigma::d3d_lighting.light_set_specularity(id, (gs_scalar)r / 255.0, (gs_scalar)g / 255.0,
                                                    (gs_scalar)b / 255.0, a);
}

bool d3d_light_set_ambient(int id, int r, int g, int b, double a) {
  return enigma::d3d_lighting.light_set_ambient(id, (gs_scalar)r / 255.0, (gs_scalar)g / 255.0, (gs_scalar)b / 255.0,
                                                a);
}
...
namespace enigma {
void d3d_light_update_positions() { enigma::d3d_lighting.light_update_positions(); }
}  // namespace enigma
