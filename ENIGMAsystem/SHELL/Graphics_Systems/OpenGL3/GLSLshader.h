...
struct Shader {
  ...
  GLuint shader;
  int type;
  Shader(int type) : type(type) { shader = glCreateShader(shadertypes[type]); }
...
struct Attribute {
  ...
  int offset = -1;
  int datatype =
      0;  //Difference is that "type" is the type inside the shader (like float for color), but "datatype" is the type for data (unsigned int for color)
  int datasize = -1;
  int vao = -1;
};
