...
#else
#define get_surface(surf, id) enigma::surface &surf = enigma::surface_array[id];
#define get_surfacev(surf, id, r) enigma::surface &surf = enigma::surface_array[id];
...
bool surface_is_supported() { return GLEW_ARB_framebuffer_object; }

int surface_create(int width, int height, bool depthbuffer, bool stencilbuffer, bool writeonly) {
  if (GLEW_ARB_framebuffer_object) {
    GLuint fbo;

    size_t id = enigma::surface_max, w = (int)width,
    ...
    if (writeonly == true) {  //These use RenderBuffers which are faster, but cannot be sampled in GLSL
      if (depthbuffer == true && stencilbuffer == true) {
        ...
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
                                  enigma::surface_array[id].depth_buffer);
        ...
        if (depthbuffer == true) {
          ...
          enigma::surface_array[id].has_depth_buffer = true;
            ...
            stencilbuffer ==
            true) {  //NOTE(harijs) : Maybe we need to combine the stencil buffer with the depth here, because stencil alone might not be supported
          ...
          glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
                                    enigma::surface_array[id].stencil_buffer);
      ...
      if (stencilbuffer ==
          true) {  //If you use stencilbuffer you MUST also use depth buffer, as a free standing stencil buffer is seldom supported in FBO (GL spec even encourages against it)
        enigma::surface_array[id].depth_buffer = enigma::graphics_create_texture_custom(
            w, h, w, h, 0, false, GL_DEPTH24_STENCIL8, GL_DEPTH_STENCIL, GL_UNSIGNED_INT_24_8);
        glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D,
                               textureStructs[enigma::surface_array[id].depth_buffer]->gltex, 0);
        flags = flags | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT;
        enigma::surface_array[id].has_depth_buffer = true;
      } else if (depthbuffer == true) {
        enigma::surface_array[id].depth_buffer = enigma::graphics_create_texture_custom(
            w, h, w, h, 0, false, GL_DEPTH_COMPONENT24, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT);
...
int surface_create_msaa(int width, int height, int samples) {
  ...
  glBindFramebuffer(GL_DRAW_FRAMEBUFFER, fbo);
  glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE,
                         textureStructs[texture]->gltex, 0);
...
void surface_add_colorbuffer(int id, int index, int internalFormat, unsigned format, unsigned type) {
  ...
  int texture = enigma::graphics_create_texture_custom(surf.width, surf.height, surf.width, surf.height, 0, false,
...
int surface_get_target() { return enigma::bound_framebuffer; }
...
void surface_free(int id) {
  ...
  if (surf.write_only == true) {
    if (surf.has_depth_buffer == true) {
      glDeleteRenderbuffers(1, &surf.depth_buffer);
    }
    if (surf.has_stencil_buffer == true) {
      glDeleteRenderbuffers(1, &surf.stencil_buffer);
    }
...
int sprite_create_from_surface(int id, int x, int y, int w, int h, bool removeback, bool smooth, bool preload,
  ...
  glReadPixels(x, y, w, h, GL_BGRA, GL_UNSIGNED_BYTE, surfbuf);
  glBindFramebuffer(GL_READ_FRAMEBUFFER, enigma::bound_framebuffer);
  enigma::sprite_set_subimage(sprid, 0, w, h, surfbuf, surfbuf,
                              enigma::ct_precise);  //TODO: Support toggling of precise.
  delete[] surfbuf;
  return sprid;
}
