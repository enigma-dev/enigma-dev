...
#define bind_array_buffer(vbo) \
  if (enigma::bound_vbo != vbo) glBindBuffer(GL_ARRAY_BUFFER, enigma::bound_vbo = vbo);
#define bind_element_buffer(vboi) \
  if (enigma::bound_vboi != vboi) glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, enigma::bound_vboi = vboi);
...
class Mesh {
 public:
  ...
  unsigned
      stride;  // Stride computed using GetStride(). After adding custom attributes that involves loops it is better to precalculate
  ...
  Mesh(int type) {
    ...
    switch (type) {
      case enigma_user::model_static:
        vbotype = GL_STATIC_DRAW;
        break;
      case enigma_user::model_dynamic:
        vbotype = GL_DYNAMIC_DRAW;
        break;
      case enigma_user::model_stream:
  ...
  unsigned GetStride() {
    if (format == -1) {
      ...
      for (auto& att : vertexFormats[format]->flags) {
        switch (att.first) {
          case enigma_user::vertex_type_float1: {
            stride += 1;
            break;
          }
          case enigma_user::vertex_type_float2: {
            stride += 2;
            break;
          }
          case enigma_user::vertex_type_float3: {
            stride += 3;
            break;
          }
          case enigma_user::vertex_type_float4: {
            stride += 4;
            break;
          }
          case enigma_user::vertex_type_ubyte4: {
            stride += 1;
            break;
          }
        }
  ...
  void AddFloat(gs_scalar val) { vertices.push_back(val); }
  ...
  void SetFormat(int fmt) { format = (fmt < -1 ? -1 : fmt); }
    ...
    switch (currentPrimitive) {
      case enigma_user::pr_pointlist:
        if (indices.size() > 0) {
          ...
          for (std::vector<GLuint>::iterator it = indices.begin(); it != indices.end(); ++it) {
...
#define OFFSET(P) ((const GLvoid*)(sizeof(gs_scalar) * (P)))
    ...
    if (STRIDE != oglmgr->last_stride) {
      ringBufferVHead = 0, ringBufferVDrawOffset = 0, ringBufferVDrawOffsetDouble = 0;
    }
    /*unsigned offset = 0;
      bool genVAO = true;
    ...
    vdata.reserve(triangleVertices.size() + lineVertices.size() + pointVertices.size() +
                  triangleIndexedVertices.size() + lineIndexedVertices.size() + pointIndexedVertices.size());
    ...
    if (lineIndexedCount > 0) {
      ...
      for (std::vector<GLuint>::iterator it = lineIndices.begin(); it != lineIndices.end(); ++it) {
        *it += interleave;
      }
    ...
    if (idata.size() > 0) {
      ...
      if (vbotype == GL_STREAM_DRAW) {
        ...
        if ((ringBufferIHead + isize) >= ringBufferISize) {
          ringBufferIHead = 0;
    ...
    if (vbotype == GL_STREAM_DRAW) {
      ...
      if ((ringBufferVHead + vsize) >= ringBufferVSize) {
        ringBufferVHead = 0, ringBufferVDrawOffset = 0, ringBufferVDrawOffsetDouble = 0;
      }  //Loop around the buffer
  ...
  void Draw(int vertex_start = 0, int vertex_count = -1) {
    ...
    if (triangleIndexedCount == 0 && lineIndexedCount == 0 && pointIndexedCount == 0 && triangleCount == 0 &&
    ...
    enigma::glsl_uniform_matrix4fv_internal(enigma::shaderprograms[enigma::bound_shader]->uni_viewMatrix, 1,
                                            enigma::view_matrix);
    enigma::glsl_uniform_matrix4fv_internal(enigma::shaderprograms[enigma::bound_shader]->uni_projectionMatrix, 1,
                                            enigma::projection_matrix);
    enigma::glsl_uniform_matrix4fv_internal(enigma::shaderprograms[enigma::bound_shader]->uni_modelMatrix, 1,
                                            enigma::model_matrix);
    enigma::glsl_uniform_matrix4fv_internal(enigma::shaderprograms[enigma::bound_shader]->uni_mvMatrix, 1,
                                            enigma::mv_matrix);
    enigma::glsl_uniform_matrix4fv_internal(enigma::shaderprograms[enigma::bound_shader]->uni_mvpMatrix, 1,
                                            enigma::mvp_matrix);
    enigma::glsl_uniform_matrix3fv_internal(enigma::shaderprograms[enigma::bound_shader]->uni_normalMatrix, 1,
                                            enigma::normal_matrix);
...
#define OFFSET(P) ((const GLvoid*)(sizeof(gs_scalar) * (P)))
    ...
    if (format != -1) {
      ...
      for (auto& att : vertexFormats[format]->flags) {
        ...
        switch (att.first) {
          case enigma_user::vertex_type_float1: {
            type = GL_FLOAT, num = 1, off = 1;
            break;
          }
          case enigma_user::vertex_type_float2: {
            type = GL_FLOAT, num = 2, off = 2;
            break;
          }
          case enigma_user::vertex_type_float3: {
            type = GL_FLOAT, num = 3, off = 3;
            break;
          }
          case enigma_user::vertex_type_float4: {
            type = GL_FLOAT, num = 4, off = 4;
            break;
          }
          case enigma_user::vertex_type_ubyte4: {
            type = GL_UNSIGNED_BYTE, num = 4, norm = false, off = 1;
            break;
          }
        }
      ...
      enigma::glsl_attribute_set_internal(enigma::shaderprograms[enigma::bound_shader]->att_vertex, vertexStride,
      ...
      if (useNormals) {
        ...
        enigma::glsl_attribute_set_internal(enigma::shaderprograms[enigma::bound_shader]->att_normal, 3, GL_FLOAT,
                                            false, STRIDE, offset);
        offset += 3;
      } else {
      ...
      if (useTextures) {
        ...
        enigma::glsl_attribute_set_internal(enigma::shaderprograms[enigma::bound_shader]->att_texture, 2, GL_FLOAT,
                                            false, STRIDE, offset);
        offset += 2;
      } else {
      ...
      if (useColors) {
        ...
        enigma::glsl_attribute_set_internal(enigma::shaderprograms[enigma::bound_shader]->att_color, 4,
                                            GL_UNSIGNED_BYTE, true, STRIDE, offset);
      } else {
        enigma::glsl_attribute_enable_internal(enigma::shaderprograms[enigma::bound_shader]->att_color, false);
      }
    }

    enigma::glsl_uniformf_internal(enigma::shaderprograms[enigma::bound_shader]->uni_color,
                                   (float)enigma::currentcolor[0] / 255.0f, (float)enigma::currentcolor[1] / 255.0f,
                                   (float)enigma::currentcolor[2] / 255.0f, (float)enigma::currentcolor[3] / 255.0f);
...
#define OFFSETE(P) ((const GLvoid*)(sizeof(GLuint) * (P)))
    ...
    if (triangleIndexedCount > 0) {
      ...
      glDrawElementsBaseVertex(GL_TRIANGLES, (vertex_count == -1 ? triangleIndexedCount : vertex_count),
                               GL_UNSIGNED_INT, OFFSETE(offset), ringBufferVDrawOffset);
...
#endif
      glDrawArrays(GL_TRIANGLES, (vertex_start == 0 ? offset : vertex_start),
                   (vertex_count == -1 ? triangleCount : vertex_count));
