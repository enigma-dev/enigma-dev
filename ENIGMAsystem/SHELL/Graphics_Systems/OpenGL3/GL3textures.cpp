...
#else
#define get_texture(tex, texid, v) const int tex = (texid == -1 ? -1 : textureStructs[texid]->gltex);
#endif
...
TextureStruct::~TextureStruct() { glDeleteTextures(1, &gltex); }

namespace enigma {
extern int bound_texture_stage;

//This allows GL3 surfaces to bind and hold many different types of data
int graphics_create_texture_custom(unsigned width, unsigned height, unsigned fullwidth, unsigned fullheight,
                                   void* pxdata, bool mipmap, int internalFormat, unsigned format, unsigned type) {
  GLuint texture;
...
void graphics_copy_texture(int source, int destination, int x, int y) {
  ...
  glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, (x + sw <= dw ? sw : dw - x), (y + sh <= dh ? sh : dh - y), GL_BGRA,
                  GL_UNSIGNED_BYTE, cropped_bitmap);
...
void graphics_replace_texture_alpha_from_texture(int tex, int copy_tex) {
  ...
  for (unsigned int i = 3; i < size; i += 4) bitmap[i] = (bitmap2[i - 3] + bitmap2[i - 2] + bitmap2[i - 1]) / 3;
...
void graphics_delete_texture(int texid) { delete textureStructs[texid]; }
...
struct SamplerState {
  ...
  ~SamplerState() { glDeleteSamplers(1, &sampler_index); }
};
...
void texture_set_enabled(bool enable) { (enable ? glEnable : glDisable)(GL_TEXTURE_2D); }

void texture_set_blending(bool enable) { (enable ? glEnable : glDisable)(GL_BLEND); }
...
gs_scalar texture_get_texel_width(int texid) { return 1.0 / textureStructs[texid]->fullwidth; }

gs_scalar texture_get_texel_height(int texid) { return 1.0 / textureStructs[texid]->fullheight; }
...
void texture_set_stage(int stage, int texid) {
  ...
  if (enigma::samplerstates[stage].bound_texture != gt) {
    ...
    if ((unsigned int)enigma::bound_texture_stage != GL_TEXTURE0 + stage) {
      glActiveTexture(enigma::bound_texture_stage = (GL_TEXTURE0 + stage));
    }
...
void texture_reset() {
  if (enigma::samplerstates[0].bound_texture != 0) {
    oglmgr->EndShapesBatching();
    if (enigma::bound_texture_stage != GL_TEXTURE0) {
      glActiveTexture(enigma::bound_texture_stage = GL_TEXTURE0);
    }
...
void texture_set_interpolation_ext(int sampler, bool enable) {
  ...
  glSamplerParameteri(enigma::samplerstates[sampler].sampler_index, GL_TEXTURE_MIN_FILTER,
                      enable ? GL_LINEAR : GL_NEAREST);
  glSamplerParameteri(enigma::samplerstates[sampler].sampler_index, GL_TEXTURE_MAG_FILTER,
                      enable ? GL_LINEAR : GL_NEAREST);
}

void texture_set_repeat_ext(int sampler, bool repeat) {
  //TODO(harijs): Check if sampler <0
  glSamplerParameteri(enigma::samplerstates[sampler].sampler_index, GL_TEXTURE_WRAP_R,
                      repeat ? GL_REPEAT : GL_CLAMP_TO_EDGE);
  glSamplerParameteri(enigma::samplerstates[sampler].sampler_index, GL_TEXTURE_WRAP_S,
                      repeat ? GL_REPEAT : GL_CLAMP_TO_EDGE);
  glSamplerParameteri(enigma::samplerstates[sampler].sampler_index, GL_TEXTURE_WRAP_T,
                      repeat ? GL_REPEAT : GL_CLAMP_TO_EDGE);
}
...
void texture_set_wrap_ext(int sampler, bool wrapu, bool wrapv, bool wrapw) {
  //TODO(harijs): Check if sampler <0
  glSamplerParameteri(enigma::samplerstates[sampler].sampler_index, GL_TEXTURE_WRAP_R,
                      wrapu ? GL_REPEAT : GL_CLAMP_TO_EDGE);
  glSamplerParameteri(enigma::samplerstates[sampler].sampler_index, GL_TEXTURE_WRAP_S,
                      wrapv ? GL_REPEAT : GL_CLAMP_TO_EDGE);
  glSamplerParameteri(enigma::samplerstates[sampler].sampler_index, GL_TEXTURE_WRAP_T,
                      wrapw ? GL_REPEAT : GL_CLAMP_TO_EDGE);
}
...
bool texture_mipmapping_supported() { return enigma::gl_extension_supported("glGenerateMipmap"); }

bool texture_anisotropy_supported() { return enigma::gl_extension_supported("GL_EXT_texture_filter_anisotropic"); }
