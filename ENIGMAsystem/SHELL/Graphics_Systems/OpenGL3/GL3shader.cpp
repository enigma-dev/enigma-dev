...
#define get_uniform(uniter, location, usize)                                                                        \
  ...
  if (location < 0) {                                                                                               \
    printf("%s - Uniform location < 0 given (%i)!\n", str, location);                                               \
    return;                                                                                                         \
  }                                                                                                                 \
  std::unordered_map<GLint, enigma::Uniform>::iterator uniter =                                                     \
      enigma::shaderprograms[enigma::bound_shader]->uniforms.find(location);                                        \
  if (uniter == enigma::shaderprograms[enigma::bound_shader]->uniforms.end()) {                                     \
    ...
    printf("%s - Uniform [%s] at location %i with %i arguments is accesed by a function with %i arguments!\n", str, \
           uniter->second.name.c_str(), location, uniter->second.size, usize);                                      \
  }
...
#define get_attribute(atiter, location)                                           \
  ...
  if (location < 0) {                                                             \
    printf("%s - Attribute location < 0 given (%i)!\n", str, location);           \
    return;                                                                       \
  }                                                                               \
  std::unordered_map<GLint, enigma::Attribute>::iterator atiter =                 \
      enigma::shaderprograms[enigma::bound_shader]->attributes.find(location);    \
...
#define get_program(ptiter, program, err)                                                     \
  if (program < 0) {                                                                          \
    printf("Program id [%i] < 0 given!\n", program);                                          \
    return err;                                                                               \
  }                                                                                           \
  if (program >= enigma::shaderprograms.size()) {                                             \
    printf("Program id [%i] > size() [%i] given!\n", program, enigma::shaderprograms.size()); \
    return err;                                                                               \
  }                                                                                           \
  if (enigma::shaderprograms[program] == nullptr) {                                           \
    printf("Program with id [%i] is deleted!\n", program);                                    \
    return err;                                                                               \
  }                                                                                           \
...
extern GLenum shadertypes[5] = {GL_VERTEX_SHADER, GL_FRAGMENT_SHADER, GL_TESS_CONTROL_SHADER, GL_TESS_EVALUATION_SHADER,
                                GL_GEOMETRY_SHADER};
...
void getUniforms(int prog_id) {
  ...
  for (int i = 0; i < uniform_count; ++i) {
    ...
    glGetActiveUniform(enigma::shaderprograms[prog_id]->shaderprogram, GLuint(i), max_length, NULL, &uniform.arraySize,
...
void getAttributes(int prog_id) {
  ...
  for (int i = 0; i < attribute_count; ++i) {
    ...
    glGetActiveAttrib(enigma::shaderprograms[prog_id]->shaderprogram, GLuint(i), max_length, NULL, &attribute.arraySize,
    ...
    if (attribute.arraySize > 1) {  //It's an array
      ...
      for (int c = 1; c < attribute.arraySize; ++c) {
        ...
        sattribute.location =
            glGetAttribLocation(enigma::shaderprograms[prog_id]->shaderprogram, sattribute.name.c_str());
...
void getDefaultUniforms(int prog_id) {
  ...
  shaderprograms[prog_id]->uni_projectionMatrix =
      enigma_user::glsl_get_uniform_location(prog_id, "transform_matrix[1]");
  ...
  shaderprograms[prog_id]->uni_material_shininess =
      enigma_user::glsl_get_uniform_location(prog_id, "Material.Shininess");
}
...
int getGLTypeSize(GLuint type) {
  switch (type) {
    ...
    default: {
      printf("getGLTypeSize Asking size for unknown type - %i!\n", type);
      return 1;
    }
  }
}
...
bool UATypeUIComp(UAType i, unsigned int j) { return (i.ui == j); }
bool UATypeIComp(UAType i, int j) { return (i.i == j); }
bool UATypeFComp(UAType i, float j) { return (i.f == j); }
}  // namespace enigma
...
bool glsl_shader_compile(int id) {
  ...
  if (compiled) {
    ...
    std::cout << "Shader[" << id << "] " << (enigma::shaders[id]->type == sh_vertex ? "Vertex shader" : "Pixel shader")
              << " - Compilation failed - Info log: " << std::endl;
...
string glsl_shader_get_infolog(int id) { return enigma::shaders[id]->log; }

string glsl_program_get_infolog(int id) { return enigma::shaderprograms[id]->log; }

void glsl_shader_free(int id) { delete enigma::shaders[id]; }
...
int glsl_program_get() { return enigma::bound_shader; }
...
void glsl_program_default_set(int id) { enigma::main_shader = id; }

void glsl_program_default_reset() { enigma::main_shader = enigma::default_shader; }
...
void glsl_program_set_name(int id, string name) { enigma::shaderprograms[id]->name = name; }
...
int glsl_get_uniform_location(int program, string name) {
  ...
  if (it == enigma::shaderprograms[program]->uniform_names.end()) {
    ...
    if (enigma::shaderprograms[program]->name == "") {
      ...
      printf("Program[%s = %i] - Uniform %s not found!\n", enigma::shaderprograms[program]->name.c_str(), program,
             name.c_str());
    }
#endif
    return -1;
