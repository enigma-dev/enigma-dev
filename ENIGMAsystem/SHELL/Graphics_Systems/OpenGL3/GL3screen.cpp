...
unsigned int bound_framebuffer =
    0;  //Shows the bound framebuffer, so glGetIntegerv(GL_FRAMEBUFFER_BINDING_EXT, &fbo); don't need to be called (they are very slow)
      ...
      if (enigma_user::background_exists(background_index[back_current])) {
        ...
        if (background_htiled[back_current] || background_vtiled[back_current]) {
          draw_background_tiled_ext(
              background_index[back_current], background_x[back_current], background_y[back_current],
              background_xscale[back_current], background_xscale[back_current], background_coloring[back_current],
              background_alpha[back_current], background_htiled[back_current], background_vtiled[back_current]);
        } else {
          draw_background_ext(background_index[back_current], background_x[back_current], background_y[back_current],
                              background_xscale[back_current], background_xscale[back_current], 0,
                              background_coloring[back_current], background_alpha[back_current]);
        }
      }
    }
  }
...
static inline void draw_insts() {
  ...
  for (map<int, pair<double, double> >::iterator it = id_to_currentnextdepth.begin();
       it != id_to_currentnextdepth.end(); it++) {
  ...
  if (enigma::particles_impl != NULL) {
    ...
    const double low = drawing_depths.rbegin() != drawing_depths.rend() ? drawing_depths.rbegin()->first
...
static inline int draw_tiles() {
  for (enigma::diter dit = drawing_depths.rbegin(); dit != drawing_depths.rend(); dit++) {
    ...
    //loop instances
    for (enigma::instance_event_iterator = dit->second.draw_events->next; enigma::instance_event_iterator != NULL;
      ...
      if (inst->myevent_draw_subcheck()) inst->myevent_draw();
      if (enigma::room_switching_id != -1) return 1;
    }
...
void clear_view(float x, float y, float w, float h, float angle, bool showcolor) {
  ...
  if (enigma::d3dMode) clear_bits |= GL_DEPTH_BUFFER_BIT;
...
static inline void draw_gui() {
  ...
  for (enigma::diter dit = drawing_depths.rbegin(); dit != drawing_depths.rend(); dit++) {
    ...
    for (enigma::instance_event_iterator = dit->second.draw_events->next; enigma::instance_event_iterator != NULL;
         enigma::instance_event_iterator = enigma::instance_event_iterator->next) {
      enigma::object_graphics* inst = ((object_graphics*)enigma::instance_event_iterator->inst);
      if (inst->myevent_drawgui_subcheck()) inst->myevent_drawgui();
...
void screen_redraw() {
  ...
  if (!view_enabled) {
    if (bound_framebuffer !=
        0)  //This fixes off-by-one error when rendering on surfaces. This should be checked to see if other GPU's have the same effect
    ...
    for (view_current = 0; view_current < 8; view_current++) {
      ...
      if (!view_visible[vc]) continue;

      int vob = (int)view_object[vc];
      if (vob != -1) follow_object(vob, vc);
      ...
      clear_view(view_xview[vc], view_yview[vc], view_wview[vc], view_hview[vc], view_angle[vc],
                 background_showcolor && draw_backs);

      if (draw_backs) draw_back();
  ...
  if (enigma::gui_used) {
    ...
    if (enigma::d3dMode) glClear(GL_DEPTH_BUFFER_BIT);
  ...
  if (enigma::msaa_fbo != 0) {
    ...
    glBlitFramebuffer(0, 0, window_get_region_width_scaled(), window_get_region_height_scaled(), 0, 0,
                      window_get_region_width_scaled(), window_get_region_height_scaled(), GL_COLOR_BUFFER_BIT,
                      GL_NEAREST);
  ...
  if (bound_framebuffer == 0 || enigma::msaa_fbo != 0) {
    screen_refresh();
  }
}
...
int screen_save_part(string filename, unsigned x, unsigned y, unsigned w,
                     unsigned h)  //Assumes native integers are little endian
{
  oglmgr->EndShapesBatching();
  unsigned sz = w * h;
...
unsigned int display_get_gui_width() { return enigma::gui_width; }

unsigned int display_get_gui_height() { return enigma::gui_height; }
