...
bool surface_is_supported() { return GLEW_EXT_framebuffer_object; }
...
int surface_create(int width, int height, bool depthbuffer) {
  ...
  glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, textureStructs[texture]->gltex,
                         0);
...
int surface_create_msaa(int width, int height, int samples) {
  ...
  glGetIntegerv(GL_FRAMEBUFFER_BINDING_EXT, &prevFbo);
  glBindFramebuffer(GL_DRAW_FRAMEBUFFER, fbo);
  glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D_MULTISAMPLE,
...
bool surface_exists(int id) { return enigma::surface_array.find(id) != enigma::surface_array.end(); }
