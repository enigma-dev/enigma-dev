...
class Mesh {
 public:
  ...
  vector<VertexElement>
      triangleVertices;  // The vertices added to triangle primitives batched into a single triangle list to be sent to the GPU
  vector<VertexElement>
      triangleIndexedVertices;  // The vertices added to indexed triangle primitives batched into a single triangle list to be sent to the GPU
  vector<GLuint>
      triangleIndices;  // The triangle indices either concatenated by batching or supplied in the temporary container.
  vector<VertexElement>
      lineVertices;  // The vertices added to line primitives batched into a single line list to be sent to the GPU
  vector<VertexElement>
      lineIndexedVertices;  // The vertices added to indexed line primitives batched into a single line list to be sent to the GPU
  vector<GLuint>
      lineIndices;  // The line indices either concatenated by batching or supplied in the temporary container.
  vector<VertexElement>
      pointVertices;  // The vertices added to point primitives batched into a single point list to be sent to the GPU
  vector<VertexElement>
      pointIndexedVertices;  // The vertices added to indexed point primitives batched into a single point list to be sent to the GPU
  vector<GLuint>
  ...
  ~Mesh() { glDeleteLists(modellist, 1); }
  ...
  void Begin(int pt) { currentPrimitive = pt; }
...
#define OFFSET(P) (char*)(&verts[0]) + ((sizeof(gs_scalar) * (P)))
  ...
  void DrawCalls(int vertex_start = 0, int vertex_count = -1) {
    ...
    if (triangleIndexedCount > 0) {
      DrawElements(GL_TRIANGLES, triangleIndexedVertices, triangleIndices,
                   (vertex_count == -1 ? triangleIndices.size() : vertex_count), vertex_start);
    }
