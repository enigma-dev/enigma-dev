...
GLenum ptypes_by_id[16] = {GL_POINTS, GL_POINTS, GL_LINES, GL_LINE_STRIP, GL_TRIANGLES, GL_TRIANGLE_STRIP,
                           GL_TRIANGLE_FAN, GL_POINTS, GL_POINTS, GL_POINTS, GL_POINTS,
...
void draw_primitive_end() { glEnd(); }

void draw_vertex(gs_scalar x, gs_scalar y) { glVertex2f(x, y); }

void draw_vertex_color(gs_scalar x, gs_scalar y, int col, float alpha) {
  glColor4ub((col & 0xFF), ((col & 0xFF00) >> 8), ((col & 0xFF0000) >> 16), (unsigned char)(alpha * 255));
  glVertex2f(x, y);
...
void d3d_primitive_end() { glEnd(); }

void d3d_vertex(gs_scalar x, gs_scalar y, gs_scalar z) { glVertex3d(x, y, z); }
...
void d3d_vertex_texture_color(gs_scalar x, gs_scalar y, gs_scalar z, gs_scalar tx, gs_scalar ty, int color,
                              double alpha) {
  glColor4ub(GETR(color), GETG(color), GETB(color), (unsigned char)(alpha * 255));
  glTexCoord2f(tx, ty);
  glVertex3d(x, y, z);
...
void d3d_vertex_normal_color(gs_scalar x, gs_scalar y, gs_scalar z, gs_scalar nx, gs_scalar ny, gs_scalar nz, int color,
                             double alpha) {
  glColor4ub(GETR(color), GETG(color), GETB(color), (unsigned char)(alpha * 255));
  glNormal3f(nx, ny, nz);
  glVertex3d(x, y, z);
...
void d3d_vertex_normal_texture(gs_scalar x, gs_scalar y, gs_scalar z, gs_scalar nx, gs_scalar ny, gs_scalar nz,
                               gs_scalar tx, gs_scalar ty) {
  glTexCoord2f(tx, ty);
  glNormal3f(nx, ny, nz);
  glVertex3d(x, y, z);
}

void d3d_vertex_normal_texture_color(gs_scalar x, gs_scalar y, gs_scalar z, gs_scalar nx, gs_scalar ny, gs_scalar nz,
                                     gs_scalar tx, gs_scalar ty, int color, double alpha) {
  glColor4ub(GETR(color), GETG(color), GETB(color), (unsigned char)(alpha * 255));
  glTexCoord2f(tx, ty);
  glNormal3f(nx, ny, nz);
  glVertex3d(x, y, z);
...
void d3d_draw_block(gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, int texId,
                    gs_scalar hrep, gs_scalar vrep, bool closed) {
  GLfloat verts[] = {x1, y1, z1, x1, y1, z2, x1, y2, z1, x1, y2, z2, x2, y2, z1, x2, y2, z2, x2, y1, z1, x2, y1, z2},
          texts[] = {0, vrep, hrep, vrep, 0, 0, hrep, 0, 0, vrep, hrep, vrep, 0, 0, hrep, 0},
...
void d3d_draw_floor(gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, int texId,
                    gs_scalar hrep, gs_scalar vrep) {
  ...
  GLfloat verts[] = {x1, y1, z1, x1, y2, z1, x2, y1, z2, x2, y2, z2}, texts[] = {0, 0, 0, vrep, hrep, 0, hrep, vrep},
...
void d3d_draw_wall(gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, int texId,
                   gs_scalar hrep, gs_scalar vrep) {
  gs_scalar nX = (y2 - y1) * (z2 - z1) * (z2 - z1);
  gs_scalar nY = (z2 - z1) * (x2 - x1) * (x2 - x1);
  gs_scalar nZ = (x2 - x1) * (y2 - y1) * (y2 - y1);
  ...
  GLfloat verts[] = {x1, y1, z1, x2, y2, z1, x1, y1, z2, x2, y2, z2}, texts[] = {0, 0, hrep, 0, 0, vrep, hrep, vrep},
          norms[] = {nX, nY, nZ, nX, nY, nZ, nX, nY, nZ, nX, nY, nZ};
...
void d3d_draw_cylinder(gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, int texId,
                       gs_scalar hrep, gs_scalar vrep, bool closed, int steps) {
  float v[100][3];
  float n[100][3];
  float t[100][3];
  steps = min(max(steps, 3), 48);
  const double cx = (x1 + x2) / 2, cy = (y1 + y2) / 2, rx = (x2 - x1) / 2, ry = (y2 - y1) / 2,
               invstep = (1.0 / steps) * hrep, pr = 2 * M_PI / steps;
  double a, px, py, tp;
  int k;
...
void d3d_draw_cone(gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, int texId,
                   gs_scalar hrep, gs_scalar vrep, bool closed, int steps) {
  steps = min(max(steps, 3), 48);
  const double cx = (x1 + x2) / 2, cy = (y1 + y2) / 2, rx = (x2 - x1) / 2, ry = (y2 - y1) / 2,
               invstep = (1.0 / steps) * hrep, pr = 2 * M_PI / steps;
...
void d3d_draw_ellipsoid(gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, int texId,
                        gs_scalar hrep, gs_scalar vrep, int steps) {
  steps = min(max(steps, 3), 24);
  const int zsteps = ceil(steps / 2.0);
  const double cx = (x1 + x2) / 2, cy = (y1 + y2) / 2, cz = (z1 + z2) / 2, rx = (x2 - x1) / 2, ry = (y2 - y1) / 2,
               rz = (z2 - z1) / 2, invstep = (1.0 / steps) * hrep, invstep2 = (1.0 / zsteps) * vrep,
               pr = 2 * M_PI / steps, qr = M_PI / zsteps;
...
void d3d_draw_icosahedron(gs_scalar x1, gs_scalar y1, gs_scalar z1, gs_scalar x2, gs_scalar y2, gs_scalar z2, int texId,
                          gs_scalar hrep, gs_scalar vrep, int steps) {
  // gs_scalar width = (x2-x1), length = (y2-y1), height = (z2-z1);  UNUSED
  static gs_scalar vdata[12][3] = {{0, 0.5, 1}, {1, 0.5, 1}, {0, 0.5, 0}, {1, 0.5, 0}, {0.5, 1, 1}, {0.5, 1, 0},
                                   {0.5, 0, 1}, {0.5, 0, 0}, {1, 1, 0.5}, {0, 1, 0.5}, {1, 0, 0.5}, {0, 0, 0.5}};
...
void d3d_draw_torus(gs_scalar x1, gs_scalar y1, gs_scalar z1, int texId, gs_scalar hrep, gs_scalar vrep, int csteps,
                    int tsteps, double radius, double tradius) {
  texture_set(texId);
