...
void vertex_submit(int buffer, int primitive, unsigned vertex_start, unsigned vertex_count) {
  ...
  size_t stride = vertexFormat->stride * sizeof(float);
  for (size_t i = 0; i < vertexFormat->flags.size(); ++i) {
    ...
    switch (flag.second) {
      case vertex_usage_position:
        ...
        glVertexPointer(elements, type, stride, &vertexBuffer->vertices[offset]);
        break;
      case vertex_usage_color:
        useColors = true;
        glEnableClientState(GL_COLOR_ARRAY);
        glColorPointer(elements, type, stride, &vertexBuffer->vertices[offset]);
        break;
      case vertex_usage_normal:
        useNormals = true;
        glEnableClientState(GL_NORMAL_ARRAY);
        glNormalPointer(type, stride, &vertexBuffer->vertices[offset]);
        break;
      case vertex_usage_textcoord:
        useTextCoords = true;
        glClientActiveTexture(GL_TEXTURE0 + texture++);
        glEnableClientState(GL_TEXTURE_COORD_ARRAY);
        glTexCoordPointer(elements, type, stride, &vertexBuffer->vertices[offset]);
        break;
      ...
      case vertex_usage_fog:
        ...
        glFogCoordPointer(type, stride, &vertexBuffer->vertices[offset]);
        break;
