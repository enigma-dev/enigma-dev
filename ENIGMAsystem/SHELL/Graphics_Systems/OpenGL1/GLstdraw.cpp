...
void draw_set_msaa_enabled(bool enable) { (enable ? glEnable : glDisable)(GL_MULTISAMPLE); }

void draw_enable_alphablend(bool enable) { (enable ? glEnable : glDisable)(GL_BLEND); }

bool draw_get_alpha_test() { return glIsEnabled(GL_ALPHA_TEST); }
...
void draw_set_alpha_test(bool enable) { (enable ? glEnable : glDisable)(GL_ALPHA_TEST); }

void draw_set_alpha_test_ref_value(unsigned val) { glAlphaFunc(GL_GREATER, val / 256); }

void draw_set_line_pattern(int pattern, int scale) {
  if (pattern == -1)
    glDisable(GL_LINE_STIPPLE);
  else
    glEnable(GL_LINE_STIPPLE), glLineStipple(scale, (short)pattern);
}
...
int draw_getpixel(int x, int y) {
  if (view_enabled) {
    ...
    y = enigma_user::view_hview[enigma_user::view_current] - (y - enigma_user::view_yview[enigma_user::view_current]) -
        1;
    if (x < 0) x = 0;
    if (y < 0) y = 0;
    if (x > enigma_user::view_wview[enigma_user::view_current] ||
        y > enigma_user::view_hview[enigma_user::view_current])
      return 0;
  } else {
...
void CALLBACK combineCallback(GLdouble coords[3], GLdouble* vertex_data[4], GLfloat weight[4], GLdouble** dataOut) {
  ...
  if (vertex) {
    ...
    for (int i = 3; i < 6; i++) {
      vertex[i] = (weight[0] ? weight[0] * vertex_data[0][i] : 0) + (weight[1] ? weight[1] * vertex_data[1][i] : 0) +
                  (weight[2] ? weight[2] * vertex_data[2][i] : 0) + (weight[3] ? weight[3] * vertex_data[3][i] : 0);
    }
  }
