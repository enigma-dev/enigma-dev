...
void d3d_depth_clear() { d3d_depth_clear_value(1.0f); }

void d3d_depth_clear_value(float value) { glClearDepthf(value); }
...
void d3d_set_lighting(bool enable) { (enable ? glEnable : glDisable)(GL_LIGHTING); }
...
void d3d_set_fog_enabled(bool enable) { (enable ? glEnable : glDisable)(GL_FOG); }

void d3d_set_fog_mode(int mode) { glFogi(GL_FOG_MODE, fogmodes[mode]); }

void d3d_set_fog_hint(int mode) { glHint(GL_FOG_HINT, mode); }
...
void d3d_set_fog_start(double start) { glFogf(GL_FOG_START, start); }

void d3d_set_fog_end(double end) { glFogf(GL_FOG_END, end); }

void d3d_set_fog_density(double density) { glFogf(GL_FOG_DENSITY, density); }
...
void d3d_set_fill_mode(int fill) { glPolygonMode(GL_FRONT_AND_BACK, fillmodes[fill]); }

void d3d_set_line_width(float value) { glLineWidth(value); }

void d3d_set_point_size(float value) { glPointSize(value); }

void d3d_set_depth_operator(int mode) { glDepthFunc(depthoperators[mode]); }

void d3d_set_depth(double dep) {}  //TODO: Write function

void d3d_clear_depth() { glClear(GL_DEPTH_BUFFER_BIT); }

void d3d_set_shading(bool smooth) { glShadeModel(smooth ? GL_SMOOTH : GL_FLAT); }
 ...
 public:
  ...
  bool light_define_direction(int id, gs_scalar dx, gs_scalar dy, gs_scalar dz, int col) {
    ...
    if (light_ind.find(id) != light_ind.end()) {
      ...
      if (it != ind_pos.end()) ind_pos.erase(it);
      ind_pos.insert(pair<int, posi>(ms, posi(-dx, -dy, -dz, 0.0f)));
    } else {
      ms = light_ind.size();
      int MAX_LIGHTS;
      glGetIntegerv(GL_MAX_LIGHTS, &MAX_LIGHTS);
      if (ms >= MAX_LIGHTS) return false;

      light_ind.insert(pair<int, int>(id, ms));
      ind_pos.insert(pair<int, posi>(ms, posi(-dx, -dy, -dz, 0.0f)));
    }

    const float dir[4] = {float(-dx), float(-dy), float(-dz), 0.0f},
                color[4] = {float(__GETR(col)), float(__GETG(col)), float(__GETB(col)), 1.0f};
  ...
  bool light_define_point(int id, gs_scalar x, gs_scalar y, gs_scalar z, double range, int col) {
    ...
    const float pos[4] = {(float)x, (float)y, (float)z, 1.0f},
                color[4] = {float(__GETR(col)), float(__GETG(col)), float(__GETB(col)), 1.0f},
...
void d3d_light_shininess(int facemode, int shine) { glMateriali(renderstates[facemode], GL_SHININESS, shine); }
...
void d3d_stencil_end_mask() { glDisable(GL_STENCIL_TEST); }

}  // namespace enigma_user

namespace enigma {
void d3d_light_update_positions() { d3d_lighting.light_update_positions(); }
}  // namespace enigma
