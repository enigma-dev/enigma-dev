...
const string unicodeAnds =
    "\x1F\x1F\x1F\x1F\x1F\x1F\x1F\x1F\x1F\x1F\x1F\x1F\x1F\x1F\x1F\x1F\x0F\x0F\x0F\x0F\x0F\x0F\x0F\x0F\x07\x07\x07\x07"
    "\x03\x03\x01";
...
void draw_set_halign(unsigned align) { halign = align; }
void draw_set_valign(unsigned align) { valign = align; }

unsigned draw_get_halign() { return halign; }
unsigned draw_get_valign() { return valign; }
...
#define get_font_null(fnt, id, r)                                    \
  ...
  const font *const fnt = fontstructarray[id];
#else
#define get_font(fnt, id, r) const font *const fnt = fontstructarray[id];
#define get_fontv(fnt, id) const font *const fnt = fontstructarray[id];
#define get_font_null(fnt, id, r) const font *const fnt = fontstructarray[id];
#endif
...
unsigned int string_width_ext(
    variant vstr, gs_scalar sep,
    gs_scalar w)  //here sep doesn't do anything, but I can't make it 'default = ""', because its the second argument
{
  string str = toString(vstr);
  get_font(fnt, currentfont, 0);
...
unsigned int string_width_ext_line(variant vstr, gs_scalar w, int line) {
  ...
  float width = 0, tw = 0;
  int cl = 0, slen = get_space_width(fnt);
  for (size_t i = 0; i < str.length(); i++) {
    uint32_t character = getUnicodeCharacter(str, i);
    if (character == '\r') {
      if (cl == line)
...
void draw_text(gs_scalar x, gs_scalar y, variant vstr) {
  ...
  gs_scalar yy = valign == fa_top ? y + fnt->yoffset
                                  : valign == fa_middle ? y + fnt->yoffset - string_height(str) / 2
                                                        : y + fnt->yoffset - string_height(str);
  ...
  if (halign == fa_left) {
    ...
    gs_scalar xx = halign == fa_center ? x - gs_scalar(string_width_line(str, 0) / 2)
                                       : x - gs_scalar(string_width_line(str, 0)),
              line = 0;
    for (size_t i = 0; i < str.length(); i++) {
      uint32_t character = getUnicodeCharacter(str, i);
      ...
      if (character == '\r') {
        ...
        xx = halign == fa_center ? x - gs_scalar(string_width_line(str, line) / 2)
                                 : x - gs_scalar(string_width_line(str, line));
      } else if (character == '\n') {
...
void draw_text_sprite(gs_scalar x, gs_scalar y, variant vstr, int sep, int lineWidth, int sprite, int firstChar,
                      int scale) {
  string str = toString(vstr);
...
void draw_text_ext(gs_scalar x, gs_scalar y, variant vstr, gs_scalar sep, gs_scalar w) {
  ...
  gs_scalar yy = valign == fa_top ? y + fnt->yoffset
                                  : valign == fa_middle ? y + fnt->yoffset - string_height_ext(str, sep, w) / 2
                                                        : y + fnt->yoffset - string_height_ext(str, sep, w);
  float slen = get_space_width(fnt);
  if (halign == fa_left) {
    ...
    for (size_t i = 0; i < str.length(); i++) {
      ...
      if (character == '\r') {
        ...
        if (character == ' ' or g.empty()) {
          ...
          if (width + tw >= w && w != -1) xx = x, yy += (sep == -1 ? fnt->height : sep), width = 0, tw = 0;
        } else {
          draw_primitive_begin_texture(pr_trianglestrip, fnt->texture);
          draw_vertex_texture(xx + g.x, yy + g.y, g.tx, g.ty);
          draw_vertex_texture(xx + g.x2, yy + g.y, g.tx2, g.ty);
    ...
    gs_scalar xx = halign == fa_center ? x - gs_scalar(string_width_ext_line(str, w, 0) / 2)
                                       : x - gs_scalar(string_width_ext_line(str, w, 0)),
              line = 0, width = 0, tw = 0;
    for (size_t i = 0; i < str.length(); i++) {
      uint32_t character = getUnicodeCharacter(str, i);
      if (character == '\r') {
        line += 1,
            xx = halign == fa_center ? x - gs_scalar(string_width_ext_line(str, w, line) / 2)
                                     : x - gs_scalar(string_width_ext_line(str, w, line)),
            yy += (sep + 2 ? fnt->height : sep), i += str[i + 1] == '\n', width = 0;
      } else if (character == '\n') {
        line += 1,
            xx = halign == fa_center ? x - gs_scalar(string_width_ext_line(str, w, line) / 2)
                                     : x - gs_scalar(string_width_ext_line(str, w, line)),
            yy += (sep + 2 ? fnt->height : sep), width = 0;
      } else {
        fontglyph g = findGlyph(fnt, character);
        if (character == ' ' or g.empty()) {
          ...
          if (width + tw >= w && w != -1)
            line += 1,
                xx = halign == fa_center ? x - gs_scalar(string_width_ext_line(str, w, line) / 2)
                                         : x - gs_scalar(string_width_ext_line(str, w, line)),
                yy += (sep == -1 ? fnt->height : sep), width = 0, tw = 0;
        } else {
          draw_primitive_begin_texture(pr_trianglestrip, fnt->texture);
          draw_vertex_texture(xx + g.x, yy + g.y, g.tx, g.ty);
          draw_vertex_texture(xx + g.x2, yy + g.y, g.tx2, g.ty);
...
void draw_text_transformed(gs_scalar x, gs_scalar y, variant vstr, gs_scalar xscale, gs_scalar yscale, double rot) {
  ...
  const gs_scalar sv = sin(rot), cv = cos(rot), svx = sv * xscale, cvx = cv * xscale, svy = sv * yscale,
  ...
  if (valign == fa_top)
    ...
    tmpsize = string_height(str), yy = y + (fnt->yoffset - tmpsize / 2) * cvy,
    xx = x + (fnt->yoffset - tmpsize / 2) * svy;
  else
...
void draw_text_ext_transformed(gs_scalar x, gs_scalar y, variant vstr, gs_scalar sep, gs_scalar w, gs_scalar xscale,
                               gs_scalar yscale, double rot) {
  string str = toString(vstr);
  get_fontv(fnt, currentfont);
  ...
  if (valign == fa_top)
    ...
    tmpsize = string_height_ext(str, sep, w), yy = y + (fnt->yoffset - tmpsize / 2) * cvy,
    xx = x + (fnt->yoffset - tmpsize / 2) * svy;
  else
    tmpsize = string_height_ext(str, sep, w), yy = y + (fnt->yoffset - tmpsize) * cvy,
    xx = x + (fnt->yoffset - tmpsize) * svy;
  tmpx = xx, tmpy = yy;
...
void draw_text_transformed_color(gs_scalar x, gs_scalar y, variant vstr, gs_scalar xscale, gs_scalar yscale, double rot,
                                 int c1, int c2, int c3, int c4, gs_scalar a) {
  string str = toString(vstr);
  get_fontv(fnt, currentfont);
  ...
  if (halign == fa_left) {
    ...
    for (size_t i = 0; i < str.length(); i++) {
      ...
      if (character == '\r') {
        ...
        lines += 1, width = 0, xx = tmpx + lines * shi, yy = tmpy + lines * chi,
                    tmpsize = string_width_line(str, lines);
      } else {
        fontglyph g = findGlyph(fnt, character);
        if (character == ' ' or g.empty()) {
          xx += sw, yy -= sh, width += sw;
        } else {
          w = g.x2 - g.x;
          const gs_scalar lx = xx + g.y * svy;
          const gs_scalar ly = yy + g.y * cvy;
...
void draw_text_ext_transformed_color(gs_scalar x, gs_scalar y, variant vstr, gs_scalar sep, gs_scalar w,
                                     gs_scalar xscale, gs_scalar yscale, double rot, int c1, int c2, int c3, int c4,
                                     gs_scalar a) {
  string str = toString(vstr);
  get_fontv(fnt, currentfont);
    ...
    for (size_t i = 0; i < str.length(); i++) {
      ...
      if (character == '\r') {
        ...
        if (character == ' ' or g.empty()) {
          ...
          if (width + tw >= w && w != -1)
            lines += 1, xx = tmpx + lines * shi, yy = tmpy + lines * chi, width = 0,
                        tmpsize = string_width_ext_line(str, w, lines);
        } else {
          wi = g.x2 - g.x;
          const gs_scalar lx = xx + g.y * svy;
...
void draw_text_ext_color(gs_scalar x, gs_scalar y, variant vstr, gs_scalar sep, gs_scalar w, int c1, int c2, int c3,
                         int c4, gs_scalar a) {
  string str = toString(vstr);
  get_fontv(fnt, currentfont);
  ...
  if (halign == fa_left) {
    ...
    for (size_t i = 0; i < str.length(); i++) {
      ...
      if (character == '\r') {
        xx = x, yy += (sep + 2 ? fnt->height : sep), i += str[i + 1] == '\n', width = 0, line += 1,
        sw = string_width_ext_line(str, w, line);
      } else if (character == '\n') {
        ...
        if (character == ' ' or g.empty()) {
          ...
          if (width + tw >= w && w != -1)
            xx = x, yy += (sep == -1 ? fnt->height : sep), width = 0, line += 1,
            sw = string_width_ext_line(str, w, line);
        } else {
    ...
    for (size_t i = 0; i < str.length(); i++) {
      ...
      if (character == '\r') {
        ...
        yy += (sep + 2 ? fnt->height : sep), width = 0, line += 1, sw = string_width_ext_line(str, w, line),
                                             xx = halign == fa_center ? x - sw / 2 : x - sw, tmpx = xx;
      } else {
        fontglyph g = findGlyph(fnt, character);
        if (character == ' ' or g.empty()) {
          ...
          if (width + tw >= w && w != -1)
            yy += (sep == -1 ? fnt->height : sep), width = 0, line += 1, sw = string_width_ext_line(str, w, line),
                                                   xx = halign == fa_center ? x - sw / 2 : x - sw, tmpx = xx;
        } else {
          hcol1 = merge_color(c1, c2, (gs_scalar)(width) / sw);
          hcol2 = merge_color(c1, c2, (gs_scalar)(width + g.xs) / sw);
...
unsigned font_height(int fnt) {
  get_font_null(f, fnt, -1);
  return f ? f->height : unsigned(-1);
}

void draw_set_font(int fnt) { enigma::currentfont = fnt; }

int draw_get_font() { return enigma::currentfont; }
