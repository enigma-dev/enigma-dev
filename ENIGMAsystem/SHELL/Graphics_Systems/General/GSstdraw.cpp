...
void draw_set_circle_precision(float pr) { enigma::circleprecision = pr < 3 ? 3 : pr; }
...
void draw_rectangle_angle(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, float angle, bool outline) {
  ...
  float xm = (x2 + x1) / 2, ym = (y2 + y1) / 2;

  float len = hypot(xm - x1, ym - y1), dir = atan2(y1 - ym, x1 - xm) + angle;

  float ldx1 = len * cos(dir), ldy1 = len * sin(dir);

  dir = atan2(y2 - ym, x1 - xm) + angle;
  float ldx2 = len * cos(dir), ldy2 = len * sin(dir);
...
void draw_rectangle_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, int c1, int c2, int c3, int c4,
                          bool outline) {
  gs_scalar alpha = draw_get_alpha();
  if (outline) {
    draw_primitive_begin(pr_linestrip);
...
void draw_circle_color_perfect(gs_scalar x, gs_scalar y, float rad, int c1, int c2, bool outline) {
  ...
  if (outline) {
    ...
    for (gs_scalar xc = -rad + 1; xc < rad; xc++) draw_vertex_color(x + xc, y + sqrt(r2 - (xc * xc)), c2, alpha);
    for (gs_scalar xc = rad; xc > -rad; xc--) draw_vertex_color(x + xc, y - sqrt(r2 - (xc * xc)), c2, alpha);
    draw_vertex_color(x - rad, y, c2, alpha);
  }
  draw_primitive_end();
}
...
void draw_ellipse(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, bool outline) {
  gs_scalar x = (x1 + x2) / 2, y = (y1 + y2) / 2, hr = fabs(x2 - x), vr = fabs(y2 - y),
...
void draw_ellipse_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, int c1, int c2, bool outline) {
  ...
  gs_scalar i;
  for (i = pr; i < 2 * M_PI; i += pr) draw_vertex_color(x + hr * cos(i), y + vr * sin(i), c2, alpha);
...
void draw_triangle_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3, int col1,
                         int col2, int col3, bool outline) {
  gs_scalar alpha = draw_get_alpha();
  if (outline) {
    draw_primitive_begin(pr_linestrip);
...
void draw_roundrect_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, float rad, int col1, int col2,
                          bool outline) {
  if (x1 > x2) {
    gs_scalar t = x2;
    x2 = x1;
...
void draw_roundrect_color_ext(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, float xrad, float yrad, int col1,
...
void draw_roundrect_precise(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, float rad, bool outline) {
  ...
  float r2 = rad * rad, r12 = rad * M_SQRT1_2, bx1 = x1 + rad, by1 = y1 + rad, bx2 = x2 - rad, by2 = y2 - rad;
  if (outline) {
    draw_primitive_begin(pr_linelist);
    draw_vertex(x1, by1);
    draw_vertex(x1, by2);
...
void draw_roundrect_precise_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, float rad, int col1, int col2,
                                  bool outline) {
  if (x1 > x2) {
    gs_scalar t = x2;
    x2 = x1;
  ...
  gs_scalar alpha = draw_get_alpha();
  gs_scalar r2 = rad * rad, r12 = rad * M_SQRT1_2, bx1 = x1 + rad, by1 = y1 + rad, bx2 = x2 - rad, by2 = y2 - rad;
  draw_primitive_begin(pr_linelist);
  if (outline) {
...
void draw_arrow(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar arrow_size, gs_scalar line_size,
                bool outline) {
  double dir = atan2(y2 - y1, x2 - x1);
  gs_scalar tc = cos(dir), ts = sin(dir), xs = x2 - tc * arrow_size, ys = y2 - ts * arrow_size,
...
void draw_button(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar border_width, bool up) {
  ...
  if (x2 - x1 < border_width * 2) {
    border_width = (x2 - x1) / 2;
  }
  if (y2 - y1 < border_width * 2) {
    border_width = (y2 - y1) / 2;
  }
  ...
  float alpha = 0.5;
  if (up == true) {
    color = make_color_rgb(127, 127, 127);
  } else {
...
void draw_healthbar(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, float amount, int backcol, int mincol,
                    int maxcol, int dir, bool showback, bool showborder) {
  if (x1 > x2) {  // Swap them
    gs_scalar t = x2;
    x2 = x1, x1 = t;
  }
...
void draw_polygon_begin() { enigma::currComplexPoly.clear(); }
...
void draw_polygon_end(bool outline, bool allowHoles) {
  ...
  if (outline) {
    if (currPoly.size() >= 2) {
      ...
      if (currPoly.back().color == -1) {
        currPoly.back().color = color;
      }
