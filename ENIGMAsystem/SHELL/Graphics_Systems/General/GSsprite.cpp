...
void draw_sprite(int spr, int subimg, gs_scalar x, gs_scalar y, int color, gs_scalar alpha) {
  ...
  const int usi = subimg >= 0 ? (subimg % spr2d->subcount)
                              : int(((enigma::object_graphics *)enigma::instance_event_iterator->inst)->image_index) %
                                    spr2d->subcount;

  const gs_scalar tbx = spr2d->texturexarray[usi], tby = spr2d->textureyarray[usi], tbw = spr2d->texturewarray[usi],
                  tbh = spr2d->textureharray[usi], xvert1 = x - spr2d->xoffset, xvert2 = xvert1 + spr2d->width,
...
void draw_sprite_ext(int spr, int subimg, gs_scalar x, gs_scalar y, gs_scalar xscale, gs_scalar yscale, double rot,
                     int color, gs_scalar alpha) {
  alpha = clamp_alpha(alpha);
  get_spritev(spr2d, spr);
  const int usi = subimg >= 0 ? (subimg % spr2d->subcount)
                              : int(((enigma::object_graphics *)enigma::instance_event_iterator->inst)->image_index) %
  ...
  const gs_scalar w = spr2d->width * xscale, h = spr2d->height * yscale, tbx = spr2d->texturexarray[usi],
                  tby = spr2d->textureyarray[usi], tbw = spr2d->texturewarray[usi], tbh = spr2d->textureharray[usi],
                  wsinrot = w * sin(rot), wcosrot = w * cos(rot);

  draw_primitive_begin_texture(pr_trianglestrip, spr2d->texturearray[usi]);
  gs_scalar ulcx = x - xscale * spr2d->xoffset * cos(rot) + yscale * spr2d->yoffset * cos(M_PI / 2 + rot),
...
void draw_sprite_pos(int spr, int subimg, gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3,
                     gs_scalar y3, gs_scalar x4, gs_scalar y4, gs_scalar alpha) {
  alpha = clamp_alpha(alpha);
  get_spritev(spr2d, spr);
  const int usi = subimg >= 0 ? (subimg % spr2d->subcount)
                              : int(((enigma::object_graphics *)enigma::instance_event_iterator->inst)->image_index) %
...
void draw_sprite_part(int spr, int subimg, gs_scalar left, gs_scalar top, gs_scalar width, gs_scalar height,
                      gs_scalar x, gs_scalar y, int color, gs_scalar alpha) {
  alpha = clamp_alpha(alpha);
  get_spritev(spr2d, spr);
  const int usi = subimg >= 0 ? (subimg % spr2d->subcount)
                              : int(((enigma::object_graphics *)enigma::instance_event_iterator->inst)->image_index) %
  ...
  const gs_scalar tbw = spr2d->width / (gs_scalar)spr2d->texturewarray[usi],
                  tbh = spr2d->height / (gs_scalar)spr2d->textureharray[usi],
...
void draw_sprite_part_offset(int spr, int subimg, gs_scalar left, gs_scalar top, gs_scalar width, gs_scalar height,
                             gs_scalar x, gs_scalar y, int color, gs_scalar alpha) {
  alpha = clamp_alpha(alpha);
  get_spritev(spr2d, spr);
  const int usi = subimg >= 0 ? (subimg % spr2d->subcount)
  ...
  const gs_scalar tbw = spr2d->width / spr2d->texturewarray[usi], tbh = spr2d->height / spr2d->textureharray[usi],
                  xvert1 = x - spr2d->xoffset, xvert2 = xvert1 + spr2d->width, yvert1 = y - spr2d->yoffset,
                  yvert2 = yvert1 + spr2d->height, tbx1 = spr2d->texturexarray[usi] + left / tbw,
                  tbx2 = spr2d->texturexarray[usi] + tbx1 + width / tbw, tby1 = spr2d->textureyarray[usi] + top / tbh,
                  tby2 = spr2d->textureyarray[usi] + tby1 + height / tbh;
...
void draw_sprite_part_ext(int spr, int subimg, gs_scalar left, gs_scalar top, gs_scalar width, gs_scalar height,
                          gs_scalar x, gs_scalar y, gs_scalar xscale, gs_scalar yscale, int color, gs_scalar alpha) {
  alpha = clamp_alpha(alpha);
  get_spritev(spr2d, spr);
  const int usi = subimg >= 0 ? (subimg % spr2d->subcount)
                              : int(((enigma::object_graphics *)enigma::instance_event_iterator->inst)->image_index) %
  ...
  const gs_scalar tbw = spr2d->width / (gs_scalar)spr2d->texturewarray[usi],
                  tbh = spr2d->height / (gs_scalar)spr2d->textureharray[usi], xvert1 = x,
                  xvert2 = xvert1 + width * xscale, yvert1 = y, yvert2 = yvert1 + height * yscale,
                  tbx1 = spr2d->texturexarray[usi] + left / tbw, tbx2 = spr2d->texturexarray[usi] + tbx1 + width / tbw,
                  tby1 = spr2d->textureyarray[usi] + top / tbh, tby2 = spr2d->textureyarray[usi] + tby1 + height / tbh;
...
void draw_sprite_general(int spr, int subimg, gs_scalar left, gs_scalar top, gs_scalar width, gs_scalar height,
                         gs_scalar x, gs_scalar y, gs_scalar xscale, gs_scalar yscale, double rot, int c1, int c2,
                         int c3, int c4, gs_scalar alpha) {
  alpha = clamp_alpha(alpha);
  get_spritev(spr2d, spr);
  const int usi = subimg >= 0 ? (subimg % spr2d->subcount)
                              : int(((enigma::object_graphics *)enigma::instance_event_iterator->inst)->image_index) %
  ...
  draw_vertex_texture_color((ulcx + wcosrot), (ulcy - wsinrot), tbx + (left + width) / tbw, tby + (top + height) / tbh,
                            c4, alpha);
...
void draw_sprite_stretched(int spr, int subimg, gs_scalar x, gs_scalar y, gs_scalar width, gs_scalar height, int color,
                           gs_scalar alpha) {
  alpha = clamp_alpha(alpha);
  get_spritev(spr2d, spr);
  const int usi = subimg >= 0 ? (subimg % spr2d->subcount)
  ...
  const gs_scalar tbx = spr2d->texturexarray[usi], tby = spr2d->textureyarray[usi], tbw = spr2d->texturewarray[usi],
                  tbh = spr2d->textureharray[usi], xvert1 = x - spr2d->xoffset, xvert2 = xvert1 + width,
...
void draw_sprite_stretched_ext(int spr, int subimg, gs_scalar x, gs_scalar y, gs_scalar width, gs_scalar height,
                               int color, gs_scalar alpha) {
...
//Draw padded
void draw_sprite_padded(int spr, int subimg, gs_scalar left, gs_scalar top, gs_scalar right, gs_scalar bottom,
                        gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, int color, gs_scalar alpha) {
  alpha = clamp_alpha(alpha);
  get_spritev(spr2d, spr);
  const int usi = subimg >= 0 ? (subimg % spr2d->subcount)
                              : int(((enigma::object_graphics *)enigma::instance_event_iterator->inst)->image_index) %
  ...
  const gs_scalar tbw = spr2d->width / (gs_scalar)spr2d->texturewarray[usi],
                  tbh = spr2d->height / (gs_scalar)spr2d->textureharray[usi];
  const gs_scalar tbl = left / tbw, tbt = top / tbh, tbr = right / tbw, tbb = bottom / tbh, tbmw = midtw / tbw,
                  tbmh = midth / tbh;

  //Draw top-left corner
  gs_scalar xvert1 = x1, xvert2 = xvert1 + left, yvert1 = y1, yvert2 = yvert1 + top;
  ...
  //Draw left side
  xvert1 = x1, xvert2 = xvert1 + left, yvert1 = y1 + top, yvert2 = yvert1 + midh;
  ...
  xvert1 = x1, xvert2 = xvert1 + left, yvert1 = y1 + top + midh, yvert2 = yvert1 + bottom;
  ...
  //Draw top
  xvert1 = x1 + left, xvert2 = xvert1 + midw, yvert1 = y1, yvert2 = yvert1 + top;
  ...
  //Draw middle
  xvert1 = x1 + left, xvert2 = xvert1 + midw, yvert1 = y1 + top, yvert2 = yvert1 + midh;
  ...
  //Draw bottom
  xvert1 = x1 + left, xvert2 = xvert1 + midw, yvert1 = y1 + midh + top, yvert2 = yvert1 + bottom;
  ...
  xvert1 = x1 + midw + left, xvert2 = xvert1 + right, yvert1 = y1, yvert2 = yvert1 + top;
  ...
  //Draw right side
  xvert1 = x1 + midw + left, xvert2 = xvert1 + right, yvert1 = y1 + top, yvert2 = yvert1 + midh;
  ...
  xvert1 = x1 + midw + left, xvert2 = xvert1 + right, yvert1 = y1 + top + midh, yvert2 = yvert1 + bottom;
...
void draw_sprite_tiled(int spr, int subimg, gs_scalar x, gs_scalar y, int color, gs_scalar alpha) {
  ...
  const int hortil = int(ceil((view_enabled ? (gs_scalar)(view_xview[view_current] + view_wview[view_current])
                                            : (gs_scalar)room_width) /
                              ((gs_scalar)spr2d->width))) +
                     1,
            vertil = int(ceil((view_enabled ? (gs_scalar)(view_yview[view_current] + view_hview[view_current])
                                            : (gs_scalar)room_height) /
                              ((gs_scalar)spr2d->height))) +
                     1;

  gs_scalar xvert1 = -x, xvert2 = xvert1 + spr2d->width, yvert1, yvert2;
  for (int i = 0; i < hortil; ++i) {
    yvert1 = -y;
    yvert2 = yvert1 + spr2d->height;
...
void draw_sprite_tiled_ext(int spr, int subimg, gs_scalar x, gs_scalar y, gs_scalar xscale, gs_scalar yscale, int color,
                           gs_scalar alpha) {
  alpha = clamp_alpha(alpha);
  get_spritev(spr2d, spr);
  const int usi = subimg >= 0 ? (subimg % spr2d->subcount)
  ...
  const gs_scalar tbx = spr2d->texturexarray[usi], tby = spr2d->textureyarray[usi], tbw = spr2d->texturewarray[usi],
                  tbh = spr2d->textureharray[usi], width_scaled = spr2d->width * xscale,
