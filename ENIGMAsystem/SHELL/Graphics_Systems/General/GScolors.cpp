...
float draw_get_alpha() { return enigma::currentcolor[3] / 255.0; }
...
int make_color_rgb(unsigned char r, unsigned char g, unsigned char b) { return r | (g << 8) | (b << 16); }
...
int make_color_hsv(int hue, int saturation, int value) {
  ...
  double vf = v;
  vf /= 255.0;
  double red = bclamp(510 - min(h, 255 - h) * 6) * vf, green = bclamp(510 - max(85 - h, h - 85) * 6) * vf,
