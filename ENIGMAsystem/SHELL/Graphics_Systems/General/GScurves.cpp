...
void draw_set_curve_width(gs_scalar width) { pr_curve_width = (width < 1.0 ? 1.0 : width); }

void draw_set_curve_mode(int mode) { pr_curve_mode = mode; }

void draw_set_curve_detail(int detail) { pr_curve_detail = (detail < 1 ? 1 : detail); }
...
void draw_bezier_quadratic_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3,
                                 int c1, int c2, gs_scalar al1, gs_scalar al2) {
  int col;
...
void draw_bezier_cubic_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3,
                             gs_scalar x4, gs_scalar y4, int c1, int c2, gs_scalar al1, gs_scalar al2) {
  int col;
  gs_scalar x, y, al;
...
gs_scalar draw_bezier_quadratic_x(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3,
                                  float t) {
  float a = 1.0 - t, b = t;
  return x1 * a * a + x2 * 2 * a * b + x3 * b * b;
}

gs_scalar draw_bezier_quadratic_y(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3,
                                  float t) {
  float a = 1.0 - t, b = t;
  return y1 * a * a + y2 * 2 * a * b + y3 * b * b;
}

gs_scalar draw_bezier_cubic_x(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3,
...
gs_scalar draw_bezier_cubic_y(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3,
                              gs_scalar x4, gs_scalar y4, float t) {
  float a = 1.0 - t, b = t, a2, b2;
  a2 = a * a;
  b2 = b * b;
...
void draw_spline_part_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3,
                            gs_scalar x4, gs_scalar y4, int c1, int c2, gs_scalar a1, gs_scalar a2) {
  int col;
  float t = 0.0, det = 1.0 / (gs_scalar)pr_curve_detail;
  gs_scalar x, y, al;
  for (int i = 0; i <= pr_curve_detail; ++i) {
...
void draw_spline2c_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3,
                         gs_scalar x4, gs_scalar y4, int c1, int c2, gs_scalar a1, gs_scalar a2) {
  draw_primitive_begin(pr_curve_mode);
...
void draw_spline3_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3, int c1,
                        int c2, gs_scalar a1, gs_scalar a2) {
  int col1;
  gs_scalar al1;
...
void draw_spline3c_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3,
                         gs_scalar x4, gs_scalar y4, gs_scalar x5, gs_scalar y5, int c1, int c2, gs_scalar a1,
                         gs_scalar a2) {
  int col1;
  gs_scalar al1;
  col1 = merge_color(c1, c2, 0.5);
...
void draw_spline4_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3,
                        gs_scalar x4, gs_scalar y4, int c1, int c2, gs_scalar a1, gs_scalar a2) {
  int col1, col2;
  gs_scalar al1, al2;
...
void draw_spline4c_color(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3, gs_scalar y3,
                         gs_scalar x4, gs_scalar y4, gs_scalar x5, gs_scalar y5, gs_scalar x6, gs_scalar y6, int c1,
                         int c2, gs_scalar a1, gs_scalar a2) {
  int col1, col2;
  gs_scalar al1, al2;
  col1 = merge_color(c1, c2, 0.33);
...
void draw_spline_vertex(gs_scalar x, gs_scalar y) {
  splinePoint point = {x, y, gs_scalar(enigma::currentcolor[3]),
                       int(enigma::currentcolor[0] + (enigma::currentcolor[1] << 8) + (enigma::currentcolor[2] << 16))};
...
void draw_spline_end() {
  ...
  if (arr.size() > 4)
    for (unsigned i = 3; i < arr.size(); ++i)
      draw_spline_part_color(arr[i - 3].x, arr[i - 3].y, arr[i - 2].x, arr[i - 2].y, arr[i - 1].x, arr[i - 1].y,
                             arr[i].x, arr[i].y, arr[i - 2].col, arr[i - 1].col, arr[i - 2].al, arr[i - 1].al);
  draw_primitive_end();
  delete &arr;
  startedSplines.pop();
...
void draw_bezier_quadratic_spline_part(gs_scalar x1, gs_scalar y1, gs_scalar x2, gs_scalar y2, gs_scalar x3,
                                       gs_scalar y3, int c1, int c2, gs_scalar a1, gs_scalar a2) {
  int col;
  float t = 0.0, det = 1.0 / (gs_scalar)pr_curve_detail;
  gs_scalar x, y, al;
  for (int i = 0; i <= pr_curve_detail; ++i) {
...
void draw_bezier_quadratic_spline_end() {
  ...
  if (arr.size() > 3)
    for (unsigned i = 2; i < arr.size(); ++i)
      draw_bezier_quadratic_spline_part(arr[i - 2].x, arr[i - 2].y, arr[i - 1].x, arr[i - 1].y, arr[i].x, arr[i].y,
                                        arr[i - 2].col, arr[i - 1].col, arr[i - 2].al, arr[i - 1].al);
  draw_primitive_end();
  delete &arr;
  startedSplines.pop();
  startedSplinesMode.pop();
...
int draw_spline_optimized_end() {
  ...
  if (arr.size() > 4) {
    for (unsigned i = 3; i < arr.size(); ++i) {
      pr_curve_detail = std::min((float)(sqrt((arr[i - 2].x - arr[i - 1].x) * (arr[i - 2].x - arr[i - 1].x) +
                                              (arr[i - 2].y - arr[i - 1].y) * (arr[i - 2].y - arr[i - 1].y)) /
                                         5.0),
                                 tmp_detail);
