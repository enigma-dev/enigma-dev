...
namespace enigma {
inline bool equal(gs_scalar x, gs_scalar y) {
  return (::fabs(x - y) <=
          std::numeric_limits<gs_scalar>::epsilon() * ::fmax(gs_scalar(1.0), ::fmax(::fabs(x), ::fabs(y))));
}
...
struct Matrix3 {
  ...
  Matrix3(){};
  Matrix3(gs_scalar a00, gs_scalar a01, gs_scalar a02, gs_scalar a10, gs_scalar a11, gs_scalar a12, gs_scalar a20,
          gs_scalar a21, gs_scalar a22) {
    m[0][0] = a00;
    m[0][1] = a01;
    m[0][2] = a02;
    m[1][0] = a10;
...
struct Matrix4 {
  ...
  Matrix4(gs_scalar a00, gs_scalar a01, gs_scalar a02, gs_scalar a03, gs_scalar a10, gs_scalar a11, gs_scalar a12,
          gs_scalar a13, gs_scalar a20, gs_scalar a21, gs_scalar a22, gs_scalar a23, gs_scalar a30, gs_scalar a31,
          gs_scalar a32, gs_scalar a33) {
    m[0][0] = a00;
    m[0][1] = a01;
    m[0][2] = a02;
    m[0][3] = a03;
  ...
  Matrix4(const gs_scalar* array) {
    ...
    for (int k = 0; k < 16; i++, j++, k++) *i = *j;
  }

  inline void set(gs_scalar a00, gs_scalar a01, gs_scalar a02, gs_scalar a03, gs_scalar a10, gs_scalar a11,
                  gs_scalar a12, gs_scalar a13, gs_scalar a20, gs_scalar a21, gs_scalar a22, gs_scalar a23,
  ...
  void init_ortho_proj_transform(gs_scalar left, gs_scalar right, gs_scalar bottom, gs_scalar top, gs_scalar znear,
                                 gs_scalar zfar);
};
...
inline bool Vector3::operator==(const Vector3& V) const { return (equal(x, V.x) && equal(y, V.y) && equal(z, V.z)); }
...
inline gs_scalar Vector3::dot(const Vector3& v) const { return x * v.x + y * v.y + z * v.z; }
...
inline Vector3 operator*(const Vector3& l, gs_scalar f) { return Vector3(l.x * f, l.y * f, l.z * f); }
...
inline gs_scalar Matrix3::operator()(unsigned int a, unsigned int b) const { return m[a][b]; }

inline Matrix3::operator gs_scalar*() { return &(m[0][0]); }
...
inline Matrix4 Matrix4::transpose() const {
  return Matrix4(m[0][0], m[1][0], m[2][0], m[3][0], m[0][1], m[1][1], m[2][1], m[3][1], m[0][2], m[1][2], m[2][2],
...
inline gs_scalar Matrix4::operator()(unsigned int a, unsigned int b) const { return m[a][b]; }

inline Matrix4::operator gs_scalar*() { return &(m[0][0]); }
...
inline void Matrix4::rotate_x(gs_scalar angle) {
  ...
  const gs_scalar m10 = m[1][0], m11 = m[1][1], m12 = m[1][2], m13 = m[1][3], m20 = m[2][0], m21 = m[2][1],
                  m22 = m[2][2], m23 = m[2][3];
...
inline void Matrix4::rotate_y(gs_scalar angle) {
  const gs_scalar s = sin(angle);
  const gs_scalar c = cos(angle);
  const gs_scalar m00 = m[0][0], m01 = m[0][1], m02 = m[0][2], m03 = m[0][3], m20 = m[2][0], m21 = m[2][1],
                  m22 = m[2][2], m23 = m[2][3];
...
inline void Matrix4::rotate_z(gs_scalar angle) {
  const gs_scalar s = sin(angle);
  const gs_scalar c = cos(angle);
  const gs_scalar m00 = m[0][0], m01 = m[0][1], m02 = m[0][2], m03 = m[0][3], m10 = m[1][0], m11 = m[1][1],
                  m12 = m[1][2], m13 = m[1][3];
...
inline Matrix4& Matrix4::inverse() {
  ...
  set(invdet * (m[1][1] * (m[2][2] * m[3][3] - m[2][3] * m[3][2]) + m[1][2] * (m[2][3] * m[3][1] - m[2][1] * m[3][3]) +
                m[1][3] * (m[2][1] * m[3][2] - m[2][2] * m[3][1])),  //m[0][0]
      -invdet * (m[0][1] * (m[2][2] * m[3][3] - m[2][3] * m[3][2]) + m[0][2] * (m[2][3] * m[3][1] - m[2][1] * m[3][3]) +
                 m[0][3] * (m[2][1] * m[3][2] - m[2][2] * m[3][1])),  //m[0][1]
      invdet * (m[0][1] * (m[1][2] * m[3][3] - m[1][3] * m[3][2]) + m[0][2] * (m[1][3] * m[3][1] - m[1][1] * m[3][3]) +
                m[0][3] * (m[1][1] * m[3][2] - m[1][2] * m[3][1])),  //m[0][2]
      -invdet * (m[0][1] * (m[1][2] * m[2][3] - m[1][3] * m[2][2]) + m[0][2] * (m[1][3] * m[2][1] - m[1][1] * m[2][3]) +
                 m[0][3] * (m[1][1] * m[2][2] - m[1][2] * m[2][1])),  //m[0][3]
      -invdet * (m[1][0] * (m[2][2] * m[3][3] - m[2][3] * m[3][2]) + m[1][2] * (m[2][3] * m[3][0] - m[2][0] * m[3][3]) +
                 m[1][3] * (m[2][0] * m[3][2] - m[2][2] * m[3][0])),  //m[1][0]
      invdet * (m[0][0] * (m[2][2] * m[3][3] - m[2][3] * m[3][2]) + m[0][2] * (m[2][3] * m[3][0] - m[2][0] * m[3][3]) +
                m[0][3] * (m[2][0] * m[3][2] - m[2][2] * m[3][0])),  //m[1][1]
      -invdet * (m[0][0] * (m[1][2] * m[3][3] - m[1][3] * m[3][2]) + m[0][2] * (m[1][3] * m[3][0] - m[1][0] * m[3][3]) +
                 m[0][3] * (m[1][0] * m[3][2] - m[1][2] * m[3][0])),  //m[1][2]
      invdet * (m[0][0] * (m[1][2] * m[2][3] - m[1][3] * m[2][2]) + m[0][2] * (m[1][3] * m[2][0] - m[1][0] * m[2][3]) +
                m[0][3] * (m[1][0] * m[2][2] - m[1][2] * m[2][0])),  //m[1][3]
      invdet * (m[1][0] * (m[2][1] * m[3][3] - m[2][3] * m[3][1]) + m[1][1] * (m[2][3] * m[3][0] - m[2][0] * m[3][3]) +
                m[1][3] * (m[2][0] * m[3][1] - m[2][1] * m[3][0])),  //m[2][0]
      -invdet * (m[0][0] * (m[2][1] * m[3][3] - m[2][3] * m[3][1]) + m[0][1] * (m[2][3] * m[3][0] - m[2][0] * m[3][3]) +
                 m[0][3] * (m[2][0] * m[3][1] - m[2][1] * m[3][0])),  //m[2][1]
      invdet * (m[0][0] * (m[1][1] * m[3][3] - m[1][3] * m[3][1]) + m[0][1] * (m[1][3] * m[3][0] - m[1][0] * m[3][3]) +
                m[0][3] * (m[1][0] * m[3][1] - m[1][1] * m[3][0])),  //m[2][2]
      -invdet * (m[0][0] * (m[1][1] * m[2][3] - m[1][3] * m[2][1]) + m[0][1] * (m[1][3] * m[2][0] - m[1][0] * m[2][3]) +
                 m[0][3] * (m[1][0] * m[2][1] - m[1][1] * m[2][0])),  //m[2][3]
      -invdet * (m[1][0] * (m[2][1] * m[3][2] - m[2][2] * m[3][1]) + m[1][1] * (m[2][2] * m[3][0] - m[2][0] * m[3][2]) +
                 m[1][2] * (m[2][0] * m[3][1] - m[2][1] * m[3][0])),  //m[3][0]
      invdet * (m[0][0] * (m[2][1] * m[3][2] - m[2][2] * m[3][1]) + m[0][1] * (m[2][2] * m[3][0] - m[2][0] * m[3][2]) +
                m[0][2] * (m[2][0] * m[3][1] - m[2][1] * m[3][0])),  //m[3][1]
      -invdet * (m[0][0] * (m[1][1] * m[3][2] - m[1][2] * m[3][1]) + m[0][1] * (m[1][2] * m[3][0] - m[1][0] * m[3][2]) +
                 m[0][2] * (m[1][0] * m[3][1] - m[1][1] * m[3][0])),  //m[3][2]
      invdet * (m[0][0] * (m[1][1] * m[2][2] - m[1][2] * m[2][1]) + m[0][1] * (m[1][2] * m[2][0] - m[1][0] * m[2][2]) +
...
inline void Matrix4::init_rotate_axis_transform(gs_scalar angle, gs_scalar RotateX, gs_scalar RotateY,
                                                gs_scalar RotateZ) {
  const gs_scalar x = RotateX;
  const gs_scalar y = RotateY;
  const gs_scalar z = RotateZ;
...
inline void Matrix4::init_camera_transform(const Vector3& from, const Vector3& to, const Vector3& up) {
  ...
  m[0][1] = xaxis.y;
  m[0][2] = xaxis.z;
  m[0][3] = -xaxis.dot(from);
  m[1][0] = yaxis.x;
  m[1][1] = yaxis.y;
  m[1][2] = yaxis.z;
  m[1][3] = -yaxis.dot(from);
  m[2][0] = -zaxis.x;
  m[2][1] = -zaxis.y;
  m[2][2] = -zaxis.z;
  m[2][3] = zaxis.dot(from);
  m[3][0] = 0.0f;
  m[3][1] = 0.0f;
  m[3][2] = 0.0f;
  m[3][3] = 1.0f;
...
inline void Matrix4::init_perspective_proj_transform(gs_scalar fovy, gs_scalar aspect_ratio, gs_scalar znear,
                                                     gs_scalar zfar) {
...
inline void Matrix4::init_ortho_proj_transform(gs_scalar left, gs_scalar right, gs_scalar bottom, gs_scalar top,
                                               gs_scalar znear, gs_scalar zfar) {
