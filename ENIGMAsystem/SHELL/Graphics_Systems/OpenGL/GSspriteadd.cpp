...
namespace enigma
{
    ...
    int new_sprexe(int sprid, int w, int h, int x, int y, int pl, int sm)
    {
         ...
         if (enigma::spritestructarray[sprid]==NULL)
         { show_error("Could not allocate enough memory for a relatively small structure. Abort likely necessary.",1); return -1; }

         enigma::spritestructarray[sprid]->id=enigma::currentspriteind;
         ...
         if (enigma::currentspriteind<=sprid) enigma::currentspriteind=sprid+1;
