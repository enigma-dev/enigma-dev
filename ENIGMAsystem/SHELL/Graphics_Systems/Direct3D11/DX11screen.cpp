...
void screen_redraw() {
  ...
  if (!view_enabled) {
    ...
    if (enigma::d3dMode) m_deviceContext->ClearDepthStencilView(m_depthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);
    ...
    for (map<int, pair<double, double> >::iterator it = id_to_currentnextdepth.begin();
      ...
      if (inst_depth != NULL) {
        ...
        inst_iter* mynewiter =
            drawing_depths[(*it).second.second].draw_events->add_inst(inst_depth->depth.myiter->inst);
