  ...
  void SmoothNormals()
  {
	...
	for (vector<gs_scalar>::const_iterator i = triangleVertices.begin(); i != triangleVertices.end(); i += stride)
	{
		...
		for (vector< vector<unsigned int> >::iterator ig = groupList.begin(); ig != groupList.end(); ++ig)
		{
			...
			gs_scalar x2 = triangleVertices[index*stride + 0].f;
			gs_scalar y2 = triangleVertices[index*stride + 1].f;
			gs_scalar z2 = triangleVertices[index*stride + 2].f; 
	...
	for (vector< vector<unsigned int> >::iterator ig = groupList.begin(); ig != groupList.end(); ++ig)
	{
		...
		{
			anx += triangleVertices[(*i)*stride+3].f;
			any += triangleVertices[(*i)*stride+4].f;
			anz += triangleVertices[(*i)*stride+5].f;
		...
		for (vector<unsigned int>::iterator i = (*ig).begin(); i != (*ig).end(); ++i)
		{
			triangleVertices[(*i)*stride+3].f = anx;
			triangleVertices[(*i)*stride+4].f = any;
			triangleVertices[(*i)*stride+5].f = anz;
		}
	}
  }
*/
