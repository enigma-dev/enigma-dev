    ...
    if (view_enabled) {
      for (int vc = 0; vc < 7; vc++)
        if (view_visible[vc]) {
          ...
          if (gridx > 0)
            for (int i = (int)((double)view_xview[vc] / gridx) * gridx; i < view_xview[vc] + view_wview[vc]; i += gridx)
              draw_line(i, view_yview[vc], i, view_yview[vc] + view_hview[vc]);
          if (gridy > 0)
            for (int i = (int)((double)view_yview[vc] / gridx) * gridy; i < view_yview[vc] + view_hview[vc]; i += gridy)
              draw_line(view_xview[vc], i, view_xview[vc] + view_wview[vc], i);
        }
    } else {
      glBlendFunc(GL_ONE_MINUS_DST_COLOR, GL_ZERO);
      if (gridx > 0)
        for (int i = 0; i < room_width; i += gridx) draw_line(i, 0, i, h);
      if (gridy > 0)
        for (int i = 0; i < room_height; i += gridy) draw_line(0, i, w, i);
    }
...
bool checkpause() {
  ...
  if ((enigma::keybdstatus[vk_control] == 1 && enigma::last_keybdstatus[vk_space] == 0 &&
       enigma::keybdstatus[vk_space] == 1) ||
...
void buildinit() {
  ...
  buildhwnd = CreateWindow("TBuild", "Build Mode Access Panel", WS_VISIBLE | DS_CONTROL, r.left, r.top,
                           r.right - r.left, r.bottom - r.top, enigma::hWndParent, NULL, Bhinst, NULL);

  //Add a combo box to that with the object data in it
  object = CreateWindow("COMBOBOX", "Combo1", WS_VISIBLE | CBS_DROPDOWNLIST | WS_CHILD, 8, 8, 304, 620, buildhwnd, NULL,
                        Bhinst, NULL);
  ...
  grid = CreateWindow("BUTTON", "Grid", WS_VISIBLE | WS_CHILD | BS_AUTOCHECKBOX, 152, 64, 60, 24, buildhwnd, NULL,
                      Bhinst, NULL);
  SendMessage(grid, BM_SETCHECK, 1, 0);

  step = CreateWindow("BUTTON", "Step", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 312 - 48, 40, 48, 56, buildhwnd, NULL,
                      Bhinst, NULL);
}
...
void finishup() {
  ...
  for (rciter = changes.begin(); rciter != changes.end(); rciter++) {
    int rm = (*rciter).first;
    fwrite(&rm, 4, 1, chout);
    fwrite(&changecount[rm], 4, 1, chout);
