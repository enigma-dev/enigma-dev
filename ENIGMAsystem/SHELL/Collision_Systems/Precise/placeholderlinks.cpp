                         ...
                         collision_type ct)  // It is called for every subimage of every sprite loaded.
{
  switch (ct) {
    case ct_precise: {
      ...
      for (unsigned int rowindex = 0; rowindex < h; rowindex++) {
        for (unsigned int colindex = 0; colindex < w; colindex++) {
          colldata[rowindex * w + colindex] =
              (input_data[4 * (rowindex * w + colindex) + 3] != 0) ? 1 : 0;  // If alpha != 0 then 1 else 0.
        }
      }
    ...
    case ct_ellipse: {
      ...
      const int minX = max(bbox.left - 2, 0), minY = max(bbox.top - 2, 0), maxX = min(bbox.right + 2, w),
    ...
    case ct_diamond: {
      ...
      // Diamond corners.
      const int xl = bbox.left, yl = (bbox.top + bbox.bottom) / 2, xr = bbox.right, yr = (bbox.top + bbox.bottom) / 2,
                xt = (bbox.left + bbox.right) / 2, yt = bbox.top, xb = (bbox.left + bbox.right) / 2, yb = bbox.bottom;
      ...
      for (int y = minY; y < maxY; y++) {
        for (int x = minX; x < maxX; x++) {
          ...
          const bool is_inside_diamond =
              cp(xlt, -ylt, xlp, -ylp) <= 0 &&  // Use cross-product to determine whether inside or outside diamond.
              cp(xlb, -ylb, xlp, -ylp) >= 0 && cp(xrt, -yrt, xrp, -yrp) >= 0 && cp(xrb, -yrb, xrp, -yrp) <= 0;
