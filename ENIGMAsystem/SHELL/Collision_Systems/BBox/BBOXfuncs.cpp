...
static inline void get_border(int* leftv, int* rightv, int* topv, int* bottomv, int left, int top, int right,
                              int bottom, cs_scalar x, cs_scalar y, cs_scalar xscale, cs_scalar yscale, double angle) {
  if (fzero(angle)) {
    const bool xsp = (xscale >= 0), ysp = (yscale >= 0);
    const double lsc = left * xscale, rsc = (right + 1) * xscale - 1, tsc = top * yscale,
                 bsc = (bottom + 1) * yscale - 1;
    ...
    *bottomv = (ysp ? bsc : tsc) + y + .5;
    ...
    const bool xsp = (xscale >= 0), ysp = (yscale >= 0), q12 = (quad == 1 || quad == 2), q23 = (quad == 2 || quad == 3),
...
static inline double point_direction(double x1, double y1, double x2, double y2) {
...
bool place_meeting(cs_scalar x, cs_scalar y, int object) { return collide_inst_inst(object, false, true, x, y); }
...
bool position_free(cs_scalar x, cs_scalar y) { return collide_inst_point(all, true, false, x + .5, y + .5) == NULL; }

bool position_empty(cs_scalar x, cs_scalar y) { return collide_inst_point(all, false, false, x + .5, y + .5) == NULL; }
...
void position_destroy_solid(cs_scalar x, cs_scalar y) { destroy_inst_point(all, true, x + .5, y + .5); }

void position_destroy(cs_scalar x, cs_scalar y) { destroy_inst_point(all, false, x + .5, y + .5); }
...
enigma::instance_t collision_rectangle(cs_scalar x1, cs_scalar y1, cs_scalar x2, cs_scalar y2, int obj,
                                       bool prec /*ignored*/, bool notme) {
  const enigma::object_collisions* r =
      collide_inst_rect(obj, false, notme, x1 + .5, y1 + .5, x2 + .5, y2 + .5);  //false is for solid_only, not prec
  return r == NULL ? noone : static_cast<int>(r->id);
}

enigma::instance_t collision_line(cs_scalar x1, cs_scalar y1, cs_scalar x2, cs_scalar y2, int obj,
                                  bool prec /*ignored*/, bool notme) {
  const enigma::object_collisions* r =
      collide_inst_line(obj, false, notme, x1 + .5, y1 + .5, x2 + .5, y2 + .5);  //false is for solid_only, not prec
  return r == NULL ? noone : static_cast<int>(r->id);
}

enigma::instance_t collision_point(cs_scalar x, cs_scalar y, int obj, bool prec /*ignored*/, bool notme) {
  const enigma::object_collisions* r =
      collide_inst_point(obj, false, notme, x + .5, y + .5);  //false is for solid_only, not prec
  return r == NULL ? noone : static_cast<int>(r->id);
}

enigma::instance_t collision_circle(cs_scalar x, cs_scalar y, double radius, int obj, bool prec /*ignored*/,
                                    bool notme) {
  const enigma::object_collisions* r =
      collide_inst_circle(obj, false, notme, x + .5, y + .5, radius);  //false is for solid_only, not prec
  return r == NULL ? noone : static_cast<int>(r->id);
}

enigma::instance_t collision_ellipse(cs_scalar x1, cs_scalar y1, cs_scalar x2, cs_scalar y2, int obj,
                                     bool prec /*ignored*/, bool notme) {
  const enigma::object_collisions* r =
      collide_inst_ellipse(obj, false, notme, ((x1 + x2) / 2) + .5, ((y1 + y2) / 2) + .5, fabs(x2 - x1) / 2,
                           fabs(y2 - y1) / 2);  //false is for solid_only, not prec
  return r == NULL ? noone : static_cast<int>(r->id);
}
...
double distance_to_object(int object) {
  ...
  if (inst1->sprite_index == -1 && (inst1->mask_index == -1)) return -1;
  double distance = std::numeric_limits<double>::infinity();
  double tempdist;
  const enigma::bbox_rect_t& box = inst1->$bbox_relative();
  const double x1 = inst1->x, y1 = inst1->y, xscale1 = inst1->image_xscale, yscale1 = inst1->image_yscale,
  ...
  for (enigma::iterator it = enigma::fetch_inst_iter_by_int(object); it; ++it) {
    ...
    if (inst2->sprite_index == -1 && (inst2->mask_index == -1)) continue;

    const enigma::bbox_rect_t& box2 = inst2->$bbox_relative();
    const double x2 = inst2->x, y2 = inst2->y, xscale2 = inst2->image_xscale, yscale2 = inst2->image_yscale,
                 ia2 = inst2->image_angle;
    int left2, top2, right2, bottom2;

    get_border(&left2, &right2, &top2, &bottom2, box2.left, box2.top, box2.right, box2.bottom, x2, y2, xscale2, yscale2,
               ia2);

    const int right = min(right1, right2), left = max(left1, left2), bottom = min(bottom1, bottom2),
              top = max(top1, top2);

    tempdist = hypot((left > right ? left - right : 0), (top > bottom ? top - bottom : 0));
...
double distance_to_point(cs_scalar x, cs_scalar y) {
  ...
  return fabs(hypot(min(left1 - x, right1 - x), min(top1 - y, bottom1 - y)));
}

double move_contact_object(int object, double angle, double max_dist, bool solid_only) {
  enigma::object_collisions* const inst1 = ((enigma::object_collisions*)enigma::instance_event_iterator->inst);
  if (inst1->sprite_index == -1 && (inst1->mask_index == -1)) return -4;
  ...
  for (enigma::iterator it = enigma::fetch_inst_iter_by_int(object); it; ++it) {
    ...
    switch (quad & 3) {
      case 0:
      default:  // Default case prevents warnings; value can never be outside this range
...
double move_outside_object(int object, double angle, double max_dist, bool solid_only) {
  ...
  const int quad = int(angle / 90.0);
  const double xscale1 = inst1->image_xscale, yscale1 = inst1->image_yscale, ia1 = inst1->image_angle;
  ...
  while (had_collision)  // If there was no collision in the last iteration, we have moved outside the object.
  {
    ...
    for (enigma::iterator it = enigma::fetch_inst_iter_by_int(object); it; ++it) {
      ...
      get_border(&left1, &right1, &top1, &bottom1, box.left, box.top, box.right, box.bottom, x1, y1, xscale1, yscale1,
                 ia1);
      ...
      get_border(&left2, &right2, &top2, &bottom2, box2.left, box2.top, box2.right, box2.bottom, x2, y2, xscale2,
                 yscale2, ia2);

      if (!(right2 >= left1 && bottom2 >= top1 && left2 <= right1 && top2 <= bottom1)) continue;
...
bool move_bounce_object(int object, bool adv, bool solid_only) {
  enigma::object_collisions* const inst1 = ((enigma::object_collisions*)enigma::instance_event_iterator->inst);
  if (inst1->sprite_index == -1 && (inst1->mask_index == -1)) return false;
  ...
  for (enigma::iterator it = enigma::fetch_inst_iter_by_int(object); it; ++it) {
    ...
    if (right2 >= left1 && bottom2 >= top1 && left2 <= right1 && top2 <= bottom1) return false;
  ...
  switch (side_type) {
    ...
    case 3:
    case 4:
    default:  //corner or both horizontal and vertical side hit
      inst1->hspeed *= -1;
      inst1->vspeed *= -1;
      break;
  }
...
void instance_deactivate_region(int rleft, int rtop, int rwidth, int rheight, bool inside, bool notme) {
  for (enigma::iterator it = enigma::instance_list_first(); it; ++it) {
    ...
    const double x = inst->x, y = inst->y, xscale = inst->image_xscale, yscale = inst->image_yscale,
                 ia = inst->image_angle;
...
void instance_deactivate_circle(int x, int y, int r, bool inside, bool notme) {
  for (enigma::iterator it = enigma::instance_list_first(); it; ++it) {
    ...
    const enigma::bbox_rect_t& box = inst->$bbox_relative();
    const double x1 = inst->x, y1 = inst->y, xscale = inst->image_xscale, yscale = inst->image_yscale,
                 ia = inst->image_angle;
...
void position_change(cs_scalar x1, cs_scalar y1, int obj, bool perf) {
  for (enigma::iterator it = enigma::fetch_inst_iter_by_int(enigma_user::all); it; ++it) {
    ...
    if (x1 >= left && x1 <= right && y1 >= top && y1 <= bottom) enigma::instance_change_inst(obj, perf, inst);
  }
}
