...
bool collide_rect_line(double rx1, double ry1, double rx2, double ry2, double px1, double py1, double px2, double py2) {
...
//unused
bool collide_rect_rect(double r1x1, double r1y1, double r1x2, double r1y2, double r2x1, double r2y1, double r2x2,
                       double r2y2) {
...
bool collide_bbox_rect(const enigma::object_collisions* inst, double ox, double oy, double x1, double y1, double x2,
                       double y2) {
  const bbox_rect_t& box = inst->$bbox_relative();
  return collide_rect_rect(box.left + ox, box.top + oy, box.right + ox, box.bottom + oy, x1, y1, x2, y2);
}

bool collide_bbox_line(const enigma::object_collisions* inst, double ox, double oy, double x1, double y1, double x2,
                       double y2) {
  const bbox_rect_t& box = inst->$bbox_relative();
  return collide_rect_line(box.left + ox, box.top + oy, box.right + ox, box.bottom + oy, x1, y1, x2, y2);
}

bool collide_bbox_bbox(const enigma::object_collisions* inst1, double ox1, double oy1,
                       const enigma::object_collisions* inst2, double ox2, double oy2) {
  const bbox_rect_t& box1 = inst1->$bbox_relative();
  const bbox_rect_t& box2 = inst2->$bbox_relative();
  return (box1.left + ox1 < box2.right + ox2 && box2.left + ox2 < box1.right + ox1 &&
          box1.top + oy1 < box2.bottom + oy2 && box2.top + oy2 < box1.bottom + oy1);
}

bool collide_bbox_point(const enigma::object_collisions* inst, double ox, double oy, double x, double y) {
  const bbox_rect_t& box = inst->$bbox_relative();
  return (x < box.right + ox && x > box.left + ox && y < box.bottom + oy && y > box.top + oy);
}
...
bool collide_bbox_ellipse(const enigma::object_collisions* inst, double ox, double oy, double x, double y, double rx,
                          double ry) {
  if (fzero(rx) || fzero(ry)) return false;
  const bbox_rect_t& box = inst->$bbox_relative();
