...
enigma::object_collisions* const collide_inst_inst(int object, bool solid_only, bool notme, double x, double y) {
  ...
  for (enigma::iterator it = enigma::fetch_inst_iter_by_int(object); it; ++it) {
    ...
    if (left1 <= right2 && left2 <= right1 && top1 <= bottom2 && top2 <= bottom1) return inst2;
  }
  return NULL;
}

enigma::object_collisions* const collide_inst_rect(int object, bool solid_only, bool notme, int x1, int y1, int x2,
                                                   int y2) {
  if (x1 > x2) {
    int x3 = x1;
    x2 = x1;
  ...
  for (enigma::iterator it = enigma::fetch_inst_iter_by_int(object); it; ++it) {
    ...
    if (left <= x2 && x1 <= right && top <= y2 && y1 <= bottom) return inst;
  }
  return NULL;
}

enigma::object_collisions* const collide_inst_line(int object, bool solid_only, bool notme, int x1, int y1, int x2,
                                                   int y2) {
  // Ensure x1 != x2 || y1 != y2.
  if (x1 == x2 && y1 == y2) return collide_inst_point(object, solid_only, notme, x1, y1);
...
enigma::object_collisions* const collide_inst_point(int object, bool solid_only, bool notme, int x1, int y1) {
  for (enigma::iterator it = enigma::fetch_inst_iter_by_int(object); it; ++it) {
    ...
    if (x1 >= left && x1 <= right && y1 >= top && y1 <= bottom) return inst;
  }
  return NULL;
}

enigma::object_collisions* const collide_inst_circle(int object, bool solid_only, bool notme, int x1, int y1,
                                                     double r) {
...
enigma::object_collisions* const collide_inst_ellipse(int object, bool solid_only, bool notme, int x1, int y1,
                                                      double rx, double ry) {
  if (fzero(rx) || fzero(ry)) return 0;
...
void destroy_inst_point(int object, bool solid_only, int x1, int y1) {
  for (enigma::iterator it = enigma::fetch_inst_iter_by_int(object); it; ++it) {
    ...
    if (x1 >= left && x1 <= right && y1 >= top && y1 <= bottom) enigma_user::instance_destroy(inst->id);
  }
}
