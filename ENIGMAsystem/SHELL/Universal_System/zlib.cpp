...
unsigned char* zlib_compress(unsigned char* inbuffer, int actualsize) {
  ...
  if (res != Z_OK) {
    ...
    if (res == Z_MEM_ERROR) show_error("Zlib failed to compress the buffer. Out of memory.", 0);
    if (res == Z_BUF_ERROR) show_error("Zlib failed to compress the buffer. Output size greater than allotted.", 0);
#endif
  }
...
int zlib_decompress(unsigned char* inbuffer, int insize, int uncompresssize, unsigned char* outbytef) {
  ...
  switch (uncompress(outbytef, &outused, (Bytef*)inbuffer, insize)) {
    ...
    case Z_BUF_ERROR:
#if DEBUG_MODE || (defined(SHOW_ERRORS) && SHOW_ERRORS)
      show_error("Zerror: Output of " + enigma_user::toString(outused) + " above allotted " +
                     enigma_user::toString(uncompresssize),
                 0);
#endif
      return -2;
    case Z_DATA_ERROR:
