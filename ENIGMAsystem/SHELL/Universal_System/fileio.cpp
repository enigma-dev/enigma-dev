...
void file_text_close(int fileid) // Closes the file with the given file id.
{
   if(fileid == -1) {
    #ifdef DEBUG_MODE
      DEBUG_MESSAGE("Cannot close an unopened file.", MESSAGE_TYPE::ERROR);
    #endif
    return;
  }
...
bool file_bin_rewrite(int fileid) // Rewrites the file with the given file id, that is, clears it and starts writing at the start.
{
  ...
  if (mf.f == NULL) {
    #ifdef DEBUGMODE
      DEBUG_MESSAGE("Failed to reopen binary file. Sure it's a binary file? Drive been removed?", MESSAGE_TYPE::ERROR);
    #endif
    return false;
  }
  return true;
