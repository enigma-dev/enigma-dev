...
bool file_exists(string fname)           // Returns whether the file with the given name exists (true) or not (false).
{
  
}
void file_delete(string fname)           // Deletes the file with the given name.
{
  
}
void file_rename(string oldname,string newname) // Renames the file with name oldname into newname.
{
  
}
void file_copy(string fname,string newname)     // Copies the file fname to the newname.
{
  
}
bool directory_exists(string dname)      // Returns whether the indicated directory does exist. The name must include the full path, not a relative path.
{
  
}
void directory_create(string dname)      // Creates a directory with the given name (including the path towards it) if it does not exist. The name must include the full path, not a relative path.
{
...
// If no such file exists, an empty string is returned. The mask can contain a
// path and can contain wildchars, for example 'C:\temp\*.doc'. The attributes
// give the additional files you want to see. (So the normal files are always 
// returned when they satisfy the mask.)
string  file_find_first(string mask,string attr)  
{
  
}

// The following constants can be ORed or added to see the type of files you want:
enum {
  fa_readonly  = 1,
  fa_hidden    = 2,
  fa_sysfile   = 4,
...
bool file_attributes(string fname,string attr) // Returns whether the file has all the attributes given in attr. Use a combination of the constants indicated above.
{
...
void   filename_change_ext(string fname,string newext) // Returns the indicated file name, with the extension (including the dot) changed to the new extension. By using an empty string as the new extension you can remove the extension.
{
...
void export_include_file(string fname)                   // Exports the included file with the name fname. This must be a string variable, so don't forget the quotes.
{
  
}
void export_include_file_location(string fname,string location) // Exports the included file with the name fname to the given location. Location must contain the path and the filename.
{
  
}
void discard_include_file(string fname)                  // Discard the included file with the name fname, freeing the memory used. This must be a string variable, so don't forget the quotes.
{
...
string parameter_string(string n)
{
