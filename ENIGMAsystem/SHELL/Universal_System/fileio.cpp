...
int file_text_open_read(
    string
        fname)  // Opens the file with the indicated name for reading. The function returns the id of the file that must be used in the other functions. You can open multiple files at the same time (32 max). Don't forget to close them once you are finished with them.
{
...
int file_text_open_append(
    string
        fname)  // Opens the indicated file for appending data at the end, creating it if it does not exist. The function returns the id of the file that must be used in the other functions.
{
...
void file_text_close(int fileid)  // Closes the file with the given file id.
{
  ...
  for (int i = enigma::file_highid; i >= 0; i--)
    if (enigma::files[i].f == NULL) enigma::file_highid = i;
}
...
string file_text_read_string(
    int fileid) {  // Reads a string from the file with the given file id and returns this string. A string ends at the end of line.
...
double file_text_read_real(int fileid) {  // Reads a real value from the file and returns this value.
  ...
  if (sscanf(mf.sdata.substr(mf.spos).c_str(), "%lf%n", &r1, &apos), apos) mf.spos += apos;
  if (mf.spos >= mf.sdata.length() && feof(mf.f)) mf.eof = true;
  return r1;
}

void file_text_readln(int fileid)  // Skips the rest of the line in the file and starts at the start of the next line.
{
  if (feof(enigma::files[fileid].f)) enigma::files[fileid].eof = true;
  string ret;
  char buf[BUFSIZ];
  buf[0] = 0;
  while (fgets(buf, BUFSIZ, enigma::files[fileid].f)) {
    ret += buf;
    if (ret[ret.length() - 1] == '\n' or ret[ret.length() - 1] == '\r') break;
    buf[0] = 0;
  }
  size_t dp;
...
int file_bin_open(
    string fname,
    int mode)  // Opens the file with the indicated name. The mode indicates what can be done with the file: 0 = reading, 1 = writing, 2 = both reading and writing). When the file does not exist it is created. The function returns the id of the file that must be used in the other functions.
{
  FILE *a;
  if (mode & 2) {
    a = fopen(fname.c_str(), "r+b");  //Open for reading + writing if it exists, failing otherwise
    if (!a)
      a = fopen(fname.c_str(), "w+b");  //Open for writing + reading, overwriting if it exists or creating otherwise
  } else
    a = fopen(fname.c_str(), (mode & 1) ? "wb" : "rb");
...
bool file_bin_rewrite(
    int fileid)  // Rewrites the file with the given file id, that is, clears it and starts writing at the start.
{
  enigma::openFile &mf = enigma::files[fileid];
...
size_t file_bin_position(
    int fileid) {  // Returns the current position (in bytes; 0 is the first position) of the file with the given file id.
...
void file_bin_seek(
    int fileid,
    size_t
        pos) {  // Moves the current position of the file to the indicated position. To append to a file move the position to the size of the file before writing.
