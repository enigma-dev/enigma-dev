...
void hspeedv::function(variant oldval) {
  if (rval.d == oldval.rval.d) {  // If no changes, return, don't make potentially negative speed non-negative.
    return;
  ...
  } else {  // If both set to zero, make speed zero. Direction not set.
    *spd = 0.0;
  }
}
