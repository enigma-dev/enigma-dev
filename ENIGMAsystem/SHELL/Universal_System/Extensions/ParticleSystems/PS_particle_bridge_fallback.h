...
static void draw_particle(particle_instance* it) {
  ...
  if (it->pt->alive) {
    ...
    if (!pt->is_particle_sprite) {  // Draw sprite.
      ...
      enigma_user::draw_sprite_ext(sprite_id, subimg, x + x_offset, y + y_offset, xscale, yscale, rot_degrees, color,
                                   (double)alpha / 255.0);
      ...
      enigma_user::draw_sprite_ext(sprite_id, 0, x + x_offset, y + y_offset, xscale, yscale, rot_degrees, color,
                                   (double)alpha / 255.0);
    }
...
void draw_particles(std::vector<particle_instance>& pi_list, bool oldtonew, double a_wiggle, int a_subimage_index,
  ...
  if (enigma::currentblendtype != blend_type || enigma::currentblendmode[0] != blend_src ||
      enigma::currentblendmode[1] != blend_dest) {
    if (blend_type == 0) {
