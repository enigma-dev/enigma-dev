...
void particle_destroyer::clear_particle_destroyer() { initialize(); }
...
void part_destroyer_destroy_all(int ps_id) {
  particle_system* p_s = enigma::get_particlesystem(ps_id);
  if (p_s != NULL) {
    for (std::map<int, particle_destroyer*>::iterator it = p_s->id_to_destroyer.begin();
         it != p_s->id_to_destroyer.end(); it++) {
      delete (*it).second;
    }
