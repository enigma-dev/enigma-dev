...
void particle_attractor::clear_particle_attractor() { initialize(); }
void particle_attractor::set_position(double p_x, double p_y) {
  x = p_x;
  y = p_y;
}
void particle_attractor::set_force(double p_force_strength, double p_dist_effect, ps_force p_force_kind,
                                   bool p_additive) {
...
void part_attractor_destroy_all(int ps_id) {
  particle_system* p_s = enigma::get_particlesystem(ps_id);
  if (p_s != NULL) {
    for (std::map<int, particle_attractor*>::iterator it = p_s->id_to_attractor.begin();
         it != p_s->id_to_attractor.end(); it++) {
      delete (*it).second;
    }
