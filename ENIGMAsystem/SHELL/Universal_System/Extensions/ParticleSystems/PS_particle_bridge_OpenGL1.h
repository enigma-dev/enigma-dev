...
static void draw_particle(particle_instance* it) {
  ...
  if (it->pt->alive) {
    ...
    double rot_degrees;
    size =
        std::max(0.0, it->size + pt->size_wiggle * particle_system::get_wiggle_result(it->size_wiggle_offset, wiggle));
    ...
    if (!pt->is_particle_sprite) {  // Draw sprite.
      ...
      const float w = ps->width * xscale, h = ps->height * yscale, tbx = 1, tby = 1, wsinrot = w * sin(rot),
                  wcosrot = w * cos(rot);

      glBegin(GL_TRIANGLE_STRIP);

      float ulcx = x - xscale * (ps->width / 2.0) * cos(rot) + yscale * (ps->height / 2.0) * cos(M_PI / 2 + rot),
