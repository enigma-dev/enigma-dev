  ...
  switch (effect_kind) {
    ...
    case ef_effects_smoke: {
      ...
      if (p_t != NULL) {
        p_t->hidden = false;
        for (int i = 0; i < num; i++) {
          part_particles_create_color(ps_id, x + dist * rand() / RAND_MAX - dist / 2.0,
                                      y + dist * rand() / RAND_MAX - dist / 2.0, p_t->id, color, 1);
        }
        p_t->hidden = true;
      }
      break;
...
static void create_effect_in_particlesystem(int& ps_id, double depth, int kind, double x, double y, int size,
                                            int color) {
...
void effect_clear() {
  ...
  enigma::ps_above_id = -1;
  {
    ...
    for (std::map<ef_pt, int>::iterator id_it = enigma::effectparticletype_to_particletypeid.begin(); id_it != id_end;
         id_it++) {
