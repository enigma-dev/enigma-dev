...
double particle_system::get_wiggle_result(double wiggle_offset) { return get_wiggle_result(wiggle_offset, wiggle); }
double particle_system::get_wiggle_result(double wiggle_offset, double wiggle_amount) {
  ...
  if (result_wiggle < 0.5)
    return -1.0 + 4 * result_wiggle;
  else
    return 3.0 - 4.0 * result_wiggle;
}
...
static inline bool is_dead_from_old_age(particle_instance& pi) { return pi.life_current <= 0; }
...
void particle_system::update_particlesystem() {
    ...
    for (std::vector<particle_instance>::iterator it = pi_list.begin(); it != pi_list.end(); it++) {
      ...
      if (it->life_current <= 0) {  // Death.
        ...
        if (pt->alive && pt->death_on) {
          std::map<int, particle_type*>::iterator death_pt_it =
              pt_manager.id_to_particletype.find(pt->death_particle_id);
  ...
  // Color and blending.
  {
    ...
    for (std::vector<particle_instance>::iterator it = pi_list.begin(); it != end; it++) {
      ...
      switch (pt->c_mode) {
        ...
        case two_color: {
          if (pt->alive) {
            const int r1 = color_get_red(pt->color1), g1 = color_get_green(pt->color1), b1 = color_get_blue(pt->color1);
            const int r2 = color_get_red(pt->color2), g2 = color_get_green(pt->color2), b2 = color_get_blue(pt->color2);
            const double part = 1.0 - 1.0 * it->life_current / it->life_start;
            it->color = make_color_rgb(int((1 - part) * r1 + part * r2), int((1 - part) * g1 + part * g2),
                                       int((1 - part) * b1 + part * b2));
          }
          break;
        }
        case three_color: {
          if (pt->alive) {
            ...
            const int r1 = color_get_red(first_color), g1 = color_get_green(first_color),
                      b1 = color_get_blue(first_color);
            const int r2 = color_get_red(second_color), g2 = color_get_green(second_color),
  ...
  // Changers.
  {
    ...
    for (std::map<int, particle_changer*>::iterator ch_it = id_to_changer.begin(); ch_it != end1; ch_it++) {
      ...
      particle_type* pt1;
      particle_type* pt2;
      std::map<int, enigma::particle_type*>::iterator pt_it1 =
          enigma::pt_manager.id_to_particletype.find(p_ch->parttypeid1);
      if (pt_it1 == enigma::pt_manager.id_to_particletype.end()) {
        continue;
      }
      std::map<int, enigma::particle_type*>::iterator pt_it2 =
          enigma::pt_manager.id_to_particletype.find(p_ch->parttypeid2);
      ...
      for (std::vector<particle_instance>::iterator it = pi_list.begin(); it != end2; it++) {
        if (p_ch->is_inside(it->x, it->y) && it->pt->id == pt1->id &&
            it->life_current > 0) {  // Skip particles with life_current <= 0.
  ...
  // Generate particles.
  for (std::vector<generation_info>::iterator it = particles_to_generate.begin(); it != particles_to_generate.end();
       it++) {
    ...
    for (std::map<int, particle_attractor*>::iterator at_it = id_to_attractor.begin(); at_it != end; at_it++) {
      ...
      for (std::vector<particle_instance>::iterator it = pi_list.begin(); it != end1; it++) {
        ...
        switch (p_a->force_kind) {
          case ps_fo_constant:
            force_effective_strength = p_a->force_strength;
            break;
          case ps_fo_linear:
            force_effective_strength = (1.0 - relative_distance) * p_a->force_strength;
            break;
          case ps_fo_quadratic:
            force_effective_strength = (1.0 - relative_distance) * (1.0 - relative_distance) * p_a->force_strength;
            break;
          default:
            force_effective_strength = p_a->force_strength;
            break;
        ...
        if (p_a->additive) {
          const double vx =
              it->speed * cos(it->direction * M_PI / 180.0) + force_effective_strength * cos(direction_radians);
          const double vy =
              -it->speed * sin(it->direction * M_PI / 180.0) - force_effective_strength * sin(direction_radians);
          it->speed = sqrt(vx * vx + vy * vy);
...
void particle_system::create_particles(double x, double y, particle_type* pt, int number, bool use_color,
                                       int given_color) {
  if (number > 0) {
  ...
  for (int i = 0; i < number; i++) {
    ...
    // Life and death.
    pi.life_current =
        pt->life_min == pt->life_max ? pt->life_min : pt->life_min + rand() % (pt->life_max - pt->life_min);
...
void particle_system::set_emitter_region(int em_id, double xmin, double xmax, double ymin, double ymax, ps_shape shape,
