...
void initialize_particle_bridge() {
  ...
  if (major_version < 3) {
    ...
    std::cerr << "Error: OpenGL 3.0 or more must be supported, but a smaller version number was found: "
  ...
  const char* fragment_source =  // TODO: Use raw literals from C++11 to write the shader instead.
      ...
      "  out_color = vec4(transfer_color.x*out_color.x, transfer_color.y*out_color.y, transfer_color.z*out_color.z, "
      "transfer_color.w*out_color.w);\n"
      "}";
  ...
  if (pi_list.size() > 0) {
    ...
    for (unsigned int i = 0; i < pi_list_size; i++) {
      ...
      if (pi.pt->alive) {
        ...
        y = pi.y;
        double size = std::max(
            0.0, pi.size + pt->size_wiggle * particle_system::get_wiggle_result(pi.size_wiggle_offset, wiggle));
        xscale = pt->xscale * size;
        yscale = pt->yscale * size;
        double rot_degrees =
            pi.angle + pt->ang_wiggle * particle_system::get_wiggle_result(pi.ang_wiggle_offset, wiggle);
        if (pt->ang_relative) {
      ...
      const double w = width * xscale, h = height * yscale, wsinrot = w * sin(rot), wcosrot = w * cos(rot);
    ...
    for (unsigned i = 0; i < switch_offset_instcount.size(); i++) {
      ...
      glDrawElements(GL_TRIANGLES, switch_count * 6, GL_UNSIGNED_INT,
                     (GLvoid*)(0 + switch_offset * 6 * sizeof(GLuint)));  // 6 vertices per particle instance.
    }
