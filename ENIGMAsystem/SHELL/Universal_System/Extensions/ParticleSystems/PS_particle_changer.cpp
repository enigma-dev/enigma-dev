...
void particle_changer::clear_particle_changer() { initialize(); }
...
void particle_changer::set_change_kind(ps_change ps_change_kind) { change_kind = ps_change_kind; }
...
void part_changer_destroy_all(int ps_id) {
  particle_system* p_s = enigma::get_particlesystem(ps_id);
  if (p_s != NULL) {
    for (std::map<int, particle_changer*>::iterator it = p_s->id_to_changer.begin(); it != p_s->id_to_changer.end();
         it++) {
      delete (*it).second;
    }
