...
void particle_emitter::clear_particle_emitter() { initialize(); }
void particle_emitter::set_region(double p_xmin, double p_xmax, double p_ymin, double p_ymax, ps_shape p_shape,
                                  ps_distr p_distribution) {
...
void part_emitter_destroy_all(int ps_id) {
  particle_system* p_s = enigma::get_particlesystem(ps_id);
  if (p_s != NULL) {
    for (std::map<int, particle_emitter*>::iterator it = p_s->id_to_emitter.begin(); it != p_s->id_to_emitter.end();
         it++) {
      delete (*it).second;
    }
...
void part_emitter_region(int ps_id, int em_id, double xmin, double xmax, double ymin, double ymax, int shape,
                         int distribution) {
  particle_system* p_s = enigma::get_particlesystem(ps_id);
  if (p_s != NULL) {
    p_s->set_emitter_region(em_id, xmin, xmax, ymin, ymax, enigma::get_ps_shape(shape),
                            enigma::get_ps_distr(distribution));
  }
}
