...
static void internal_draw_particlesystems(double high, double low) {
  ...
  for (std::map<double, particle_depth_layer>::iterator ne_it = negated_particle_depths.lower_bound(-high);
       ne_it != ne_end && ne_it != ne_upper_it; ne_it++) {
    std::set<int>::iterator ps_ids_end = (*ne_it).second.particlesystem_ids.end();
    for (std::set<int>::iterator ps_ids_it = (*ne_it).second.particlesystem_ids.begin(); ps_ids_it != ps_ids_end;
         ps_ids_it++) {
...
static inline void internal_clear_effects() { effect_clear(); }
