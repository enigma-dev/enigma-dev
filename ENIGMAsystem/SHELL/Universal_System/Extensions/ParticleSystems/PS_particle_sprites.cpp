...
static inline double angle_difference(double dir1, double dir2) {
  return fmod((fmod((dir1 - dir2), 2 * M_PI) + 3 * M_PI), 2 * M_PI) - M_PI;
}
long random_seed = 0;
...
class V2 {
 public:
  ...
  double dotproduct(const V2 &other) const { return x * other.x + y * other.y; }
  double crossproduct(const V2 &other) const { return x * other.y - y * other.x; }
  V2 add(const V2 &other) const { return V2(x + other.x, y + other.y); }
  V2 minus(const V2 &other) const { return V2(x - other.x, y - other.y); }
  double norm() const { return sqrt(x * x + y * y); }
  V2 unit_vector() const {
    const double nor = norm();
    return V2(x / nor, y / nor);
  }
  V2 multiply(double value) const { return V2(value * x, value * y); }
};

// Assumes fullwidth == fullheight == 2^n for some n>0.
static void create_particle_sprite(unsigned width, unsigned height, unsigned fullwidth, unsigned fullheight,
                                   char *imgpxdata, pt_shape pt_sh) {
...
static void generate_star() {
  ...
  V2 ss[5];
  for (int i = 0; i < 5; i++) {
    ss[i] = V2(int(center_x + rad * cos(i * 2 * M_PI / 5.0 - M_PI / 2)),
...
static void generate_snow() {
  ...
  for (int i = 0; i < 6; i++) {
    ss[i] =
        V2(center_x + radius * cos(i * M_PI * 2 / 6 + M_PI / 2), center_y - radius * sin(i * M_PI * 2 / 6 + M_PI / 2));
  }
...
int get_particle_actual_sprite(pt_shape particle_shape) {
  ...
  sprite_new_empty(sprid, 1, ps->width, ps->height, ps->width / 2.0, ps->height / 2.0, 0, ps->height, 0, ps->width,
                   true, false);
