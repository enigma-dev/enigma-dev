...
static inline double blen(const path_point* p0, const path_point* p1, const path_point* p2) {
  ...
  double x1 = p0->x, y1 = p0->y, x2 = p1->x, y2 = p1->y, x3 = p2->x, y3 = p2->y, length = 0, lx = 0.5 * (x1 + x2),
         ly = 0.5 * (y1 + y2), x, y;
...
path::path(unsigned pathid, bool smth, bool close, int prec, unsigned pointcount)
  ...
  if (enigma::path_idmax <= pathid) enigma::path_idmax = pathid + 1;
}
  ...
  for (size_t i = 0; i < pc; i++) {
    const double len =
        (pth->smooth)
            ...
            blen(i == 0 ? &start : &pth->pointarray[i - 1], &pth->pointarray[i],
                 i + 1 == pc ? &end : &pth->pointarray[i + 1])
...
inline ppi_t path_point_iterator_at(path* pth, cs_scalar position) { return --pth->pointoffset.upper_bound(position); }
inline int path_point_index_at(path* pth, cs_scalar position) { return path_point_iterator_at(pth, position)->second; }
...
void paths_init() {
  ...
  for (unsigned i = 0; i < enigma::path_idmax; i++) pathstructarray[i] = NULL;
}
}  // namespace enigma
