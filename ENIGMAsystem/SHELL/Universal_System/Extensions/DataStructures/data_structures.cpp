...
template <typename t>
bool tequal(t v1, t v2) {
  return v1 == v2;
}
template <>
bool tequal(float v1, float v2) {
  return fequal(v1, v2);
}
template <>
bool tequal(double v1, double v2) {
  return fequal(v1, v2);
}
...
class grid {
 ...
 public:
  ...
  grid(const unsigned int w, const unsigned int h) {
    ygrid = h;
    xgrid = w;
    grid_array = new t[w * h];
  }
  ~grid() {}

  void destroy() { delete[] grid_array; }
  void clear(const t val) {
    for (unsigned i = 0; i < ygrid; i++)
      for (unsigned ii = 0; ii < xgrid; ii++) grid_array[i * xgrid + ii] = val;
  }
  void resize(unsigned w, unsigned h) {
    grid<variant> temp(w, h);
    const unsigned int wm = minv(xgrid, w), hm = minv(ygrid, h);
    for (unsigned i = 0; i < hm; i++)
      for (unsigned ii = 0; ii < wm; ii++) temp.grid_array[i * w + ii] = grid_array[i * xgrid + ii];
    delete[] grid_array;
    (*this) = temp;
  }
  void copy(const grid& copy_id) {
    ...
    ygrid = copy_id.ygrid;
    for (unsigned i = 0; i < ygrid; i++)
      for (unsigned ii = 0; ii < xgrid; ii++) grid_array[i * xgrid + ii] = copy_id.grid_array[i * copy_id.xgrid + ii];
  }
  unsigned int width() { return xgrid; }
  unsigned int height() { return ygrid; }
  void insert(const unsigned int x, const unsigned int y, const t val) {
    if (x < xgrid && y < ygrid) grid_array[y * xgrid + x] = val;
  }
  ...
  void insert_region(const unsigned int x1, const unsigned int y1, unsigned int x2, const unsigned int y2,
                     const t val) {
    const int tx1 = minv(x1, x2), ty1 = minv(y1, y2), tx2 = maxv(x1, x2), ty2 = maxv(y1, y2), xd = xgrid - tx1,
              yd = ygrid - ty1;
    if (xd > 0 && yd > 0) {
      const int px1 = maxv(tx1, 0), py1 = maxv(ty1, 0), px2 = minv(tx2 + 1, (int)xgrid),
                py2 = minv(ty2 + 1, (int)ygrid);
      for (int i = py1; i < py2; i++)
        for (int ii = px1; ii < px2; ii++) grid_array[i * xgrid + ii] = val;
    }
  }
  ...
  void multiply_region(const unsigned int x1, const unsigned int y1, unsigned int x2, const unsigned int y2,
                       const double val) {
    const int tx1 = minv(x1, x2), ty1 = minv(y1, y2), tx2 = maxv(x1, x2), ty2 = maxv(y1, y2), xd = xgrid - tx1,
              yd = ygrid - ty1;
    if (xd > 0 && yd > 0) {
    ...
    if (tx2 >= 0 && ty2 >= 0 && tx1 < int(xgrid) && ty1 < int(ygrid)) {
      ...
      for (int i = py1; i < py2; i++)
        for (int ii = px1; ii < px2; ii++)
          if ((x - ii) * (x - ii) + (y - i) * (y - i) <= rr) grid_array[i * xgrid + ii] = val;
    }
  }
  ...
  void insert_grid_region(const grid& source_id, const unsigned int sx1, const unsigned int sy1, const unsigned int sx2,
                          const unsigned int sy2, const unsigned int x, const unsigned int y) {
    if (x < xgrid && y < ygrid) {
      const int tx1 = minv(sx1, sx2), ty1 = minv(sy1, sy2), tx2 = maxv(sx1, sx2), ty2 = maxv(sy1, sy2),
                xd = source_id.xgrid - tx1, yd = source_id.ygrid - ty1;
      if (xd > 0 && yd > 0) {
        const int upx = minv(tx2 - tx1 + 1, minv(int(xgrid - x), xd)),
                  upy = minv(ty2 - ty1 + 1, minv(int(ygrid - y), yd));
        for (int i = 0; i < upy; i++)
  ...
  void add_grid_region(const grid& source_id, const unsigned int sx1, const unsigned int sy1, const unsigned int sx2,
                       const unsigned int sy2, const unsigned int x, const unsigned int y) {
    if (x < xgrid && y < ygrid) {
      const int tx1 = minv(sx1, sx2), ty1 = minv(sy1, sy2), tx2 = maxv(sx1, sx2), ty2 = maxv(sy1, sy2),
                xd = source_id.xgrid - tx1, yd = source_id.ygrid - ty1;
  ...
  void multiply_grid_region(const grid& source_id, const unsigned int sx1, const unsigned int sy1,
                            const unsigned int sx2, const unsigned int sy2, const unsigned int x,
  ...
  t find(unsigned int x, unsigned int y) { return (grid_array[y * xgrid + x]); }
  t find_region_sum(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2) {
    ...
    if (xd > 0 && yd > 0) {
      ...
      variant sum = 0;
      for (int i = py1; i < py2; i++)
        for (int ii = px1; ii < px2; ii++) sum += grid_array[i * xgrid + ii];
      return sum;
    }
    return t();
  }
    ...
    if (xd > 0 && yd > 0) {
      ...
      for (int i = py1; i < py2; i++)
        for (int ii = px1; ii < px2; ii++) {
          ...
          if (val_check > max_check) max_check = val_check;
        }
      return max_check;
    }
    return t();
  ...
  t find_region_min(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2) {
    ...
    if (xd > 0 && yd > 0) {
      ...
      for (int i = py1; i < py2; i++)
        for (int ii = px1; ii < px2; ii++) {
          val_check = grid_array[i * xgrid + ii];
          if (val_check < min_check) min_check = val_check;
        }
      return min_check;
    }
    return t();
  ...
  t find_disk_sum(const double x, const double y, const double r) {
    ...
    if (tx2 >= 0 && ty2 >= 0 && tx1 < int(xgrid) && ty1 < int(ygrid)) {
      ...
      for (int i = py1; i < py2; i++)
        for (int ii = px1; ii < px2; ii++)
          if ((x - ii) * (x - ii) + (y - i) * (y - i) <= rr) sum += grid_array[i * xgrid + ii];
      return sum;
    }
    return t();
  }
  ...
  bool value_region_exists(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, const t val) {
    ...
    if (xd > 0 && yd > 0) {
      ...
      for (int i = py1; i < py2; i++)
        for (int ii = px1; ii < px2; ii++)
          if (tequal(grid_array[i * xgrid + ii], val)) return true;
    }
    return false;
  }
  int value_region_x(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, const t val) {
    ...
    if (xd > 0 && yd > 0) {
      ...
      for (int i = py1; i < py2; i++)
        for (int ii = px1; ii < px2; ii++)
          if (tequal(grid_array[i * xgrid + ii], val)) return ii;
    }
    return 0;
  }
  int value_region_y(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, const t val) {
    ...
    if (xd > 0 && yd > 0) {
      ...
      for (int i = py1; i < py2; i++)
        for (int ii = px1; ii < px2; ii++)
          if (tequal(grid_array[i * xgrid + ii], val)) return i;
    }
    return 0;
  }
  ...
  void shuffle() { random_shuffle(grid_array, grid_array + (xgrid * ygrid - 1)); }
};
...
unsigned int ds_grid_create(const unsigned int w, const unsigned int h) {
  ...
  pair<map<unsigned int, grid<variant> >::iterator, bool> ins =
...
void ds_grid_resize(const unsigned int id, const unsigned int w, const unsigned int h) { ds_grids[id].resize(w, h); }
...
variant ds_grid_get_sum(const unsigned int id, const unsigned int x1, const unsigned int y1, const unsigned int x2,
  ...
  return (((x1 < ds_grids[id].width() || x2 < ds_grids[id].width()) &&
           (y1 < ds_grids[id].height() || y2 < ds_grids[id].height()))
              ? ds_grids[id].find_region_sum(x1, y1, x2, y2)
              : variant());
}
...
variant ds_grid_get_max(const unsigned int id, const unsigned int x1, const unsigned int y1, const unsigned int x2,
  ...
  return (((x1 < ds_grids[id].width() || x2 < ds_grids[id].width()) &&
           (y1 < ds_grids[id].height() || y2 < ds_grids[id].height()))
              ? ds_grids[id].find_region_max(x1, y1, x2, y2)
              : variant());
}
...
variant ds_grid_get_min(const unsigned int id, const unsigned int x1, const unsigned int y1, const unsigned int x2,
  ...
  return (((x1 < ds_grids[id].width() || x2 < ds_grids[id].width()) &&
           (y1 < ds_grids[id].height() || y2 < ds_grids[id].height()))
              ? ds_grids[id].find_region_min(x1, y1, x2, y2)
              : variant());
}
...
variant ds_grid_get_mean(const unsigned int id, const unsigned int x1, const unsigned int y1, const unsigned int x2,
  ...
  return (((x1 < ds_grids[id].width() || x2 < ds_grids[id].width()) &&
           (y1 < ds_grids[id].height() || y2 < ds_grids[id].height()))
              ? ds_grids[id].find_region_mean(x1, y1, x2, y2)
              : variant());
}
...
bool ds_grid_value_exists(const unsigned int id, const unsigned int x1, const unsigned int y1, const unsigned int x2,
  ...
  return (((x1 < ds_grids[id].width() || x2 < ds_grids[id].width()) &&
           (y1 < ds_grids[id].height() || y2 < ds_grids[id].height()))
              ? ds_grids[id].value_region_exists(x1, y1, x2, y2, val)
              : false);
}
...
int ds_grid_value_x(const unsigned int id, const unsigned int x1, const unsigned int y1, const unsigned int x2,
  ...
  return (((x1 < ds_grids[id].width() || x2 < ds_grids[id].width()) &&
           (y1 < ds_grids[id].height() || y2 < ds_grids[id].height()))
              ? ds_grids[id].value_region_x(x1, y1, x2, y2, val)
              : 0);
}
...
int ds_grid_value_y(const unsigned int id, const unsigned int x1, const unsigned int y1, const unsigned int x2,
  ...
  return (((x1 < ds_grids[id].width() || x2 < ds_grids[id].width()) &&
           (y1 < ds_grids[id].height() || y2 < ds_grids[id].height()))
              ? ds_grids[id].value_region_y(x1, y1, x2, y2, val)
              : 0);
}
  ...
  for (unsigned y = 0; y < dsGrid.height(); ++y) {
    for (unsigned x = 0; x < dsGrid.width(); ++x) {
      ...
      if (vari.type == ty_real) {
        ...
        for (unsigned i = 0; i < sizeof(double); ++i) ss << b[i];
      } else {
        ss.width(4);
        ss << vari.sval.length();
        ss.width(1);
        for (size_t j = 0; j < vari.sval.length(); ++j) ss << vari.sval[j];
      }
    }
  }
...
std::string ds_map_write(const unsigned int id) {
  ...
  while (it != dsMap.end()) {
    ...
    if ((*it).first.type == ty_real) {
      ...
      ss.width(4);
      ss << (*it).first.sval.length();
      ss.width(1);
      for (size_t j = 0; j < (*it).first.sval.length(); ++j) ss << (*it).first.sval[j];
    }
    ...
    if ((*it).second.type == ty_real) {
      ...
      ss.width(4);
      ss << (*it).second.sval.length();
      ss.width(1);
      for (size_t j = 0; j < (*it).second.sval.length(); ++j) ss << (*it).second.sval[j];
    }
...
void ds_list_add(const unsigned int id, const enigma::varargs& values) {
  ...
  for (int i = 0; i < values.argc; ++i) ds_lists[id].push_back(values.get(i));
}
...
std::string ds_list_write(const unsigned int id) {
  ...
  for (size_t i = 0; i < dsList.size(); ++i) {
    ...
    if (dsList[i].type == ty_real) {
      ...
      ss.width(4);
      ss << dsList[i].sval.length();
      ss.width(1);
      for (size_t j = 0; j < dsList[i].sval.length(); ++j) ss << dsList[i].sval[j];
    }
  }
...
unsigned int ds_priority_create() {
  ...
  ds_prioritys.insert(
      pair<unsigned int, multimap<variant, variant> >(ds_prioritys_maxid++, multimap<variant, variant>()));
...
void ds_queue_enqueue(const unsigned int id, const enigma::varargs& values) {
  ...
  for (int i = 0; i < values.argc; ++i) ds_queues[id].push_back(values.get(i));
}
...
std::string ds_queue_write(const unsigned int id) {
  ...
  for (size_t i = 0; i < dsQueue.size(); ++i) {
    ...
    if (dsQueue[i].type == ty_real) {
      ...
      ss.width(4);
      ss << dsQueue[i].sval.length();
      ss.width(1);
      for (size_t j = 0; j < dsQueue[i].sval.length(); ++j) ss << dsQueue[i].sval[j];
    }
  }
...
void ds_stack_push(const unsigned int id, const enigma::varargs& values) {
  ...
  for (int i = 0; i < values.argc; ++i) ds_stacks[id].push_front(values.get(i));
}
...
std::string ds_stack_write(const unsigned int id) {
  ...
  for (size_t i = 0; i < dsStack.size(); ++i) {
    ...
    if (dsStack[i].type == ty_real) {
      ...
      ss.width(4);
      ss << dsStack[i].sval.length();
      ss.width(1);
      for (size_t j = 0; j < dsStack[i].sval.length(); ++j) ss << dsStack[i].sval[j];
    }
  }
