...
void seek(gme_callback *userdata, float position) { handle_error(gme_seek(userdata->emu, position * 1000)); }
...
int sound_add_from_gme(string filename, int track) {
  ...
  return enigma::sound_add_from_stream(sid, (size_t(*)(void *, void *, size_t))callback, (void (*)(void *, float))seek,
                                       (void (*)(void *))cleanup, (void *)userdata)
             ? -1
             : sid;
}
}  // namespace enigma_user
