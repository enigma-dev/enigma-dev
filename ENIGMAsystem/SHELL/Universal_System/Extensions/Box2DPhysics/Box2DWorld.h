...
extern vector<B2DBody*> b2dbodies;
  ...
  #define get_worldr(w,id,r) \
    if (unsigned(id) >= b2dworlds.size() || id < 0) { \
      DEBUG_MESSAGE("Cannot access Box2D physics world with id " + toString(id), MESSAGE_TYPE::M_ERROR); \
      return r; \
  ...
  #define get_bodyr(b,id,r) \
    if (unsigned(id) >= b2dbodies.size() || id < 0) { \
      DEBUG_MESSAGE("Cannot access Box2D physics body with id " + toString(id), MESSAGE_TYPE::M_ERROR); \
      return r; \
