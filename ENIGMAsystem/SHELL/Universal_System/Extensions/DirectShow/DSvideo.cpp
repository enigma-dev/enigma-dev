...
bool video_exists(int id) { return (enigma::videoStructs[id] != NULL); }
...
void video_set_destination_region(int id, long x, long y, long w, long h) {
  ...
  videoStruct->pGraph->QueryInterface(IID_IBasicVideo, (LPVOID *)&pBasicVideo);
...
int video_get_frame_rate(int id) {
  get_videor(videoStruct, id, -1);
  IQualProp *pQualProp;
  videoStruct->pGraph->QueryInterface(IID_IQualProp, (LPVOID *)&pQualProp);
