...
namespace gui {
	...
	Button::Button(){
	  style_id = guiElements.gui_style_button; //Default style
	...
	void Button::update_text_pos(int state){
	  //gui_styles[style_id].update_text_pos(box, state);
	}
} //namespace gui
} //namespace enigma
...
namespace enigma_user
{
	int gui_button_create(){
		if (gui_bound_skin == -1){ //Add default one
			guiElements.gui_elements.emplace(std::piecewise_construct, std::forward_as_tuple(guiElements.gui_elements_maxid), std::forward_as_tuple(Button(), guiElements.gui_elements_maxid));
		}else{
      get_data_elementv(ski,Skin,GUI_TYPE::SKIN,gui_bound_skin,-1);
      get_elementv(but,Button,GUI_TYPE::BUTTON,ski.button_style,-1);
      guiElements.gui_elements.emplace(std::piecewise_construct, std::forward_as_tuple(guiElements.gui_elements_maxid), std::forward_as_tuple(but, guiElements.gui_elements_maxid));
		}
		Button &b = guiElements.gui_elements[guiElements.gui_elements_maxid];
		b.visible = true;
		b.id = guiElements.gui_elements_maxid;
  ...
  int gui_button_duplicate(int id){
    get_elementv(but,Button,GUI_TYPE::BUTTON,id,-1);
    guiElements.gui_elements.emplace(std::piecewise_construct, std::forward_as_tuple(guiElements.gui_elements_maxid), std::forward_as_tuple(but, guiElements.gui_elements_maxid));
    ...
    if (but.parent_id != -1){
      ::enigma_user::gui_window_remove_button(but.parent_id, id);
	  }
  ...
  void gui_button_set_style(int id, int style_id){
    ...
    but.style_id = (style_id != -1? style_id : guiElements.gui_style_button);
  }
  ...
  int gui_button_get_style(int id){
    ...
    get_elementv(but,Button,GUI_TYPE::BUTTON,id,false);
		return but.active;
	}
  ...
  string gui_button_get_text(int id){
    get_elementv(but,Button,GUI_TYPE::BUTTON,id,"");
    return but.text;
  }
  ...
  ///Drawers
    ...
    font_style psty = get_current_draw_state();
      ...
      if (b.second.type == GUI_TYPE::BUTTON){
        get_element(but,Button,GUI_TYPE::BUTTON,b.first);
  ...
  void gui_button_add_button(int id, int bid){
    get_element(ele,Button,GUI_TYPE::BUTTON,id);
  ...
  int gui_button_get_button_count(int id){
    get_elementv(ele,Button,GUI_TYPE::BUTTON,id,-1);
