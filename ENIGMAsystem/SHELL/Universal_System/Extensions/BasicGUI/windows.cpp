...
namespace enigma_user
{
  int gui_window_create(){
    if (gui_bound_skin == -1){ //Add default one
      guiElements.gui_elements.emplace(std::piecewise_construct, std::forward_as_tuple(guiElements.gui_elements_maxid), std::forward_as_tuple(Window(), guiElements.gui_elements_maxid));
    }else{
      get_data_elementv(ski,Skin,GUI_TYPE::SKIN,gui_bound_skin,-1);
      get_elementv(win,Window,GUI_TYPE::WINDOW,ski.window_style,-1);
      guiElements.gui_elements.emplace(std::piecewise_construct, std::forward_as_tuple(guiElements.gui_elements_maxid), std::forward_as_tuple(win, guiElements.gui_elements_maxid));
    }
  ...
  bool gui_windows_update(){
    ...
    for (int ind = gui_window_order.size()-1; ind >= 0; --ind){
      ...
      if (win.visible == true && win.parent_id == -1){
        ...
        if (windowStopPropagation == true && window_click == false) { window_click = true; window_swap_id = ind; }
        win.update();
  ...
  bool gui_windows_group_update(int gid, bool continueProp){
    if (continueProp == false){ windowStopPropagation = false; }
    if (gui_window_order.size() == 0) return false;
