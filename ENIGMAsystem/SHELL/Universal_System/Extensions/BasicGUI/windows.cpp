...
int gui_window_create() {
  if (gui::gui_bound_skin == -1) {  //Add default one
    gui::gui_elements.emplace(std::piecewise_construct, std::forward_as_tuple(gui::gui_elements_maxid),
                              std::forward_as_tuple(gui::Window(), gui::gui_elements_maxid));
  } else {
    ...
    gui::gui_elements.emplace(std::piecewise_construct, std::forward_as_tuple(gui::gui_elements_maxid),
                              std::forward_as_tuple(win, gui::gui_elements_maxid));
  }
...
void gui_window_destroy(int id) {
  ...
  gui::gui_window_order.erase(std::remove(gui::gui_window_order.begin(), gui::gui_window_order.end(), id),
                              gui::gui_window_order.end());
}
...
void gui_continue_propagation() { gui::windowStopPropagation = false; }
}  // namespace enigma_user
