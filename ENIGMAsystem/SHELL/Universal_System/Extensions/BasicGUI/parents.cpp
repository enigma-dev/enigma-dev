...
void Parent::update_children(gs_scalar ox, gs_scalar oy) {
  ...
  for (const auto &x : child_elements) {
    ...
    switch (gui::gui_elements[x].type) {
      case gui::GUI_TYPE::BUTTON: {
        gui::Button &element = gui::gui_elements[x];
        if (element.visible == true) element.update(ox, oy);
        break;
      }
      case gui::GUI_TYPE::TEXTBOX: {
        gui::Textbox &element = gui::gui_elements[x];
        if (element.visible == true) element.update(ox, oy);
        break;
      }
      case gui::GUI_TYPE::TOGGLE: {
        gui::Toggle &element = gui::gui_elements[x];
        if (element.visible == true) element.update(ox, oy);
        break;
      }
      case gui::GUI_TYPE::SCROLLBAR: {
        gui::Scrollbar &element = gui::gui_elements[x];
        if (element.visible == true) element.update(ox, oy);
        break;
      }
      case gui::GUI_TYPE::SLIDER: {
        gui::Slider &element = gui::gui_elements[x];
        if (element.visible == true) element.update(ox, oy);
        break;
      }
      case gui::GUI_TYPE::WINDOW: {
        gui::Window &element = gui::gui_elements[x];
        if (element.visible == true) element.update(ox, oy);
        break;
      }
...
void Parent::draw_children(gs_scalar ox, gs_scalar oy) {
  ...
  for (const auto &x : child_elements) {
    switch (gui::gui_elements[x].type) {
      ...
      case gui::GUI_TYPE::LABEL: {
        gui::Label &element = gui::gui_elements[x];
        if (element.visible == true) element.draw(ox, oy);
        break;
      }
      case gui::GUI_TYPE::SCROLLBAR: {
...
void Parent::textbox_add(int id) {
  ...
  if (tex.parent_id != -1) {
    SHOW_ERROR("BasicGUI: Textbox with ID " + to_string(id) + " already has a parent with ID " +
               to_string(tex.parent_id) + "! Parenting not done!\n");
  }
  child_elements.push_back(id);
