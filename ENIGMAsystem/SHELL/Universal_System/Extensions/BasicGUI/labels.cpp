...
void Label::draw(gs_scalar ox, gs_scalar oy) {
  ...
  if (sty.sprites[enigma_user::gui_state_default] != -1) {
    if (sty.border.zero == true) {
      enigma_user::draw_sprite_stretched(sty.sprites[enigma_user::gui_state_default], -1, ox + box.x, oy + box.y, box.w,
                                         box.h, sty.sprite_styles[enigma_user::gui_state_default].color,
                                         sty.sprite_styles[enigma_user::gui_state_default].alpha);
    } else {
      enigma_user::draw_sprite_padded(sty.sprites[enigma_user::gui_state_default], -1, sty.border.left, sty.border.top,
                                      sty.border.right, sty.border.bottom, ox + box.x, oy + box.y, ox + box.x + box.w,
                                      oy + box.y + box.h, sty.sprite_styles[enigma_user::gui_state_default].color,
                                      sty.sprite_styles[enigma_user::gui_state_default].alpha);
    }
  }
...
int gui_label_create() {
  if (gui::gui_bound_skin == -1) {  //Add default one
    gui::gui_elements.emplace(std::piecewise_construct, std::forward_as_tuple(gui::gui_elements_maxid),
                              std::forward_as_tuple(gui::Label(), gui::gui_elements_maxid));
  } else {
    ...
    gui::gui_elements.emplace(std::piecewise_construct, std::forward_as_tuple(gui::gui_elements_maxid),
                              std::forward_as_tuple(lab, gui::gui_elements_maxid));
  }
