...
bool mp_potential_step_object(const double x, const double y, const double stepsize, const int object,
  ...
  if (fequal(inst->x, x) && fequal(inst->y, y)) return true;
  ...
  if (abs((dir - inst->direction + 540) % (variant)360 - 180) <= mp_potential::maxrot) {
    if (hypot(x - inst->x, y - inst->y) <= stepsize) {
      ...
      if (mp_potential::onspot) {
        inst->direction += min(mp_potential::maxrot,
                               max(-mp_potential::maxrot, (goaldir - inst->direction + 540) % (variant)360 - 180));
      }
      return false;
    }
    xstep = cos(dir * M_PI / 180) * stepsize;
    ystep = -sin(dir * M_PI / 180) * stepsize;
    if (solid_only ? (place_free(inst->x + mp_potential::ahead * xstep, y + mp_potential::ahead * ystep) &&
                      place_free(inst->x + xstep, inst->y + ystep))
                   : (!place_meeting(inst->x + mp_potential::ahead * xstep, y + mp_potential::ahead * ystep, object) &&
                      !place_meeting(inst->x + xstep, inst->y + ystep, object))) {
      inst->direction = dir;
      inst->x += xstep;
      inst->y += ystep;
      return true;
  ...
  for (int i = mp_potential::rotstep; i < 180; i += mp_potential::rotstep) {
    ...
    if (abs((dir - inst->direction + 540) % (variant)360 - 180) <= mp_potential::maxrot) {
      ...
      ystep = -sin(dir * M_PI / 180) * stepsize;
      if (solid_only
              ? (place_free(inst->x + mp_potential::ahead * xstep, y + mp_potential::ahead * ystep) &&
                 place_free(inst->x + xstep, inst->y + ystep))
              : (!place_meeting(inst->x + mp_potential::ahead * xstep, y + mp_potential::ahead * ystep, object) &&
                 !place_meeting(inst->x + xstep, inst->y + ystep, object))) {
        inst->direction = dir;
    ...
    if (abs((dir - inst->direction + 540) % (variant)360 - 180) <= mp_potential::maxrot) {
      ...
      ystep = -sin(dir * M_PI / 180) * stepsize;
      if (solid_only
              ? (place_free(inst->x + mp_potential::ahead * xstep, inst->y + mp_potential::ahead * ystep) &&
                 place_free(inst->x + xstep, inst->y + ystep))
              : (!place_meeting(inst->x + mp_potential::ahead * xstep, inst->y + mp_potential::ahead * ystep, object) &&
                 !place_meeting(inst->x + xstep, inst->y + ystep, object))) {
        inst->direction = dir;
        inst->x += xstep;
        inst->y += ystep;
...
bool mp_potential_path_object(int path, const double x, const double y, const double stepsize, double factor,
  ...
  while (path_get_length(path) < max_dist) {
    ...
    if (abs((dir - pathpdir + 540) % (variant)360 - 180) <= mp_potential::maxrot) {
      ...
      ystep = -sin(dir * M_PI / 180) * stepsize;
      if (solid_only
              ? (place_free(pathpx + mp_potential::ahead * xstep, pathpy + mp_potential::ahead * ystep) &&
                 place_free(pathpx + xstep, pathpy + ystep))
              : (!place_meeting(pathpx + mp_potential::ahead * xstep, pathpy + mp_potential::ahead * ystep, object) &&
                 !place_meeting(pathpx + xstep, pathpy + ystep, object))) {
        pathpdir = dir;
        pathpx += xstep;
        pathpy += ystep;
        path_add_point(path, pathpx, pathpy, 100);
    ...
    if (!pathfound) {
      ...
      for (int i = mp_potential::rotstep; i < 180; i += mp_potential::rotstep) {
        ...
        if (abs((dir - pathpdir + 540) % (variant)360 - 180) <= mp_potential::maxrot) {
          xstep = cos(dir * M_PI / 180) * stepsize;
          ystep = -sin(dir * M_PI / 180) * stepsize;
          if (solid_only ? (place_free(pathpx + mp_potential::ahead * xstep, pathpy + mp_potential::ahead * ystep) &&
                            place_free(pathpx + xstep, pathpy + ystep))
                         : (!place_meeting(pathpx + mp_potential::ahead * xstep, pathpy + mp_potential::ahead * ystep,
                                           object) &&
                            !place_meeting(pathpx + xstep, pathpy + ystep, object))) {
            pathpdir = dir;
            pathpx += xstep;
...
bool mp_linear_step_object(const double x, const double y, const double stepsize, const int object,
  ...
  if (solid_only ? (place_free(inst->x + xstep, inst->y + ystep))
                 : (!place_meeting(inst->x + xstep, inst->y + ystep, object))) {
    inst->x += xstep;
    inst->y += ystep;
    return true;
...
bool mp_linear_path_object(int path, const double x, const double y, const double stepsize, const int object,
  ...
  while (path_get_length(path) < max_dist) {
    ...
    if (solid_only ? (place_free(pathpx + xstep, pathpy + ystep))
                   : (!place_meeting(pathpx + xstep, pathpy + ystep, object))) {
      pathpx += xstep;
      pathpy += ystep;
      path_add_point(path, pathpx, pathpy, 100);
