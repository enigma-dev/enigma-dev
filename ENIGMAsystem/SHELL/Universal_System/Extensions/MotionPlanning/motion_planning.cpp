...
int collision_rectangle(cs_scalar x1, cs_scalar y1, cs_scalar x2, cs_scalar y2, int obj, bool prec /*ignored*/,
                        bool notme);
}
...
unsigned mp_grid_create(int left, int top, int hcells, int vcells, int cellwidth, int cellheight,
...
void mp_grid_destroy(unsigned id) { delete enigma::gridstructarray[id]; }
...
void mp_grid_copy(unsigned id, unsigned srcid) {
  ...
  for (unsigned int i = 0; i < grid->hcells; i++) {
    for (unsigned int c = 0; c < grid->vcells; c++) {
      if (i > 0) {
        grid->nodearray[i * grid->vcells + c].neighbor_nodes.push_back(
            &grid->nodearray[(i - 1) * grid->vcells + c]);  //left
        if (c > 0)
          grid->nodearray[i * grid->vcells + c].neighbor_nodes.push_back(
              &grid->nodearray[(i - 1) * grid->vcells + c - 1]);  //top-left
        if (c < grid->vcells - 1)
          grid->nodearray[i * grid->vcells + c].neighbor_nodes.push_back(
              &grid->nodearray[(i - 1) * grid->vcells + c + 1]);  //bottom-left
      }
      if (c > 0)
        grid->nodearray[i * grid->vcells + c].neighbor_nodes.push_back(
            &grid->nodearray[i * grid->vcells + c - 1]);  //top
      if (i < grid->hcells - 1) {
        grid->nodearray[i * grid->vcells + c].neighbor_nodes.push_back(
            &grid->nodearray[(i + 1) * grid->vcells + c]);  //right
        if (c > 0)
          grid->nodearray[i * grid->vcells + c].neighbor_nodes.push_back(
              &grid->nodearray[(i + 1) * grid->vcells + c - 1]);  //top-right
        if (c < grid->vcells - 1)
          grid->nodearray[i * grid->vcells + c].neighbor_nodes.push_back(
              &grid->nodearray[(i + 1) * grid->vcells + c + 1]);  //bottom-bottom
      }
      if (c < grid->vcells - 1)
        grid->nodearray[i * grid->vcells + c].neighbor_nodes.push_back(
            &grid->nodearray[i * grid->vcells + c + 1]);  //bottom
    }
  }
}

void mp_grid_clear_all(unsigned id, unsigned cost) {
  for (vector<enigma::node>::iterator it = enigma::gridstructarray[id]->nodearray.begin();
       it != enigma::gridstructarray[id]->nodearray.end(); ++it)
    (*it).cost = cost;
...
void mp_grid_clear_cell(unsigned id, int h, int v, unsigned cost) {
  ...
  if (enigma::gridstructarray[id]->threshold < cost) {
    enigma::gridstructarray[id]->threshold = cost;
  }
}
...
void mp_grid_add_rectangle(unsigned id, double x1, double y1, double x2, double y2, unsigned cost) {
  ...
  x1 -= x, x2 -= x, y1 -= y, y2 -= y;
  unsigned int tx1 = fmax(floor(fmin(x1, x2) / grid->cellwidth), 0),
               tx2 = fmin(ceil((fmin(x1, x2) + fabs(x2 - x1)) / grid->cellwidth), grid->hcells),
               ty1 = fmax(floor(fmin(y1, y2) / grid->cellheight), 0),
               ty2 = fmin(ceil((fmin(y1, y2) + fabs(y2 - y1)) / grid->cellheight), grid->vcells);
  for (unsigned int i = tx1; i < tx2; i++) {
    for (unsigned int c = ty1; c < ty2; c++) {
      if (grid->nodearray[i * grid->vcells + c].cost > max_cost) {
        max_cost = grid->nodearray[i * grid->vcells + c].cost;
      }
...
void mp_grid_add_instances(unsigned id, int obj, bool prec, unsigned cost) {
  ...
  for (unsigned int i = 0; i < grid->hcells; i++) {
    for (unsigned int c = 0; c < grid->vcells; c++) {
      ...
      if (collision_rectangle(x + i * grid->cellwidth, y + c * grid->cellheight, x + (i + 1) * grid->cellwidth,
                              y + (c + 1) * grid->cellheight, obj, prec, false) != -4) {
        grid->nodearray[i * grid->vcells + c].cost = cost;
      }
    }
  }
...
void mp_grid_add_cell(unsigned id, int h, int v, unsigned cost) {
  ...
  if (enigma::gridstructarray[id]->threshold < max_cost) {
    enigma::gridstructarray[id]->threshold = max_cost;
  }
}
...
unsigned mp_grid_get_threshold(unsigned id) { return enigma::gridstructarray[id]->threshold; }

void mp_grid_set_threshold(unsigned id, unsigned value) { enigma::gridstructarray[id]->threshold = value; }

double mp_grid_get_speed_modifier(unsigned id) { return enigma::gridstructarray[id]->speed_modifier; }

void mp_grid_set_speed_modifier(unsigned id, double value) { enigma::gridstructarray[id]->speed_modifier = value; }

bool mp_grid_path(unsigned id, unsigned pathid, double xstart, double ystart, double xgoal, double ygoal,
                  bool allowdiag) {
  enigma::grid *gr = enigma::gridstructarray[id];
  int vc = int(enigma::gridstructarray[id]->vcells), xs = floor((xstart - gr->left) / int(gr->cellwidth)),
      ys = floor((ystart - gr->top) / int(gr->cellheight)), xg = floor((xgoal - gr->left) / int(gr->cellwidth)),
      yg = floor((ygoal - gr->top) / int(gr->cellheight));
  ...
  multimap<unsigned, enigma::node *> nodelist =
      enigma::find_path(id, &gr->nodearray[xs * vc + ys], &gr->nodearray[xg * vc + yg], allowdiag, status);
  ...
  for (it = nodelist.rbegin(); it != nodelist.rend(); it++) {
    point = enigma::path_point(gr->left + ((*it).second->x + 0.5) * gr->cellwidth,
                               gr->top + ((*it).second->y + 0.5) * gr->cellheight,
                               gr->speed_modifier / double((*it).second->cost));
    path->pointarray.push_back(point);
  }
  ...
  if (status == true) {
    ...
    point = enigma::path_point(path->pointarray.back().x, path->pointarray.back().y,
                               gr->speed_modifier / double(gr->nodearray[xg * vc + yg].cost));
    path->pointarray.push_back(point);
  }
...
void mp_grid_draw_neighbours(unsigned int id, unsigned int h, unsigned int v, unsigned int mode) {
  ...
  for (vector<enigma::node *>::iterator it = enigma::gridstructarray[id]->nodearray[h * vc + v].neighbor_nodes.begin();
       it != enigma::gridstructarray[id]->nodearray[h * vc + v].neighbor_nodes.end(); ++it) {
    draw_vertex_color(grid->left + (*it)->x * grid->cellwidth, grid->top + (*it)->y * grid->cellheight, 0x0000FF,
                      (mode == 0 ? 0.5 : 1.0));
    draw_vertex_color(grid->left + ((*it)->x + 1) * grid->cellwidth, grid->top + (*it)->y * grid->cellheight, 0x0000FF,
                      (mode == 0 ? 0.5 : 1.0));
    draw_vertex_color(grid->left + ((*it)->x + 1) * grid->cellwidth, grid->top + ((*it)->y + 1) * grid->cellheight,
                      0x0000FF, (mode == 0 ? 0.5 : 1.0));
    draw_vertex_color(grid->left + (*it)->x * grid->cellwidth, grid->top + ((*it)->y + 1) * grid->cellheight, 0x0000FF,
                      (mode == 0 ? 0.5 : 1.0));
  }
  draw_primitive_end();
  if (mode == 1) {
    ...
    for (vector<enigma::node *>::iterator it =
             enigma::gridstructarray[id]->nodearray[h * vc + v].neighbor_nodes.begin();
         it != enigma::gridstructarray[id]->nodearray[h * vc + v].neighbor_nodes.end(); ++it) {
      draw_text(((*it)->x + 0.5) * grid->cellwidth, ((*it)->y + 0.5) * grid->cellheight,
                (*it)->x * grid->vcells + (*it)->y);
    }
    draw_set_color(tc);
  }
}
...
void mp_grid_draw(unsigned int id, unsigned int mode, unsigned int color_mode) {
  ...
  if (mode) {
    ...
    if (mode == 1) {  //draw cost
      for (unsigned int i = 0; i < grid->hcells; i++) {
        for (unsigned int c = 0; c < grid->vcells; c++) {
          draw_text(x + (i + 0.5) * grid->cellwidth, y + (c + 0.5) * grid->cellheight,
                    grid->nodearray[i * grid->vcells + c].cost);
        }
      }
    } else {  //draw coords
      for (unsigned int i = 0; i < grid->hcells; i++) {
        for (unsigned int c = 0; c < grid->vcells; c++) {
          draw_text(x + (i + 0.5) * grid->cellwidth, y + (c + 0.5) * grid->cellheight,
                    toString(variant(grid->nodearray[i * grid->vcells + c].x)) + ":" +
                        toString(variant(grid->nodearray[i * grid->vcells + c].y)));
        }
      }
    }
    draw_set_color(tc);
