...
extern vector<fixtureInstance*> fixtures;
  ...
  #define get_worldr(w,id,r) \
    if (unsigned(id) >= worlds.size() || id < 0) { \
      DEBUG_MESSAGE("Cannot access GayMaker: Stupido physics world with id " + toString(id), MESSAGE_TYPE::ERROR); \
      return r; \
  ...
  #define get_fixturer(f,id,r) \
    if (unsigned(id) >= fixtures.size() || id < 0) { \
      DEBUG_MESSAGE("Cannot access GayMaker: Stupido physics fixture with id " + toString(id), MESSAGE_TYPE::ERROR); \
      return r; \
