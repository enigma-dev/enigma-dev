...
**/
  ...
  #define get_worldr(w,id,r) \
    if (unsigned(id) >= bulletWorlds.size() || id < 0) { \
      DEBUG_MESSAGE("Cannot access Bullet Dynamics physics world with id " + toString(id), MESSAGE_TYPE::ERROR); \
      return r; \
  ...
  #define get_worldc(w,id, c) \
    if (unsigned(id) >= bulletWorlds.size() || id < 0) { \
      DEBUG_MESSAGE("Cannot cast Bullet Dynamics physics world with id " + toString(id), MESSAGE_TYPE::ERROR); \
      return; \
