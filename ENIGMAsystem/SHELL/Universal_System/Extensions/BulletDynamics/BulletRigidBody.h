...
#define get_body(w, id)                                                                      \
  ...
  BulletBody* w = bulletBodies[id];
#else
#define get_bodyr(w, id, r) BulletBody* w = bulletBodies[id];
#define get_body(w, id) BulletBody* w = bulletBodies[id];
#endif
...
struct BulletBody {
  ...
  BulletBody(int sid, double mass, double ix, double iy, double iz, double friction, double restitution) {
    ...
    shapeid = sid;
    btRigidBody::btRigidBodyConstructionInfo rigidBodyCI(mass, motionstate, bulletShapes[shapeid]->colShape,
                                                         btVector3(ix, iy, iz));
  ...
  double getRoll() { return rigidBody->getOrientation().getAxis().getX(); }

  double getPitch() { return rigidBody->getOrientation().getAxis().getY(); }

  double getYaw() { return rigidBody->getOrientation().getAxis().getZ(); }

  double getAngle() { return rigidBody->getOrientation().getAngle(); }

  double getQuatX() { return rigidBody->getOrientation().getX(); }

  double getQuatY() { return rigidBody->getOrientation().getY(); }

  double getQuatZ() { return rigidBody->getOrientation().getZ(); }

  double getQuatW() { return rigidBody->getOrientation().getW(); }
};
