...
time_t date_current_datetime() { return time(NULL); }
...
int date_get_year(time_t date) { return localtime(&date)->tm_year + 1900; }

int date_get_month(time_t date) { return localtime(&date)->tm_mon + 1; }
...
int date_get_day(time_t date) { return localtime(&date)->tm_mday; }

int date_get_hour(time_t date) { return localtime(&date)->tm_hour; }

int date_get_minute(time_t date) { return localtime(&date)->tm_min; }

int date_get_second(time_t date) { return localtime(&date)->tm_sec; }

int date_get_weekday(time_t date) { return localtime(&date)->tm_wday + 1; }

int date_get_day_of_year(time_t date) { return localtime(&date)->tm_yday + 1; }
...
int date_days_in_month(time_t date) {
  ...
  if ((timeinfo->tm_year + 1900) % 400 == 0 ||
      ((timeinfo->tm_year + 1900) % 100 != 0 && (timeinfo->tm_year + 1900) % 4 == 0))
...
bool date_is_today(time_t date) {
  struct tm* timeinfo;
  timeinfo = localtime(&date);
  int year = timeinfo->tm_year, month = timeinfo->tm_mon, day = timeinfo->tm_mday;
  time_t rawtime;
  time(&rawtime);
  timeinfo = localtime(&rawtime);
...
string date_datetime_string(time_t date) {
  char buffer[80];
  strftime(buffer, 80, "%x %X", localtime(&date));
