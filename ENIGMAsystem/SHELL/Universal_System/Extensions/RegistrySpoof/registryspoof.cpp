...
void save_ini() { regIni.saveToFile(RegistryFilename); }

}  // namespace

namespace enigma_user {

void registry_write_string(std::string name, std::string str) { registry_write_string_ext(GameKey, name, str); }
...
void registry_write_real(std::string name, int x) { registry_write_real_ext(GameKey, name, x); }
...
std::string registry_read_string(std::string name) { return registry_read_string_ext(GameKey, name); }
std::string registry_read_string_ext(std::string key, std::string name) {
  open_ini();
  return regIni.read(CurrRoot + "\\" + key, name, "");
}

int registry_read_real(std::string name) { return registry_read_real_ext(GameKey, name); }
int registry_read_real_ext(std::string key, std::string name) {
  open_ini();
  return regIni.read(CurrRoot + "\\" + key, name, 0);
}

bool registry_exists(std::string name) { return registry_exists_ext(GameKey, name); }
