...
namespace enigma_user
{
	...
	void ini_open(std::string filename)
	{
		...
		if (filename.find_first_of(InvalidFilenameChars)!=std::string::npos) {
			DEBUG_MESSAGE("IniFileSystem - cannot open new ini file; filename contains invalid characters.", MESSAGE_TYPE::ERROR);
			return;
		}
	...
	void ini_close()
	{
		...
		if (!currIniFile.empty()) {
			...
			if (!currIni.saveToFile(currIniFile)) {
				DEBUG_MESSAGE("IniFileSystem - could not save ini file to output; perhaps it's in a protected location?", MESSAGE_TYPE::ERROR);
			}
	...
	void ini_key_delete(std::string section, std::string key)
	{
		...
		if (currIniFile.empty()) {
			DEBUG_MESSAGE("IniFileSystem - cannot delete key, as there is no ini file currently open.", MESSAGE_TYPE::ERROR);
		} else {
	...
	void ini_section_delete(std::string section)
	{
		//This won't work if the file isn't open.
		if (currIniFile.empty()) {
			DEBUG_MESSAGE("IniFileSystem - cannot delete section, as there is no ini file currently open.", MESSAGE_TYPE::ERROR);
		} else {
	...
	bool ini_key_exists(std::string section, std::string key)
	{
		//This won't work if the file isn't open.
		if (currIniFile.empty()) {
			DEBUG_MESSAGE("IniFileSystem - cannot check key, as there is no ini file currently open.", MESSAGE_TYPE::ERROR);
			return false;
		} else {
	...
	bool ini_section_exists(std::string section)
	{
		//This won't work if the file isn't open.
		if (currIniFile.empty()) {
			DEBUG_MESSAGE("IniFileSystem - cannot check section, as there is no ini file currently open.", MESSAGE_TYPE::ERROR);
			return false;
		} else {
	...
	void ini_write_real(string section, string key, float value)
	{
		...
		if (currIniFile.empty()) {
			DEBUG_MESSAGE("IniFileSystem - cannot write real, as there is no ini file currently open.", MESSAGE_TYPE::ERROR);
		} else {
	...
	void ini_write_string(string section, string key, string value)
	{
		//GM will err out when trying to read/write an unopened ini file.
		if (currIniFile.empty()) {
			DEBUG_MESSAGE("IniFileSystem - cannot write string, as there is no ini file currently open.", MESSAGE_TYPE::ERROR);
		} else {
			currIni.write(section, key, value);
		}
	}
	...
	string ini_read_string(string section, string key, string def)
	{
		//GM will err out when trying to read/write an unopened ini file.
		if (currIniFile.empty()) {
			DEBUG_MESSAGE("IniFileSystem - cannot read string, as there is no ini file currently open.", MESSAGE_TYPE::ERROR);
			return def;
		} else {
	...
	float ini_read_real(string section, string key, float def)
	{
		//GM will err out when trying to read/write an unopened ini file.
		if (currIniFile.empty()) {
			DEBUG_MESSAGE("IniFileSystem - cannot read real, as there is no ini file currently open.", MESSAGE_TYPE::ERROR);
			return def;
		} else {
			return currIni.read(section, key, def);
		}
