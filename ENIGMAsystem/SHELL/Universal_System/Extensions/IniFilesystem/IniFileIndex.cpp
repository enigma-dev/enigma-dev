...
std::string optional_quote(const std::string& str, const char commentChar) {
  if (!str.empty() && (str[0] == ' ' || str[0] == '\t' || str[str.size() - 1] == ' ' || str[str.size() - 1] == '\t' ||
                       str.find(commentChar) != std::string::npos)) {
...
void enigma::IniFileIndex::delKey(const std::string& section, const std::string& key) {
  ...
  std::vector<std::string>::iterator remIt =
      std::find(secIt->second.propKeyOrder.begin(), secIt->second.propKeyOrder.end(), key);
...
std::string enigma::IniFileIndex::toString() const {
  ...
  for (std::vector<std::string>::const_iterator secIt = secNameOrder.begin(); secIt != secNameOrder.end(); secIt++) {
    ...
    res << sec.prefix;
    res << "[" << (*secIt) << "]\n";
    for (std::vector<std::string>::const_iterator keyIt = sec.propKeyOrder.begin(); keyIt != sec.propKeyOrder.end();
         keyIt++) {
      const IniValue& val = sec.props.find(*keyIt)->second;
      res << val.prefix;
      res << *keyIt << " = " << optional_quote(val.value, commentChar)
          << (val.postfix.empty() ? "" : std::string(" ") + val.postfix) << "\n";
    }
  }
  res << postComment;
  return res.str();
