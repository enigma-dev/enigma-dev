...
#ifndef ENIGMA_ROOM_SYSTEM_H
#define ENIGMA_ROOM_SYSTEM_H
...
void rooms_load();
void game_start();
struct roomv : multifunction_variant {
...
int room_set_background(int indx, int bind, bool vis, bool fore, bool back, double x, double y, bool htiled,
                        bool vtiled, double hspeed, double vspeed, double alpha = 1, int color = 0xFFFFFF);
...
int room_tile_add_ext(int indx, int bck, int left, int top, int width, int height, int x, int y, int depth, int xscale,
                      int yscale, double alpha, int color = 0xFFFFFF);
...
inline int room_assign(int indx, int roomindx) { return room_duplicate(indx, true, roomindx); }
...
extern rvt view_hborder, view_hport, view_hspeed, view_hview, view_object, view_vborder, view_visible, view_vspeed,
    view_wport, view_wview, view_xport, view_xview, view_yport, view_yview, view_angle;

extern enigma::roomv room;

inline void game_restart() { room_goto_first(true); }
}  // namespace enigma_user

#endif  //ENIGMA_ROOM_SYSTEM_H