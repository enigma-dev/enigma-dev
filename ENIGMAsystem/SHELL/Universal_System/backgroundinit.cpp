...
void exe_loadbackgrounds(FILE* exe) {
  int nullhere;
  unsigned bkgid, width, height, transparent, smoothEdges, preload, useAsTileset, tileWidth, tileHeight, hOffset,
      vOffset, hSep, vSep;

  if (!fread(&nullhere, 4, 1, exe) or nullhere != *(int*)"BKG ") return;

  // Determine how many backgrounds we have
  int bkgcount;
  if (!fread(&bkgcount, 4, 1, exe)) return;

  // Fetch the highest ID we will be using
  int bkg_highid;
  if (!fread(&bkg_highid, 4, 1, exe)) return;
  ...
  for (int i = 0; i < bkgcount; i++) {
    ...
    if (size != sz2) {
      show_error("Failed to load background: Data is truncated before exe end. Read " + toString(sz2) +
