...
static inline int DEBUG_ID_CHECK(int id, int objind) {
  ...
  if (it != instance_list.end()) {
    show_error("Two instances were given the same ID! Object `" +
                   enigma_user::object_get_name(it->second->inst->object_index) + "' and new object `" +
                   enigma_user::object_get_name(objind) + "' both have ID " + enigma_user::toString(id) +
                   ...
                   " feature to assign new, ordered IDs to all instances.",
               false);
    return maxid++;
  }
  return id;
...
#define errcheck(objid, err) \
  if (objid < 0 or objid >= enigma::objectcount or !enigma::objectdata[objid]) return (show_error(err, 0), 0)
#define errcheck(objid, err) \
  if (objid < 0 or objid >= enigma::objectcount or !enigma::objectdata[objid]) show_error(err, 0)
#else
#define errcheck(objid, err)
