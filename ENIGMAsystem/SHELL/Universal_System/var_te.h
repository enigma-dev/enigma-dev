...
#define types_extrapolate_real_p(prefix, suffix...)                                                             \
  prefix(int x) EVCONST suffix prefix(long x) EVCONST suffix prefix(unsigl x) EVCONST suffix prefix(unsigned x) \
      EVCONST suffix                                                                                            \
...
#define types_binary_extrapolate_real_p(prefix, type2, suffix...)                                        \
  prefix(int x, type2 y) EVCONST suffix prefix(long x, type2 y) EVCONST suffix prefix(unsigl x, type2 y) \
      EVCONST suffix                                                                                     \
...
#define types_binary_bitwise_extrapolate_real_p(op, type2, sentiment...) \
  ...
  long long operator op(long long x, type2 y) EVCONST {                  \
    sentiment;                                                           \
    return x op(long long)(y);                                           \
  }                                                                      \
  unsigll operator op(unsigll x, type2 y) EVCONST {                      \
    sentiment;                                                           \
    return x op(unsigll)(y);                                             \
  }                                                                      \
  long operator op(double x, type2 y) EVCONST {                          \
    sentiment;                                                           \
    return long(x) op long(y);                                           \
  }                                                                      \
  long operator op(float x, type2 y) EVCONST {                           \
    sentiment;                                                           \
    return long(x) op long(y);                                           \
  }
...
#define types_extrapolate_string_p(prefix, suffix...) \
  prefix(const char *x) EVCONST suffix prefix(string x) EVCONST suffix

#define types_binary_extrapolate_string_p(prefix, type2, suffix...) \
  prefix(const char *x, type2 y) EVCONST suffix prefix(string x, type2 y) EVCONST suffix

#define types_extrapolate_mix_p(prefix, suffix...) prefix(variant x) EVCONST suffix prefix(var x) EVCONST suffix
...
#define types_binary_extrapolate_alldecc(type, fname, type2) \
  types_binary_extrapolate_real_p(type fname, type2, ;) types_binary_extrapolate_string_p(string fname, type2, ;)
#define types_binary_extrapolate_alldecce(type, fname, type2) \
  types_binary_extrapolate_real_p(type fname, type2, ;) types_binary_extrapolate_string_p(type fname, type2, ;)
