...
pvrect::pvrect(int a, int b, int c, int d, int e) : x(a), y(b), w(c), h(d), placed(e) {}
...
rectpnode::rectpnode(int xx, int yy, int w, int h, rectpnode *c1, rectpnode *c2) : x(xx), y(yy), wid(w), hgt(h), c(-1) {
  child[0] = c1, child[1] = c2;
}
...
rectpnode *rninsert(rectpnode *who, unsigned int c, pvrect *boxes) {
  ...
  if (who->child[0])  // Already split
  {
    ...
    if (who->wid - boxes[c].w < 2 and
        who->hgt - boxes[c].h < 2)  //tight squeeze; too tight to fit a glyph inside, I promise.
      return (who->c = c, who);
