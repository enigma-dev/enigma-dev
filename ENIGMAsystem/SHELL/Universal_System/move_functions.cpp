...
void move_snap(const cs_scalar hsnap, const cs_scalar vsnap) {
  ...
  if (fnzero(hsnap)) inst->x = round(inst->x / hsnap) * hsnap;
  if (fnzero(vsnap)) inst->y = round(inst->y / vsnap) * vsnap;
}
...
void move_random(const cs_scalar snapHor, const cs_scalar snapVer) {
  ...
  if (spr_ind == -1 && (mask_ind == -1)) {
    ...
    x1 = sprite_get_xoffset(mask);
    y1 = sprite_get_yoffset(mask);
    x2 = room_width - sprite_get_width(mask) + sprite_get_xoffset(mask);
    y2 = room_height - sprite_get_height(mask) + sprite_get_yoffset(mask);
  }
