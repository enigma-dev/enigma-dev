...
#define get_current_instance() ((enigma::object_graphics *)enigma::instance_event_iterator->inst)

bool get_sprite(enigma::sprite *&spr, int id) {
#ifdef DEBUG_MODE
  if (id < -1 || size_t(id) > enigma::sprite_idmax || !enigma::spritestructarray[id]) {
    show_error("Cannot access sprite with id " + enigma_user::toString(id), false);
    return false;
  }
#endif
...
int sprite_add(string filename, int imgnumb, bool precise, bool transparent, bool smooth, bool preload, int x_offset,
               int y_offset, bool mipmap) {
...
bool sprite_replace(int ind, string filename, int imgnumb, bool precise, bool transparent, bool smooth, bool preload,
  ...
  enigma::sprite *spr;
  if (!get_sprite_mtx(spr, ind)) return false;
...
bool sprite_exists(int spr) { return (unsigned(spr) < enigma::sprite_idmax) and bool(enigma::spritestructarray[spr]); }
...
void sprite_delete(int ind, bool free_texture) {
  ...
  if (free_texture)
    for (int ii = 0; ii < spr->subcount; ii++) enigma::graphics_delete_texture(spr->texturearray[ii]);
...
int sprite_duplicate(int copy_sprite) {
  ...
  if (!get_sprite_mtx(spr_copy, copy_sprite)) return -1;
...
void sprite_assign(int ind, int copy_sprite, bool free_texture) {
  ...
  if (!get_sprite_mtx(spr, ind)) return;
  if (!get_sprite_mtx(spr_copy, copy_sprite)) return;
...
void sprite_set_alpha_from_sprite(int ind, int copy_sprite, bool free_texture) {
  ...
  for (int i = 0; i < spr->subcount; i++)
    enigma::graphics_replace_texture_alpha_from_texture(spr->texturearray[i],
                                                        spr_copy->texturearray[i % spr_copy->subcount]);
}
...
void sprites_init() {
  ...
  for (unsigned i = 0; i < sprite_idmax; i++) spritestructarray[i] = NULL;
}
...
int sprite_new_empty(unsigned sprid, unsigned subc, int w, int h, int x, int y, int bbt, int bbb, int bbl, int bbr,
  ...
  if (sprite_idmax < sprid + 1) sprite_idmax = sprid + 1;

  return sprid;
}

void sprite_add_to_index(sprite *ns, string filename, int imgnumb, bool precise, bool transparent, bool smooth,
...
const bbox_rect_t &sprite_get_bbox(int sprid) {
  sprite *spr;
  if (!get_sprite(spr, sprid)) return dummy_bbox;
...
int sprite_get_width(int sprid) {
  enigma::sprite *spr;
  if (!get_sprite(spr, sprid)) return 32;
...
int sprite_get_bbox_left(int sprid) {
  enigma::sprite *spr;
  if (!get_sprite(spr, sprid)) return 0;
