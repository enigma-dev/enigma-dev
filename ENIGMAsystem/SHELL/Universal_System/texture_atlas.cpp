...
#else
#define get_sprite(spr, id) enigma::sprite *const spr = enigma::spritestructarray[id];
#endif
...
struct TextureAtlasRect {
  ...
  const bool operator<(TextureAtlasRect const &rhs) const { return this->size < rhs.size; }
};

std::unordered_map<unsigned int, texture_atlas> texture_atlas_array;
size_t texture_atlas_idmax = 0;

bool textures_pack(int ta, bool free_textures) {
  // Implement packing algorithm.
  std::vector<texture_element> &textures = texture_atlas_array[ta].textures;
  std::vector<enigma::rect_packer::pvrect> metrics(textures.size());  //This means every resource has only one image
  for (unsigned int i = 0; i < textures.size(); i++) {                //This adds the rest of the images
    switch (textures[i].type) {
      ...
      default:
        break;  //We do nothing for the rest
    }
  }
    ...
    switch (textures[i].type) {
      ...
      case 2: {  //Metrics for font glyps
        ...
        for (size_t g = 0; g < fnt->glyphRangeCount; g++) {
          enigma::fontglyphrange *fgr = fnt->glyphRanges[g];
          for (size_t s = 0; s < fgr->glyphcount; s++) {
            metrics[counter].w = fgr->glyphs[s]->x2 - fgr->glyphs[s]->x,
            metrics[counter].h = fgr->glyphs[s]->y2 - fgr->glyphs[s]->y;
            counter++;
          }
        }
      } break;
      default:
        break;  //We do nothing for the rest (there shouldn't be any "rest")
    }
  }

  std::vector<TextureAtlasRect> boxes;
  ...
  for (auto i = boxes.rbegin(); i != boxes.rend();) {
    ...
    if (nn) {
      enigma::rect_packer::rncopy(nn, &metrics.front(), indx), i++;
      max_textures++;
    } else {
      if (texture_atlas_array[ta].width != -1 && texture_atlas_array[ta].height != -1) {
        break;  //We are already at max size
      }
      w > h ? h <<= 1 : w <<= 1, rectplane = enigma::rect_packer::expand(rectplane, w, h);
  ...
  for (unsigned int i = 0; i < textures.size(); i++) {
    switch (textures[i].type) {
      case 0: {  //Copy textures for all sprite subimages
        enigma::sprite *sspr = enigma::spritestructarray[textures[i].id];
        for (int s = 0; s < sspr->subcount; s++) {
          enigma::graphics_copy_texture(sspr->texturearray[s], enigma::texture_atlas_array[ta].texture,
                                        metrics[counter].x, metrics[counter].y);
      ...
      case 1: {  //Copy textures for all the backgrounds
        enigma::background *bkg = enigma::backgroundstructarray[textures[i].id];
        enigma::graphics_copy_texture(bkg->texture, enigma::texture_atlas_array[ta].texture, metrics[counter].x,
                                      metrics[counter].y);
        if (free_textures == true) {
      ...
      case 2: {  //Copy textures for all font glyps
        ...
        for (size_t g = 0; g < fnt->glyphRangeCount; g++) {
          ...
          for (size_t s = 0; s < fgr->glyphcount; s++) {
            double tix,
                tiy;  //Calculate texture position in image space (pixels) instead of normalized 0-1. We sadly don't hold this information, but maybe we should
            ...
            enigma::graphics_copy_texture_part(fnt->texture, enigma::texture_atlas_array[ta].texture, tix, tiy, gw, gh,
                                               metrics[counter].x, metrics[counter].y);
...
int texture_atlas_create(int w, int h) {
  ...
  if (w != -1 && h != -1) {                                                                //If we set the size manually
    ...
    enigma::texture_atlas_array[id].texture =
        enigma::graphics_create_texture(fullwidth, fullheight, fullwidth, fullheight, nullptr, false);
  } else {
...
inline void texture_atlas_pack_sprite(int tp, int spr) {
  enigma::texture_atlas_array[tp].textures.emplace_back(spr, 0);
}
inline void texture_atlas_pack_background(int tp, int bkg) {
  enigma::texture_atlas_array[tp].textures.emplace_back(bkg, 1);
}
