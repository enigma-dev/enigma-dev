...
inline void action_set_caption(const int vscore, const string scoreCaption, const int vlives, string livesCaption,
                               const int vhealth, const string healthCaption) {
  show_score = vscore;
...
inline bool action_if_number(const int object, const double number, const int operation) {
  switch (operation) {
    case 0:
      return (instance_number(object) == lrint(number));
      break;
    case 1:
...
inline void action_move_point(const double x, const double y, const double speed) {
  ...
  }  //is speed also relative?
  else
    move_towards_point(x, y, speed);
}

inline bool action_if(const double x) { return x >= 0.5; }

inline bool action_if_previous_room() { return room_previous(room) != -1; }

inline bool action_if_next_room() { return room_next(room) != -1; }
...
inline void action_move(const char dir[9], int argspeed) {
  ...
  const double newdir = ((enigma::object_planar*)enigma::instance_event_iterator->inst)->direction.rval.d =
      chosendirs[choices];
  if (argument_relative) argspeed += ((enigma::object_planar*)enigma::instance_event_iterator->inst)->speed;
  const double newspd = ((enigma::object_planar*)enigma::instance_event_iterator->inst)->speed.rval.d =
      chosendirs[choices] == -1 ? 0 : argspeed;
...
inline void action_reverse_xdir() {
  ((enigma::object_graphics*)enigma::instance_event_iterator->inst)->hspeed =
      -((enigma::object_graphics*)enigma::instance_event_iterator->inst)->hspeed;
}

inline void action_reverse_ydir() {
  ((enigma::object_graphics*)enigma::instance_event_iterator->inst)->vspeed =
      -((enigma::object_graphics*)enigma::instance_event_iterator->inst)->vspeed;
}

inline bool action_if_aligned(const double snapHor, const double snapVer) { return place_snapped(snapHor, snapVer); }
...
inline void action_execute_script(string script, string argument0, string argument1, string argument2, string argument3,
                                  string argument4) {}
#define action_execute_script(script, argument0, argument1, argument2, argument3, argument4) \
  script((argument0), (argument1), (argument2), (argument3), (argument4))
...
void action_create_object_random(const int object1, const int object2, const int object3, const int object4,
                                 const double x, const double y);
void action_create_object_random(const int object1, const int object2, const int object3, const int object4,
                                 const double x, const double y) {
  int obj_ar[4], obj_num = 0;
  if (object1 != -1) obj_ar[obj_num++] = object1;
  if (object2 != -1) obj_ar[obj_num++] = object2;
  if (object3 != -1) obj_ar[obj_num++] = object3;
  if (object4 != -1) obj_ar[obj_num++] = object4;
...
inline bool action_if_health(const double value, const int operation) {
  switch (operation) {
    case 0:
      return (fabs(health - value) < _V_EPSILON);
      break;
    case 1:
      return (health < value);
      break;
...
inline bool action_if_life(const double value, const int operation) {
  switch (operation) {
    case 0:
      return (fabs(lives - value) < _V_EPSILON);
      break;
    case 1:
      return (lives < value);
      break;
...
inline bool action_if_score(const double value, const int operation) {
  switch (operation) {
    case 0:
      return (fabs(score - value) < _V_EPSILON);
      break;
    case 1:
      return (score < value);
      break;
...
inline bool action_if_mouse(const int button) { return mouse_check_button(button); }

inline void action_move_random(const double snapHor, const double snapVer) { move_random(snapHor, snapVer); }

inline void action_snap(const double hor, const double vert) { move_snap(hor, vert); }

inline void action_highscore_clear() { highscore_clear(); }

inline void action_create_object_motion(int object, double x, double y, double speed, double direction) {
  if (argument_relative) {
    enigma::object_planar* const inst = ((enigma::object_planar*)enigma::instance_event_iterator->inst);
    enigma::object_planar* const ii =
        ((enigma::object_planar*)enigma::fetch_instance_by_int(instance_create(inst->x + x, inst->y + y, object)));
    ii->speed.rval.d = speed;
    ii->direction = direction;
  } else {
    enigma::object_planar* const ii =
        ((enigma::object_planar*)enigma::fetch_instance_by_int(instance_create(x, y, object)));
    ii->speed.rval.d = speed;
    ii->direction = direction;
  }
}
...
enum { ev_step_normal = 0, ev_step_begin = 1, ev_step_end = 2 };
...
inline variant event_perform(int type, int numb) { return enigma::ev_perf(type, numb); }

inline variant event_user(int numb) { return event_perform(enigma_user::ev_other, numb + enigma_user::ev_user0); }
