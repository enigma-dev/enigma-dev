...
unsigned int readBits2(unsigned char* bytes, size_t& pos, const size_t size, unsigned int& subBlockEnd,
                       unsigned int& currBit, unsigned int currCodeSize, unsigned int& res) {
  if (currCodeSize > MaxCodeSize) {
    return ERR_CODEBITS_PAST_12;
  }
  if (currBit > 8) {
    return ERR_BITS_PAST_8;
  }
...
unsigned int load_gif_file(const char* filename, unsigned char*& out, unsigned int& gif_width, unsigned int& gif_height,
  ...
  if (gifVers != "87a" && gifVers != "89a") {
    return ERR_BAD_GIF_VERS;
  }
  ...
  if (bytes[pos++] != 0) {
    return ERR_NONZERO_PIXEL_ASPECT_RATIO;
  }

  //Read the table of global colors (optional).
  size_t globalColorStart = 0;
  if (screen.gctFlag) {
    if (pos + (screen.gctSize * 3) > size) {
      return ERR_OUT_OF_BYTES;
    }
  ...
  num_images = 0;
  {
    ...
    for (;;) {
      ...
      if (ctrlCode == 0x21) {  //It's an extension; skip it.
        ...
        myPos += len;
        if (!skipSubBlocks(bytes, myPos, size)) {
          return ERR_OUT_OF_BYTES;
        }
  ...
  for (;;) {
    ...
    if (ctrlCode == 0x21) {  //It's an extension; skip it.
      ...
      pos += extLen;
      if (!skipSubBlocks(bytes, pos, size)) {
        clearmem(out);
        return ERR_OUT_OF_BYTES;
      }
    } else if (ctrlCode == 0x3B) {  //EOF; done;
      ...
      if (pos > subBlockEnd) {
        clearmem(out);
        return ERR_OUT_OF_BYTES;
      }  //Might be better as "not enough image data"?
      if (subBlockEnd + 1 > size) {
        clearmem(out);
        return ERR_OUT_OF_BYTES;
      }
...
const char* load_gif_error_text(unsigned int err) {
  switch (err) {
    case ERR_SUCCESS:
      return ERRMSG_SUCCESS;
    case ERR_FILE_CANT_OPEN:
      return ERRMSG_FILE_CANT_OPEN;
    case ERR_OUT_OF_BYTES:
      return ERRMSG_OUT_OF_BYTES;
    case ERR_BAD_MAG_NUM:
      return ERRMSG_BAD_MAG_NUM;
    case ERR_BAD_GIF_VERS:
      return ERRMSG_BAD_GIF_VERS;
    case ERR_NONZERO_PIXEL_ASPECT_RATIO:
      return ERRMSG_NONZERO_PIXEL_ASPECT_RATIO;
    case ERR_UNKNOWN_CONTROL_CODE:
      return ERRMSG_UNKNOWN_CONTROL_CODE;
    case ERR_INTERLACED_IMAGE:
      return ERRMSG_INTERLACED_IMAGE;
    case ERR_EXPECTED_CLEAR_CODE:
      return ERRMSG_EXPECTED_CLEAR_CODE;
    case ERR_OUT_OF_BITS_IN_BYTESTREAM:
      return ERRMSG_OUT_OF_BITS_IN_BYTESTREAM;
    case ERR_OVERSCAN:
      return ERRMSG_OVERSCAN;
    case ERR_INDEX_COUNT_MISMATCH:
      return ERRMSG_INDEX_COUNT_MISMATCH;
    case ERR_BITS_PAST_8:
      return ERRMSG_BITS_PAST_8;
    case ERR_CODEBITS_PAST_12:
      return ERRMSG_CODEBITS_PAST_12;
    default:
