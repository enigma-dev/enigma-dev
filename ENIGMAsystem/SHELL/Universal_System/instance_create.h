...
#include "Preprocessor_Environment_Editable/IDE_EDIT_object_switch.h"
  ...
  newinst->x = x;
  newinst->y = y;
  newinst->yprevious = yprevious;
  newinst->xprevious = xprevious;
  newinst->xstart = xstart;
  newinst->ystart = ystart;
  newinst->image_index = image_index;
  newinst->image_speed = image_speed;
  newinst->image_xscale = image_xscale;
  newinst->image_yscale = image_yscale;
  newinst->image_angle = image_angle;
  newinst->hspeed = hspeed;
  newinst->vspeed = vspeed;
  if (perf) newinst->myevent_create();
}

object_basic* instance_create_id(int x, int y, int object,
                                 int idn) {  //This is for use by the system only. Please leave be.
  if (maxid < idn) maxid = idn;
  object_basic* ob;
  switch (object) {
#define NEW_OBJ_PREFIX ob =
#include "Preprocessor_Environment_Editable/IDE_EDIT_object_switch.h"
...
#include "Preprocessor_Environment_Editable/IDE_EDIT_object_switch.h"
  ...
  if (perf) newinst->myevent_create();
  newinst->yprevious = inst->yprevious;
  newinst->xprevious = inst->xprevious;
  newinst->xstart = inst->xstart;
  newinst->ystart = inst->ystart;
  newinst->image_index = inst->image_index;
  newinst->image_speed = inst->image_speed;
  newinst->visible = inst->visible;
  newinst->image_xscale = inst->image_xscale;
  newinst->image_yscale = inst->image_yscale;
  newinst->image_angle = inst->image_angle;
  newinst->hspeed = inst->hspeed;
  newinst->vspeed = inst->vspeed;
}
}  // namespace enigma_user
