...
iterator::iterator() : it(NULL) { addme(); }
iterator::iterator(const iterator &other) {
  copy(other);
  addme();
}
iterator::iterator(inst_iter *iter) : it(iter) { addme(); }
iterator::iterator(object_basic *ob) : temp_iter(ob, NULL, NULL), it(&temp_iter) { addme(); }

iterator::~iterator() { central_iterator_cache.erase(this); }

void update_iterators_for_destroy(const inst_iter *dd) {
  for (central_iterator_cache_iterator it = central_iterator_cache.begin(); it != central_iterator_cache.end(); ++it) {
    (*it)->handle_unlink(dd);
  }
}
...
inst_iter *event_iter::add_inst(object_basic *ninst) {
  ...
  if (prev)
    prev->next = a;  // If we have a final item, set its next node to this item.
  else
    next = a;       // Otherwise, set our first item to this item.
...
temp_event_scope::temp_event_scope(object_basic *ninst)
    : oinst(instance_event_iterator->inst), oiter(instance_event_iterator), prev_other(instance_other) {
  instance_event_iterator = &dummy_event_iterator;
  instance_event_iterator->inst = ninst;
...
object_basic *fetch_instance_by_int(int x) {
  ...
  if (x < 0) switch (x) {
      case self:
      case local:
        return instance_event_iterator ? instance_event_iterator->inst : NULL;
      case other:
        return instance_other;
      case all: {
        iterator i = instance_list_first();
        return i ? *i : NULL;
      }
      case global:
        return ENIGMA_global_instance;
      case noone:
      default:
        return NULL;
    }

  if (x < 100000) return x < object_idmax ? objects[x].next ? objects[x].next->inst : NULL : NULL;
...
iterator fetch_inst_iter_by_int(int x) {
  ...
  if (x < 0)
    switch (x)  // Keyword-based lookup
    {
      case self:
      case local:
        return iterator(instance_event_iterator->inst);
      case other:
        return instance_other ? iterator(instance_other) : iterator();
      case all:
        return instance_list_first();
      case global:
        return &ENIGMA_global_instance_iterator;
      case noone:
      default:
        return iterator();
    }
...
void winstance_list_iterator_delete(pinstance_list_iterator whop) { delete whop; }
...
void dispose_destroyed_instances() {
  for (set<object_basic *>::iterator i = cleanups.begin(); i != cleanups.end(); i++) delete (*i);
  cleanups.clear();
}
