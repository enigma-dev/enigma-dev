...
namespace enigma
{
	int maxid=10000,instancecount=0,id_current=0;
    ...
    struct object{
		...
		virtual enigma::variant myevent_draw()          { if (sprite_index!=-1) draw_sprite_ext(sprite_index,image_index,x,y,image_xscale,image_yscale,image_angle,image_blend,image_alpha); return 0; }
		...
		virtual ~object() {} //no destructor, but needs cleanup. Segfault issues
	};
    ...
    void step_basic(object* instance)
    {
        ...
        if (instance->gravity != 0 || instance->friction != 0)
        {
          ...
          instance->hspeed.realval-=instance->friction*cos(instance->direction.realval/180.0*pi);
          instance->vspeed.realval-=instance->friction*-sin(instance->direction.realval/180.0*pi);

          if (instance->speed.realval<0)
          {
            instance->direction.realval=180+(int(180+180*(1-atan2(instance->vspeed,instance->hspeed)/pi)))%360;
            instance->speed.realval=-sqrt(instance->hspeed*instance->hspeed+instance->vspeed*instance->vspeed);
          }
          else
          {
            instance->direction.realval=(int(180+180*(1-atan2(instance->vspeed,instance->hspeed)/pi)))%360;
            instance->speed.realval=sqrt(instance->hspeed*instance->hspeed+instance->vspeed*instance->vspeed);
          }
        }
