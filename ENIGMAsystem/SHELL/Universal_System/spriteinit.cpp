...
void exe_loadsprs(FILE* exe) {
  ...
  for (int i = 0; i < sprcount; i++) {
    ...
    switch (shape) {
      case ct_precise:
        coll_type = ct_precise;
        break;
      case ct_bbox:
        coll_type = ct_bbox;
        break;
      case ct_ellipse:
        coll_type = ct_ellipse;
        break;
      case ct_diamond:
        coll_type = ct_diamond;
        break;
      case ct_polygon:
        coll_type = ct_bbox;
        break;  //FIXME: Change to ct_polygon once polygons are supported.
      case ct_circle:
        coll_type = ct_circle;
        break;
      default:
    ...
    for (int ii = 0; ii < subimages; ii++) {
      ...
      if (!cpixels) {  //FIXME: Uncomment these when tostring is available
        show_error("Failed to load sprite: Cannot allocate enough memory " + enigma_user::toString(unpacked), 0);
        break;
      }
      unsigned int sz2 = fread(cpixels, 1, size, exe);
      if (size != sz2) {
        show_error("Failed to load sprite: Data is truncated before exe end. Read " + enigma_user::toString(sz2) +
      ...
      switch (coll_type) {
        ...
        case ct_polygon:
          collision_data = 0;
          break;  //FIXME: Support vertex data.
        default:
          collision_data = 0;
          break;
      };
