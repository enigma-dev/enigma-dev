...
template <typename T>
typename std::enable_if<is_std_vector_v<std::decay_t<T>>, std::size_t>::type inline byte_size(const T &value);
...
template <typename T>
typename std::enable_if<is_std_set_v<std::decay_t<T>>, std::size_t>::type inline byte_size(const T &value);
