...
#ifndef ENIGMA_DARRAY_H
#define ENIGMA_DARRAY_H
...
struct darray {
  ...
  atype& operator[](unsigned int ind) {
    if (allocd <= ind) {
      ...
      if (olds > 0) memcpy(nar, where, olds * sizeof(atype));
...
struct varray {
  ...
  varray() {
    safety_val = new atype;
    where = new atype*[1];
    allocd = (where != 0);
    if (allocd == 0 or safety_val == 0) exit(-18);
    where[0] = 0;
    size = 0;
  }
  ~varray() {
    if (where != 0)
      for (unsigned int i = 0; i < size; i++) delete where[i];
  }
};
...
struct varray_ns {
  ...
  ~varray_ns() {
    if (where != 0)
      for (unsigned int i = 0; i < allocd; i++) delete where[i];
  }
};
