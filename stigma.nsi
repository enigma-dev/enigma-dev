...
!define MUI_ICON "C:\stigma-dev\Resources\icon.ico"
...
Section "STIGMA Sources"
  ...
  FileOpen $7 $INSTDIR\stigma-dev\stigma-launch.bat w
  FileWrite $7 "$INSTDIR\msys64\msys2_shell.cmd -defterm -mingw64 -no-start -c $\"EDIR=$$(echo \$\"$INSTDIR\stigma-dev\$\" | tr $\'\\$\' $\'/$\') && cd $$EDIR && ./LGM.exe$\""
  FileClose $7

  FileOpen $8 $INSTDIR\stigma-dev\stigma-update.bat w
  FileWrite $8 "$INSTDIR\msys64\msys2_shell.cmd -defterm -mingw64 -no-start -c $\"EDIR=$$(echo \$\"$INSTDIR\stigma-dev\$\" | tr $\'\\$\' $\'/$\') && cd $$EDIR && pacman -Syuu --noconfirm && git pull && ./install.sh && make$\""
  FileClose $8

  FileOpen $9 $INSTDIR\stigma-dev\msys2-shell.bat w
  FileWrite $9 "$INSTDIR\msys64\msys2_shell.cmd -defterm -mingw64 -no-start -c $\"EDIR=$$(echo \$\"$INSTDIR\stigma-dev\$\" | tr $\'\\$\' $\'/$\') && cd $$EDIR && bash$\""
  FileClose $9
