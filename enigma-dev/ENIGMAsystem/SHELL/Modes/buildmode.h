...
namespace buildmode
{
  ...
  void bmain()
  {
    ...
    if (pausing) { if (enigma::keybdstatus[vk_space]==0) pausing=0; else return; }
    
    if (enigma::keybdstatus[vk_enter]==1 || (SendMessage(resume,BM_GETSTATE,0,0))) unpausing=1;
    else if (unpausing) { paused=0; unpausing=0; delete[] scrpixels; return; }
    
    
    if((enigma::last_keybdstatus[vk_right]==1 && enigma::keybdstatus[vk_right]==0))
    stepping=1; else if (stepping) { stepping=0; take_a_step=1; }
    if (SendMessage(step,BM_GETSTATE,0,0)==108)
    { if (!stepdown) take_a_step=1; stepdown=1; } else stepdown=0; 
    ...
    { if ((*enigma::objiter).second.name==itemtext) { objnum=(*enigma::objiter).first; break; } }
    ...
    if (!enigma::keybdstatus[vk_control])
    {
      ...
      if (objnum != -1)
      {
        if (!enigma::mousestatus[0])
        {
          if (enigma::objectdata[objnum].sprite_index != -1)
          {
            int xx,yy;
            if (enigma::keybdstatus[vk_alt])       { xx=(int)mouse_x; yy=(int)mouse_y; } 
            else { xx=gridx*(int)(mouse_x/gridx); yy=gridy*(int)(mouse_y/gridy); }
        ...
        else
        {
            if (!enigma::last_mousestatus[0] || enigma::keybdstatus[vk_shift])
            {
              ...
              int insthere=0; for (int i=0;i<changecount[room];i++)
              if (changes[room][i].x==xx && changes[room][i].y==yy) { insthere=1; break; }
        ...
        if (!enigma::last_mousestatus[0])
        {
                 ...
                 enigma::instance_iterator++)
            {
              ...
              if (fobj!=NULL)
              {
                ...
                { dragid=(*enigma::instance_iterator).first; break; }
              }
            }
          }
          else if (dragid>0)
          {
            ...
            else
            {
              ...
              if (enigma::roomdata[room].instances[i].id==dragid) { instthere=1; break; }
    ...
    if (!enigma::last_mousestatus[1] && enigma::mousestatus[1])
    {
             ...
             enigma::instance_iterator++)
        {
          ...
          if (fobj!=NULL)
          {
            ...
            if (bx<16) bx=16; if (by<16) by=16;
            if ((mouse_x>fobj->x) && (mouse_y>fobj->y) && (mouse_x<(fobj->x+bx)) && (mouse_y<(fobj->x+by)))
            { fnd=(*enigma::instance_iterator).first; break; }
          }
        }
        if (fnd>0)
        {
