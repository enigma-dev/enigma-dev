...
macro_function::macro_function(string n, string val): macro_type(n,0), value(), args() {
  __REGISTER const size_t bs = val.length();
  if (bs) {
  ...
  for (size_t i = 0; i < value.size(); i++)
    if (value[i].is_arg)
    {
      if (last_was_arg) {
        ...
        __REGISTER const char* argname = ts.c_str();
        __REGISTER size_t sz = ts.length();
        while (is_useless(*argname)) ++argname, --sz;
        memcpy(bufat, argname, sz);
        bufat += sz;
      }
      else {
        __REGISTER const string& argname = arg_list[value[i].metric];
