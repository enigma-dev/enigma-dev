...
/*
extern map<int,std::string> __fnames;
extern int __fcount;*/
#include "externs.h"
...
namespace syncheck
{
  ...
  /*
   ...
   ////////////////////////////////////////
      ...
      int syntacheck(string code) //std::string for substr() member
      {
          error="";
          errorpos=-1;
            ...
            else //It's a symbol: Not a letter or digit
            {
              ...
              if (code[pos]=='(')
              {
                ...
                if (lastnamed[level]==LN_VARNAME)
                {
                  ...
                  int cf=checkfunction(code,pos,len);
                  if (cf!=-1) return cf;
                  lastnamed[level]=LN_NOTHING;
                }
                else
