...
class LanguageFrontend {
 public:
  ...
  virtual void definition_parameter_bounds(jdi::definition *d, unsigned &min, unsigned &max) const = 0;

  /// Create a script with the given name (and an assumed 16 parameters, all defaulted) to the given scope.
  virtual void quickmember_script(jdi::definition_scope* scope, std::string name) = 0;
  /// Create a standard integer variable member in the given scope.
  virtual void quickmember_integer(jdi::definition_scope* scope, std::string name) = 0;
