...
class AstBuilder: public AstBuilderTestAPI {
  ...
  const Token &current_token() override{
    return token;
  }
  ...
  FullType TryParseTypeID() override{
    FullType type;
    while (next_is_type_specifier()) {
      TryParseTypeSpecifier(&type);
    }
  ...
  std::unique_ptr<AST::Node> TryParseStatement() override{
  ...
  std::unique_ptr<AST::CodeBlock> ParseCode() override{
  ...
  std::unique_ptr<AST::CodeBlock> ParseCodeBlock() override{
