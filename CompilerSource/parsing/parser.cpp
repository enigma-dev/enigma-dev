 ...
 */
  ...
  bool next_is_class_key() { return is_class_key(token); }

  bool is_cv_qualifier(const Token &tok) { return tok.type == TT_CONST || tok.type == TT_VOLATILE; }

  bool next_is_cv_qualifier() { return is_cv_qualifier(token); }

  bool is_ref_qualifier(const Token &tok) { return tok.type == TT_AMPERSAND || tok.type == TT_AND; }

  bool next_is_ref_qualifier() { return is_ref_qualifier(token); }
  ...
  bool next_is_operatorkw() { return is_operatorkw(token); }
  ...
  bool next_is_user_defined_type() { return is_user_defined_type(token); }
  ...
  bool next_is_type_specifier() { return is_type_specifier(token); }
  ...
  bool next_maybe_nested_name() { return maybe_nested_name(token); }
  ...
  bool is_template_type(jdi::definition *def) { return def->flags & jdi::DEF_TEMPLATE; }

  bool next_is_template_type() { return is_template_type(token); }

  bool maybe_ptr_decl_operator(const Token &tok) {
    return tok.type == TT_STAR || is_ref_qualifier(tok) || maybe_nested_name(tok);
  }

  bool next_maybe_ptr_decl_operator() { return maybe_ptr_decl_operator(token); }
  ...
  std::size_t sizeof_builtin_type(std::string_view type) {
    static const std::unordered_map<std::string_view, std::size_t> sizes{
        {"char", sizeof(char)},         {"char8_t", 1},
        {"char16_t", sizeof(char16_t)}, {"char32_t", sizeof(char32_t)},
        {"wchar_t", sizeof(wchar_t)},   {"bool", sizeof(bool)},
        {"short", sizeof(short)},       {"int", sizeof(int)},
        {"long", sizeof(long)},         {"float", sizeof(float)},
        {"double", sizeof(double)},     {"void", 1},
    };
  ...
  void MaybeConsumeSemicolon() {
    if (token.type == TT_SEMICOLON) token = lexer->ReadToken();
  }
  ...
  bool next_is_start_of_initializer() { return is_start_of_initializer(token); }
  ...
  bool next_is_start_of_id_expression() { return is_start_of_id_expression(token); }
  ...
  bool next_maybe_functional_cast() { return maybe_functional_cast(token); }
  ...
  void maybe_infer_int(FullType &type) {
    ...
    // from being legal
    if (type.def == nullptr &&
        (contains_decflag_bitmask(type.flags, "long") || contains_decflag_bitmask(type.flags, "short") ||
         contains_decflag_bitmask(type.flags, "long long") || contains_decflag_bitmask(type.flags, "signed") ||
 ...
 public:
  ...
  const Token &current_token() { return token; }

  std::unique_ptr<AST::Node> TryParseConstantExpression() { return TryParseExpression(Precedence::kTernary); }
      ...
      while (token.type != TT_ENDPARENTH) {
        if (is_decl_specifier(token) && maybe_expression) {
          auto declaration = TryParseEitherFunctionalCastOrDeclaration(
              AST::DeclaratorType::MAYBE_ABSTRACT, false, false, AST::DeclarationStatement::StorageClass::TEMPORARY);
          ...
          if (declaration->type == AST::NodeType::DECLARATION) {
            ...
            for (auto &param : params.as<FunctionParameterNode::ParameterList>()) {
              auto decl_expr =
                  std::unique_ptr<AST::Node>(reinterpret_cast<AST::Node *>(param.type->decl.to_expression()));
              if (param.default_value != nullptr) {
                decl_expr = std::make_unique<AST::BinaryExpression>(
                    std::move(decl_expr),
                    std::unique_ptr<AST::Node>(reinterpret_cast<AST::Node *>(param.default_value)), TT_EQUALS);
          ...
          if (!params.is<void *>()) {
            herr->Error(token) << "Internal error: params.parameters is not FunctionCallExpression in "
                                  "AstBuilder::TryParseParametersAndQualifiers";
          } else {
            reinterpret_cast<AST::FunctionCallExpression *>(params.as<void *>())
  ...
  void TryParseTemplateArgs(jdi::definition *def) {
    if (def->flags & jdi::DEF_TEMPLATE) {
      ...
      if (require_token(TT_GREATER, "Expected '>' after template arguments")) {
          ...
          if (argk[i].type == jdi::arg_key::AKT_FULLTYPE) {
            remap[template_def->params[i].get()] =
                std::make_unique<jdi::definition_typed>(template_def->params[i]->name, template_def, argk[i].ft(),
                                                        jdi::DEF_TYPENAME | jdi::DEF_TYPED)
                    .release();
          } else if (argk[i].type == jdi::arg_key::AKT_VALUE) {
            remap[template_def->params[i].get()] =
                std::make_unique<jdi::definition_valued>(
                    template_def->params[i]->name, template_def, template_def->params[i]->integer_type.def,
                    template_def->params[i]->integer_type.flags, jdi::DEF_VALUED, argk[i].val())
                    .release();
          } else {
        ...
        auto errc =
            jdi::ErrorContext{new jdi::DefaultErrorHandler{}, jdi::SourceLocation{"lol", token.position, token.line}};
  ...
  bool matches_token_type(jdi::definition *def, const Token &tok) {
    switch (tok.type) {
      case TT_ENUM:
        return def->flags & jdi::DEF_ENUM;
      case TT_STRUCT:
      case TT_CLASS:
        return def->flags & jdi::DEF_CLASS;
      case TT_UNION:
        return def->flags & jdi::DEF_UNION;
  ...
  std::unique_ptr<AST::Node> TryParseNoPtrDeclarator(FullType *type, AST::DeclaratorType is_abstract,
    ...
    auto maybe_infix_operator = [this]() {
      return map_contains(Precedence::kBinaryPrec, token.type) || map_contains(Precedence::kTernaryPrec, token.type);
    };

    auto maybe_postfix_operator = [this]() { return map_contains(Precedence::kUnaryPostfixPrec, token.type); };

    // Do not accidentally consume the pointer declarators
    if (maybe_expression && maybe_prefix_operator() && token.type != TT_STAR && token.type != TT_AMPERSAND) {
    ...
    if (token.type == TT_BEGINPARENTH) {
      ...
      // declarators
      if (maybe_expression && (maybe_infix_operator() || maybe_postfix_operator()) && token.type != TT_BEGINPARENTH &&
          token.type != TT_BEGINBRACKET) {
        if (inner_decl_expr == nullptr) {
          inner_decl_expr = TryParseExpression(
              Precedence::kAll, std::unique_ptr<AST::Node>(reinterpret_cast<AST::Node *>(inner.decl.to_expression())));
        } else {
      ...
      if (maybe_expression && (maybe_infix_operator() || maybe_postfix_operator()) && token.type != TT_BEGINPARENTH &&
          token.type != TT_BEGINBRACKET && token.type != TT_EQUALS && token.type != TT_BEGINBRACE &&
          token.type != TT_COMMA) {
        if (inner_decl_expr != nullptr) {
    ...
    if (maybe_expression && (maybe_infix_operator() || maybe_postfix_operator()) && token.type != TT_EQUALS &&
        token.type != TT_BEGINBRACE && token.type != TT_COMMA) {
      return TryParseExpression(Precedence::kAll,
  ...
  AST::InitializerNode TryParseInitializer(bool allow_paren_init = true) {
    switch (token.type) {
      ...
      case TT_BEGINBRACE:
        return AST::Initializer::from(TryParseBraceInitializer());
  ...
  std::unique_ptr<AST::Node> TryParseNewExpression(bool is_global) {
    ...
    if (token.type == TT_BEGINPARENTH) {
      ...
      if (next_is_type_specifier()) {
        ...
        is_array =
            !type.decl.components.empty() && type.decl.components.begin()->kind == DeclaratorNode::Kind::ARRAY_BOUND;

        MaybeConsumeSemicolon();

        return std::make_unique<AST::NewExpression>(is_global, is_array, std::move(placement), std::move(type),
    ...
    is_array = !type.decl.components.empty() && type.decl.components.begin()->kind == DeclaratorNode::Kind::ARRAY_BOUND;
  ...
  std::unique_ptr<AST::Node> TryParseDeleteExpression(bool is_global) {
    ...
    auto node =
        std::make_unique<AST::DeleteExpression>(is_global, is_array, TryParseExpression(Precedence::kUnaryPrefix));
    MaybeConsumeSemicolon();
  ...
  std::unique_ptr<AST::Node> TryParseOperand() {
    switch (token.type) {
      ...
      case TT_S_DELETE:
        return TryParseDeleteExpression(false);
      ...
      case TT_LOCAL: {
        if (next_maybe_functional_cast()) {
          ...
          if (token.type == TT_BEGINPARENTH) {
            ...
            return std::make_unique<AST::CastExpression>(AST::CastExpression::Kind::FUNCTIONAL, tok, std::move(type),
                                                         std::move(expr), TT_BEGINPARENTH);
          } else if (token.type == TT_BEGINBRACE) {
            auto tok = token;
            auto init = TryParseInitializer(false);
            require_token(TT_ENDBRACE, "Expected closing brace ('}') after temporary object initializer");
            return std::make_unique<AST::CastExpression>(AST::CastExpression::Kind::FUNCTIONAL, tok, std::move(type),
                                                         std::move(init), TT_BEGINBRACE);
          } else {
    ...
    herr->Error(token) << "Internal error: unreachable (" __FILE__ ":" << __LINE__ << ")";
    return nullptr;
  }

  static bool ShouldAcceptPrecedence(const OperatorPrecedence &prec, int target_prec) {
    return target_prec >= prec.precedence ||
           (target_prec == prec.precedence && prec.associativity == Associativity::RTL);
  }
  ...
  std::unique_ptr<AST::BinaryExpression> TryParseBinaryExpression(int precedence, std::unique_ptr<AST::Node> operand) {
    while (map_contains(Precedence::kBinaryPrec, token.type) &&
      ...
      auto right = (rule.associativity == Associativity::LTR)   ? TryParseExpression(rule.precedence - 1)
                   : (rule.associativity == Associativity::RTL) ? TryParseExpression(rule.precedence)
                                                                : nullptr;
  ...
  std::unique_ptr<AST::TernaryExpression> TryParseTernaryExpression(int precedence,
                                                                    std::unique_ptr<AST::Node> operand) {
    (void)precedence;
    //  Token oper = token;
    token = lexer->ReadToken();  // Consume the operator
  ...
  std::unique_ptr<AST::Node> TryParseStatement() {
    ...
    switch (token.type) {
      ...
      case TT_S_CASE:
        return ParseCaseOrDefaultStatement(false);
      case TT_S_DEFAULT:
        return ParseCaseOrDefaultStatement(true);
      case TT_S_FOR:
        return ParseForLoop();
      case TT_S_IF:
      ...
      if (next_is_type_specifier() ||
        ...
        if (declarator != nullptr) {
          ...
          if (type.decl.has_nested_declarator && type.decl.nested_declarator == 0) {
            type.decl =
                std::move(*type.decl.components[0].as<NestedNode>().as<std::unique_ptr<Declarator>>().release());
          }
  ...
  std::unique_ptr<AST::Node> ParseCaseOrDefaultStatement(bool is_default) {
    ...
    while (token.type != TT_S_CASE && token.type != TT_S_DEFAULT && token.type != TT_ENDBRACE) {
      if (token.type == TT_BEGINBRACE) {
        ...
        if (auto stmt = TryParseStatement()) body->statements.emplace_back(std::move(stmt));
      }
    }
