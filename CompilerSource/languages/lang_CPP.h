...
struct lang_CPP: language_adapter {
  ...
  // Sizable utilities
  int link_globals(parsed_object*, EnigmaStruct*,vector<parsed_script*>&, vector<parsed_script*>& tlines);
  int link_ambiguous(parsed_object*, EnigmaStruct*, vector<parsed_script*>&, vector<parsed_script*>& tlines);

  // IDE_EDITABLEs added before compile
  int compile_parseAndLink(EnigmaStruct*, vector<parsed_script*>&, vector<parsed_script*>& tlines, const std::set<std::string>& script_names);
  int compile_parseSecondary(map<int,parsed_object*>&, vector<parsed_script*>&, vector<parsed_script*>& tlines, map<int,parsed_room*>&,parsed_object*, const std::set<std::string>&);
