...
int lang_CPP::compile_write_shader_data(compile_context &ctex)
{
  ofstream wto((makedir +"Preprocessor_Environment_Editable/IDE_EDIT_shaderarrays.h").c_str(),ios_base::out);
  
  wto << gen_license << "#include \"Universal_System/shaderstruct.h\"\n" << "namespace enigma {\n";
  wto << "  ShaderStruct shaderstructarray[] = {\n";
  
  int idmax = 0;
  for (int i = 0; i < ctex.es->shaderCount; i++)
  {
    while (idmax < ctex.es->shaders[i].id) {
      ++idmax, wto << "ShaderStruct(),\n";
	}
	string vertexcode = ctex.es->shaders[i].vertex;
	string fragmentcode = ctex.es->shaders[i].fragment;
	//TODO: Replace quotations with escape sequences.
    wto << "    { "
        << '"' << esc(vertexcode)   << "\", "
        << '"' << esc(fragmentcode) << "\", "
        << '"' << ctex.es->shaders[i].type << "\", "
        << (ctex.es->shaders[i].precompile? "true" : "false")
        << " },\n";
	idmax += 1;
  }
