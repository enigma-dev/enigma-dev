...
inline bool iscomment(const string &n) {
  ...
  for (unsigned i = 2; i < n.length(); i++) {
    if (n[i] == '*' and n[i + 1] == '/') return (i + 2 >= n.length());
  }
  return true;
}
...
static inline void declare_scripts(std::ostream &wto, const GameData &game, const CompileState &state) {
  ...
  for (size_t i = 0; i < game.scripts.size(); i++) {
    ...
    scr->globargs =
        16;  //Fixes too many arguments error (scripts can be called dynamically with any number of arguments)
...
static inline void declare_extension_casts(std::ostream &wto, const ParsedExtensionVec &parsed_extensions) {
  ...
  for (unsigned i = 0; i < parsed_extensions.size(); i++) {
    if (!parsed_extensions[i].implements.empty()) {
      wto << "    " << parsed_extensions[i].implements << " *as_" << parsed_extensions[i].implements
          << "(object_basic* x) {\n";
...
static inline void declare_object_locals_class(std::ostream &wto, const ParsedExtensionVec &parsed_extensions) {
...
static inline void write_extension_casts(std::ostream &wto, const ParsedExtensionVec &parsed_extensions) {
  // Write extension cast methods; these are a temporary fix until the new instance system is in place.
  ...
  for (pos = 0; pos < addls.length(); pos++) {
    ...
    if (addls[pos] == '[') {
      ...
      while (cnt and ++pos < addls.length())
        if (addls[pos] == '[' or addls[pos] == '(')
          cnt++;
        else if (addls[pos] == ')' or addls[pos] == ']')
          cnt--;
...
static inline bool parent_declares(parsed_object *parent, const deciter decl) {
  for (parsed_object *obj = parent; obj != NULL; obj = obj->parent) {
    for (deciter it = obj->locals.begin(); it != obj->locals.end(); it++) {
      if (it->first == decl->first && it->second.prefix == decl->second.prefix &&
          it->second.type == decl->second.type && it->second.suffix == decl->second.suffix) {
        return true;
      }
    }
  }
...
static std::vector<std::pair<std::string, dectrip>> write_object_locals(language_adapter *lang, std::ostream &wto,
  ...
  wto << "\n    std::vector<std::byte> serialize() override {\n"
         "      auto bytes = "
      << (object->parent ? object->parent->name : "object_locals") << "::serialize();\n";
  ...
  wto << "\n    std::size_t deserialize_self(std::byte *iter) override {\n"
         "      auto len = "
      << (object->parent ? object->parent->name : "object_locals") << "::deserialize_self(iter);\n";
  ...
  wto << "\n    std::pair<OBJ_" << object->name
      << ", std::size_t> deserialize(std::byte *iter) {\n"
         "      OBJ_"
      << object->name
      << " result;\n"
         "      auto len = result.deserialize_self(iter);\n"
...
static inline void write_object_scripts(std::ostream &wto, parsed_object *object, const CompileState &state) {
  ...
  for (parsed_object::funcit it = object->funcs.begin(); it != object->funcs.end();
       it++)  //For each function called by this object
  {
    ...
    if (subscr != state.script_lookup.end()             // If we've got ourselves a script
        and
        subscr->second
            ->global_code) {  // And it has distinct code for use at the global scope (meaning it's more efficient locally)
      const char *comma = "";
...
static inline void write_object_timelines(std::ostream &wto, const GameData & /*game*/, parsed_object *object,
  ...
  for (parsed_object::tlineit it = object->tlines.begin(); it != object->tlines.end();
       it++)  //For each timeline potentially set by this object.
  {
...
static void declare_event_groups(std::ostream &wto, const parsed_object *object) {
  ...
  for (const ParsedEventGroup &event_stack : object->stacked_events) {
    wto << "    void myevent_" << event_stack.event_key.BaseFunctionName();
...
static void implement_event_groups(std::ostream &wto, const parsed_object *object) {
  wto << "\n// Stacked event bases and dispatchers\n";
  for (const ParsedEventGroup &event_stack : object->stacked_events) {
    wto << "void enigma::OBJ_" << object->name << "::myevent_" << event_stack.event_key.BaseFunctionName();
    if (event_stack.event_key.HasDispatcher()) wto << "_dispatcher";
    wto << "() {\n";
    for (ParsedEvent *event : event_stack) {
      ...
      const std::string logic =
          ev.HasDispatcher() ? ev.DispatcherCode("myevent_" + evname) : "myevent_" + evname + "();";
      PrintIndentedCode(wto, logic, indent);
      if (ev.HasSubCheck()) wto << "  }\n";
    }
    wto << "}\n";
  }
}

static inline void write_event_perform(std::ostream &wto, const EventData &events, parsed_object *object) {
  /* Event Perform Code */
  wto << "\n    // Event Perform Code\n";
...
static inline void write_object_unlink(std::ostream &wto, parsed_object *object) {
  ...
  for (const ParsedEventGroup &group : object->registered_events) {
    ...
    if (event.HasIteratorDeclareCode()) {
      ...
      wto << "      enigma::inst_iter *ENOBJ_ITER_myevent_" << event.FunctionName() << ";\n";
    }
  }
  ...
  for (const ParsedEventGroup &group : object->registered_events) {
    ...
    if (event.HasIteratorRemoveCode()) {
      if (!iscomment(event.IteratorRemoveCode())) wto << "      " << event.IteratorRemoveCode() << ";\n";
    } else {
...
static inline void write_object_constructors(std::ostream &wto, parsed_object *object) {
  ...
  */
  wto << "\n    OBJ_" << object->name
      << "(int enigma_genericconstructor_newinst_x = 0, int enigma_genericconstructor_newinst_y = 0, const int id = "
         "(enigma::maxid++)"
      << ", const int enigma_genericobjid = " << object->id << ", bool handle = true)";

  if (object->parent) {
    wto << ": OBJ_" << object->parent->name
        << "(enigma_genericconstructor_newinst_x,enigma_genericconstructor_newinst_y,id,enigma_genericobjid,false)";
  } else {
  ...
  for (const ParsedEventGroup &group : object->registered_events) {
    ...
    if (event.HasIteratorInitializeCode()) {
      if (!iscomment(event.IteratorInitializeCode())) wto << "      " << event.IteratorInitializeCode() << ";\n";
    } else {
...
static void write_object_destructor(std::ostream &wto, parsed_object *object) {
  ...
  for (const ParsedEventGroup &group : object->registered_events) {
    ...
    if (event.HasIteratorDeleteCode()) {
      if (!iscomment(event.IteratorDeleteCode())) wto << "      " << event.IteratorDeleteCode() << ";\n";
    } else {
...
static void write_object_class_body(parsed_object *object, language_adapter *lang, std::ostream &wto,
                                    const GameData &game, const CompileState &state) {
  ...
  if (!locals.empty()) {
    wto << "\n  const std::unordered_map<std::string_view, OBJ_" << object->name << "::Deserializer> OBJ_"
        << object->name << "::deserializers{\n";
    for (auto &[name, type] : locals) {
...
static inline void write_object_family(parsed_object *object, language_adapter *lang, std::ostream &wto,
                                       const GameData &game, const CompileState &state) {
...
static inline void write_object_class_bodies(language_adapter *lang, std::ostream &wto, const GameData &game,
...
static inline string resname(string name) { return name.empty() ? "-1" : name; }

static inline void write_object_data_structs(std::ostream &wto, const ParsedObjectVec &parsed_objects) {
  wto << "  std::vector<objectstruct> object_array() {\n";
  wto << "    static std::vector<objectstruct> objs = {\n" << std::fixed;
  for (parsed_object *object : parsed_objects) {
    wto << "    { " << resname(object->sprite_name) << ", " << resname(object->polygon_name) << ", " << object->solid
        << ", " << object->visible << ", " << object->depth << ", " << object->persistent << ", "
        << resname(object->mask_name) << ", " << resname(object->parent_name) << ", " << object->id << " },\n";
  }
...
static inline void write_script_implementations(ofstream &wto, const GameData &game, const CompileState &state,
                                                int mode);
static inline void write_timeline_implementations(ofstream &wto, const GameData &game, const CompileState &state);
static inline void write_event_bodies(ofstream &wto, const GameData &game, int mode,
...
static inline void write_object_functionality(const GameData &game, const CompileState &state, int mode) {
  ...
  ofstream wto(
      (codegen_directory / "Preprocessor_Environment_Editable/IDE_EDIT_objectfunctionality.h").u8string().c_str(),
      ios_base::out);

  wto << license;
  wto << endl << "#define log_xor || log_xor_helper() ||" << endl;
  wto << "struct log_xor_helper { bool value; };" << endl;
  wto << "template<typename LEFT> log_xor_helper operator ||(const LEFT &left, const log_xor_helper &xorh) { "
         "log_xor_helper nxor; nxor.value = (bool)left; return nxor; }"
      << endl;
  wto << "template<typename RIGHT> bool operator ||(const log_xor_helper &xorh, const RIGHT &right) { return "
         "xorh.value ^ (bool)right; }"
      << endl
      << endl;
...
static inline void write_script_implementations(ofstream &wto, const GameData &game, const CompileState &state,
  ...
  for (size_t i = 0; i < game.scripts.size(); i++) {
    ...
    print_to_file(state.parse_context, override_code.empty() ? upev.code : override_code,
                  override_synt.empty() ? upev.synt : override_synt, upev.strc, upev.strs, 2, wto);
    wto << "\n  return 0;\n}\n\n";
  }
}
...
static inline void write_timeline_implementations(ofstream &wto, const GameData &game, const CompileState &state) {
  ...
  for (const auto &tline : state.timeline_lookup) {
    for (const auto &moment : tline.second.moments) {
      ...
      auto &upev = (moment.script->global_code ? *moment.script->global_code : moment.script->code).ast.junkshit;
...
static void write_object_script_funcs(ofstream &wto, const parsed_object *const t,
                                      const ScriptLookupMap &script_lookup);
static void write_object_timeline_funcs(ofstream &wto, const GameData &game, const parsed_object *const t,
                                        const TimelineLookupMap &timeline_lookup);
static void write_can_cast_func(ofstream &wto, const parsed_object *const pobj);

static void write_event_bodies(ofstream &wto, const GameData &game, int mode, const ParsedObjectVec &parsed_objects,
                               const ScriptLookupMap &script_lookup, const TimelineLookupMap &timeline_lookup) {
...
static void write_event_func(ofstream &wto, const ParsedEvent &event, string objname, string evname, int mode) {
  ...
  if (!event.ev_id.UsesEventLoop()) wto << "  enigma::temp_event_scope ENIGMA_PUSH_ITERATOR_AND_VALIDATE(this);\n";
  if (event.ev_id.HasConstantCode()) PrintIndentedCode(wto, event.ev_id.ConstantCode(), 2);
...
static inline void write_object_script_funcs(ofstream &wto, const parsed_object *const t,
                                             const ScriptLookupMap &script_lookup) {
  for (parsed_object::const_funcit it = t->funcs.begin(); it != t->funcs.end();
       ++it) {                                    // For each function called by this object
...
static inline void write_known_timelines(ofstream &wto, const GameData &game, const parsed_object *const t,
                                         const TimelineLookupMap &timeline_lookup);
static inline void write_object_timeline_funcs(ofstream &wto, const GameData &game, const parsed_object *const t,
                                               const TimelineLookupMap &timeline_lookup) {
  bool hasKnownTlines = false;
  for (parsed_object::const_tlineit it = t->tlines.begin(); it != t->tlines.end();
       ++it) {                                     //For each timeline potentially set by this object
    auto timit = timeline_lookup.find(it->first);  //Check if it's a timeline
    if (timit != timeline_lookup.end()) {          // If we've got ourselves a timeline
      ...
      for (const auto &moment : timit->second.moments) {
        ...
        wto << "void enigma::OBJ_" << t->name << "::TLINE_" << timit->first << "_MOMENT_" << moment.step << "() {\n";
...
static inline void write_basic_constructor(ofstream &wto, const CompileState &state) {
  ...
  wto << "  void constructor(object_basic* instance_b) {\n"
      ...
      << (copts.compliance_mode <= setting::COMPL_GM7 ? "true" : "false")
      << ";\n"
