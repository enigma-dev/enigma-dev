...
int lang_CPP::compile_writeDefraggedEvents(const GameData &game, const std::set<EventGroupKey> &used_events,
  ...
  for (size_t i = 0; i < game.timelines.size(); i++) {
    for (int j = 0; j < game.timelines[i]->moments().size(); j++) {
      wto << "void TLINE_" << game.timelines[i].name << "_MOMENT_" << game.timelines[i]->moments(j).step() << "();\n";
    }
  }
  wto << "\n";
  ...
  *****************************************************************/
  ...
  wto << "  {" << endl;
  wto << "    std::vector<std::byte> serialize() override { return " << system_get_uppermost_tier()
      << "::serialize(); }\n\n";
  wto << "    std::size_t deserialize_self(std::byte *iter) override { return " << system_get_uppermost_tier()
      << "::deserialize_self(iter); }\n\n";
  ...
  for (const auto &event : used_events) {
    ...
    if (event.HasSubCheck() && !e_is_inst) {
      if (event.HasLocalDeclarations()) {
        wto << "    virtual bool myevent_" << fname << "_subcheck() { return false; }";
      } else {
    ...
    wto << "    virtual " << (e_is_void ? "void" : "variant") << " myevent_" << fname
        << (e_has_dispatch ? "_dispatcher()" : "()");
    if (event.HasDefaultCode()) {
      wto << " {" << endl
          << "  " << event.DefaultCode() << endl
          << (e_is_void ? "    }" : "      return 0;\n    }") << endl;
    } else {
      wto << (e_is_void ? " { } // No default " : " { return 0; } // No default ") << event.HumanName() << " code."
          << endl;
    }
  }
  ...
  for (size_t i = 0; i < game.timelines.size(); i++) {
    ...
    for (int j = 0; j < game.timelines[i]->moments().size(); j++) {
      wto << "            case " << j << ": {\n";
      wto << "              ::TLINE_" << game.timelines[i].name << "_MOMENT_" << game.timelines[i]->moments(j).step()
          << "();\n";
      wto << "              break;\n";
      wto << "            }\n";
    }
  ...
  *****************************************************************************/
  ...
  for (const auto &event : used_events) wto << "  event_iter *event_" << event.FunctionName() << ";" << endl;

  // Here's the initializer
  wto << "  int event_system_initialize()" << endl << "  {" << endl;
  wto << "    events = new event_iter[" << used_events.size() << "]; // Allocated here; not really meant to change."
      << endl;
  ...
  wto << "    objects = new objectid_base[" << (obj_high_id + 1) << "]; // Allocated here; not really meant to change."
      << endl;
  ...
  if (!game.settings.general().show_cursor()) wto << "    window_set_cursor(cr_none);" << endl;

  if (game.settings.windowing().stay_on_top()) wto << "    window_set_stayontop(true);" << endl;
  ...
  for (const auto &event : used_events) {
    if (event.HasSuperCheckFunction()) {
      wto << "  static inline bool supercheck_" << event.FunctionName() << "() " << event.SuperCheckFunction()
          << "\n\n";
    }
  }
  ...
  *****************************************************************************/
  ...
  for (const EventGroupKey &event : used_events) {
    ...
    if (((EventDescriptor &)event).HasInsteadCode()) {
      ...
      wto << base_indent << "  for (instance_event_iterator = event_" << fname
          << "->next; instance_event_iterator != NULL; instance_event_iterator = instance_event_iterator->next) {\n";
      if (callsubcheck) {
        wto << base_indent << "    if (((enigma::event_parent*)(instance_event_iterator->inst))->myevent_" << fname
            << "_subcheck()) {\n";
      }
    ...
    wto << base_indent << endl << base_indent << "enigma::update_globals();" << endl << base_indent << endl;
  }
  ...
  {
    wto << "    //if (keyboard_check_pressed(vk_f5)) game_save('_save" << game.settings.general().game_id() << ".sav');"
        << endl;
    wto << "    //if (keyboard_check_pressed(vk_f6)) game_load('_save" << game.settings.general().game_id() << ".sav');"
        << endl;
  }
