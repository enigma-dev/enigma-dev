 ...
 *
 *
 * //You forget that this thing programs other things. wait, wghat were you typing
 // If you fuck something up in the strings that contain code for this thing to enter in the new games,
 // It may have worked here, but certainly won't in the game
 //okay, let me teach you something. this is compiled by g++, correct?
 //yes. you don't have to tell me.
 //and eventually, the games are subject to a compiler such as g++
 //which follows essentially the same rules regarding pre-processing directives
 //therefore, shit don't matter one way or another. It does if you're defining macros here and not there
 //then let's simply TEST it to prove my point. WELL, I'd rather it worked in the first place before we complicate my life further.
 //okay. But apparently, G++ still freezes, even when told to log shit.
 //hrm. hang on.
 //thinking...
 //also do whatever you were gonna do with taskmgr
 //press winkey+r for me real quick.
 //should have known. wait, why is there a fucking pagefile in your root directory? haha. O_O?
 ...
 * code look a little less messy, and a lot more simple.
 */
char* writehere;
FILE* enigma_file;
int EXPECTNULL=0;
 ...
 */
   struct scriptdata
   {
       ...
       map<int,string> varnamestars;
       map<int,string> varnametags;
       map<int,string> varnamearrays;
...
double CompileEGMf(string filename,string outname,int debug=0,int build=0,int fullcomp=0)
{
  ...
  **************************************************************/
      ...
      for(int i=0;i<script_count;i++)
      {
         ...
         for(int ii=0;ii<__varcount;ii++)
         {
           scripts[i].varnames[ii]=__varnames[ii];
           scripts[i].varnamestars[ii]=__varnamestars[ii];
           scripts[i].varnametags[ii]=__varnametags[ii];
           scripts[i].varnamearrays[ii]=__varnamearrays[ii];
         }
         for(int ii=0;ii<__fcount;ii++)
         scripts[i].fnames[ii]=__fnames[ii];

         scripts[i].varcount=__varcount;
         scripts[i].fcount=__fcount;

         if (_errorlast != 0) return _errorlast;
      }
  ...
  **************************************************************/
      ...
      for(int i=0;i<object_count;i++)
      {
         ...
         {
            //Read the name
            objects[i].events[ii].name=reads(namelen);
            //Read the null
            int_read=readi();
            ...
            for(int iii=0;iii<__varcount;iii++)
            {
              ...
              objects[i].events[ii].varnamestars[iii]=__varnamestars[iii];
            ...
            {
              int scrfind=-1;
              for (int iv=0;iv<script_count;iv++) if (scripts[iv].name==__fnames[iii]) { scrfind=iv; break; }
              if (scrfind==-1)
              {
                printf(
"***********************************************************************************************\n\
...
************************************************************************************************\n",
                __fnames[iii].c_str()); fflush(stdout);
                return -11;
              }
              else //if the script exists
              {
                ...
                for (int iv=0;iv<objects[i].scrcount;iv++)
                  if (objects[i].scriptnames[iv]==__fnames[iii]) { scrinobj1=1; break; }
                if (scrinobj1 != -1)
                {
                  objects[i].scriptnames[objects[i].scrcount]=__fnames[iii];
                  objects[i].scrcount++;
                  ...
                  for (int iv=0;iv<scripts[scrfind].varcount;iv++)
                  {
                    ...
                    if (!alrin)
                    {
                      ...
                      objects[i].events[ii].varnamestars[tvc]=scripts[scrfind].varnamestars[iv];
                      objects[i].events[ii].varnametags[tvc]=scripts[scrfind].varnametags[iv];
                      objects[i].events[ii].varnamearrays[tvc]=scripts[scrfind].varnamearrays[iv];
  ...
  *************************************************************/
      ...
      for (int i=0;i<room_count;i++)
      {
          ...
          // Syntax check and parse the code
              ...
              for (int ii=0;ii<__varcount;ii++)
              {
                if (!varname_onlists(__varnames[ii]))
                {
                  if (__varnametags[ii]=="") vardecs+="var "+__varnames[ii]+";\n";
                  else                       vardecs+=__varnametags[ii]+" "+__varnames[ii]+";\n";
                }
              }
              __varcount=0;
  ...
  **************************************************************/
      ...
      *********************************************************************/
      ...
      for (int i=0;i<object_count;i++)
      {
         ...
         int write_count=0;
         map<int,string> to_write,to_write_stars,to_write_tags,to_write_arrays;
         for (int ii=0;ii<objects[i].eventcount;ii++)
         {
             for (int iii=0;iii<objects[i].events[ii].varcount;iii++)
             {
                 ...
                 vstars=objects[i].events[ii].varnamestars[iii];
                 vtags=objects[i].events[ii].varnametags[iii];
                 varrays=objects[i].events[ii].varnamearrays[iii];
         ...
         for (int ii=0;ii<objects[i].eventcount;ii++)
         {
            ...
            if (objects[i].events[ii].name=="mouse")
            {
               ...
               switch (addind)
               {
                 ...
                 case 60:
                   mouselist+="  for (int enigma__ACALLBACKITERATOR=enigma::mousewheel;enigma__ACALLBACKITERATOR>0;enigma__ACALLBACKITERATOR-=120) "; break;
                 case 61:
                   mouselist+="  for (int enigma__ACALLBACKITERATOR=(-enigma::mousewheel);enigma__ACALLBACKITERATOR>0;enigma__ACALLBACKITERATOR-=120) "; break;
...
JOYSTICK2 BUTTON5 = 40;  JOYSTICK2 BUTTON6 = 41;  JOYSTICK2 BUTTON7 = 42;  JOYSTICK2 BUTTON8 = 43;*/
  ...
  **************************************************************/
      ...
      *************************************************************************/
      ...
      map<int,string> alltypes,allnames,allstars,allarrays; int allvarcount=0;
      for (int i=0;i<object_count;i++)
      {
         for (int ii=0; ii<objects[i].eventcount; ii++)
         {
            for (int iii=0;iii<objects[i].events[ii].varcount; iii++)
            {
                ...
                {
                   on=1;
                   if (alltypes[iv]!=objects[i].events[ii].varnametags[iii] && objects[i].events[ii].varnametags[iii]!="" && alltypes[iv]!="")
                   {
                ...
                if (!on) for(int iv=0;iv<localc;iv++)
                  if (localn[iv]==objects[i].events[ii].varnames[iii])
                  { on=1; break; }
                if (!on) for(int iv=0;iv<object_count;iv++)
                  if (objects[iv].name==objects[i].events[ii].varnames[iii])
                  { on=1; break; }
                if (!on) for(int iv=0;iv<sprite_count;iv++)
                ...
                if (!on)
                {
                   ...
                   allstars[allvarcount]=objects[i].events[ii].varnamestars[iii];
                   if (objects[i].events[ii].varnametags[iii]!="")
                   alltypes[allvarcount]=objects[i].events[ii].varnametags[iii];
                   else alltypes[allvarcount]="var";
                   allarrays[allvarcount]=objects[i].events[ii].varnamearrays[iii];
                   allvarcount++;
                }
            }
         }
      ...
      for (int i=0;i<__globalcount;i++)
      {
          int on=0;
          for(int ii=0;ii<allvarcount;ii++)
          if (allnames[ii]==__globals[i])
          {
            on=1;
            if (alltypes[ii]!=__globaltags[i] && __globaltags[i]!="" && alltypes[ii]!="")
            {
          ...
          if (!on) on=varname_onlists(__globals[i]);
          if (!on)
          {
            allnames[allvarcount]=__globals[i];
            alltypes[allvarcount]=__globaltags[i];
            allstars[allvarcount]=__globalstars[i];
            allarrays[allvarcount]=__globalarrays[i];
      ...
      map<int,string> alltypenames,alltypestars,alltypearrays; int alltypecount=0;
      for (int i=0; i<allvarcount; i++)
      {
        ...
        if (!isatype) { alltypenames[alltypecount]=alltypes[i];
                        ...
                        alltypestars[alltypecount]=allstars[i];
                        alltypecount++; }
      }
      ...
      for (int ii=0; ii<allvarcount; ii++)
      {
         for (int iii=0; iii<__globalcount;iii++)
         {
            if (!varname_oninheriteds(__globals[iii]))
            if (__globals[iii]==allnames[ii])
            {
              __globaltags[iii]=alltypes[ii];
              __globalstars[iii]=allstars[ii];
              __globalarrays[iii]=allarrays[ii];
            }
         }
      }
...
\\*********************************************************************************/\r\n\r\n\r\n",IDE_EDIT_wildclass);
      ...
      for (int i=0; i<alltypecount; i++)
      {
        ...
        string_replace_all(string_replace_all(alltypestars[i],"*","__star_"),"(","__parenth_").c_str(),
      ...
      *********************************************************************************************/
      ...
      for (int ii=0; ii<allvarcount; ii++)
      {
         ...
         string_replace_all(string_replace_all(allstars[ii],"*","__star_"),"(","__parenth_").c_str(),
  ...
  *************************************************************************/
      ...
      for (int i=0; i<__globalcount;i++)
      {
         if (!varname_oninheriteds(__globals[i]))
         {
           if (__globaltags[i]=="")
             writes(IDE_EDIT_withdeclarations,"var "+__globals[i]+"; \r\n");
           else
             fprintf(IDE_EDIT_withdeclarations,"%s %s%s%s; \r\n",__globaltags[i].c_str(),__globalstars[i].c_str(),__globals[i].c_str(),__globalarrays[i].c_str());
         }
      }
