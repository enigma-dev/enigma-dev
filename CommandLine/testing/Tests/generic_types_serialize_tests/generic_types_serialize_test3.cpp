...
TEST(enigma_internal_sizeof_function, Test_pair) {
  std::pair<int, char> pair1 = {1, 2};
  ASSERT_EQ(ebs::enigma_internal_sizeof(pair1), 5);

  std::pair<double, long> pair2 = {12232.324342, 13123};
  ASSERT_EQ(ebs::enigma_internal_sizeof(pair2), 16);

  std::pair<char, bool> pair3 = {'a', false};
  ASSERT_EQ(ebs::enigma_internal_sizeof(pair3), 2);
}

TEST(enigma_internal_sizeof_function, Test_lua_table) {
  ...
  ASSERT_EQ(ebs::enigma_internal_sizeof(lua_table1), 64);  // 8+ 8+ 10*4+ 8
  ...
  ASSERT_EQ(ebs::enigma_internal_sizeof(lua_table2), 39);  // 8+ 8+ 15+ 8
  ...
  ASSERT_EQ(ebs::enigma_internal_sizeof(lua_table3), 184);  // 8+ 8+ 20*8+ 8
}
...
TEST(serialize_deserialize_functions, Test_pair1) {
  ...
  ebs::enigma_serialize(inputPair, len, bytes);
  ...
  len = 0;
  ebs::enigma_deserialize(outputPair, iter, len);
...
TEST(serialize_deserialize_functions, Test_pair5) {
  ...
  ebs::enigma_serialize(inputPair1, len, bytes);
  ASSERT_EQ(bytes.size(), 5);

  ebs::enigma_serialize(inputPair2, len, bytes);
  ...
  std::size_t len1 = 0;
  ebs::enigma_deserialize(outputPair1, iter, len1);
  ASSERT_EQ(outputPair1, inputPair1);

  std::pair<double, bool> outputPair2;
  ebs::enigma_deserialize(outputPair2, iter, len1);
...
TEST(serialize_deserialize_functions, Test_pair6) {
  ...
  ebs::enigma_deserialize(outputPair, iter, len1);
  ASSERT_EQ(outputPair, inputPair);
}
