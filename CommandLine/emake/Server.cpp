...
class CompilerServiceImpl final : public Compiler::Service {
  ...
  Status SyntaxCheck(ServerContext* /*context*/, const SyntaxCheckRequest* request, SyntaxError* reply) override {
    ...
    std::vector<std::string> script_names_str;
    script_names_str.reserve(request->script_names().size());
    for (std::string_view str : request->script_names()) {
      script_names_str.push_back(std::string(str));
      script_names.push_back(script_names_str.back().c_str());
    }
