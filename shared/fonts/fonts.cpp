...
static std::unordered_map<std::string, std::filesystem::path> fontNames;
static std::unordered_map<std::string, std::filesystem::path> fontFamilies;
...
bool font_load(const std::filesystem::path& fName, unsigned size, RawFont& rawFont) {
...
void font_add_search_path(const std::filesystem::path& path, bool recursive) {
  if (std::find(fontSearchPaths.begin(), fontSearchPaths.end(), path) == fontSearchPaths.end()) { 
    ...
    } else DEBUG_MESSAGE("Freetype warning searching non-existing directory: " + path.u8string(), MESSAGE_TYPE::M_WARNING);
  }
}
